...
namespace {
inline dfloat distance(dfloat x1, dfloat x2, dfloat y1, dfloat y2, dfloat z1,
                       dfloat z2) {
...
inline void computeDirection(dfloat x1, dfloat x2, dfloat y1, dfloat y2,
                             dfloat z1, dfloat z2, dfloat *direction) {
  direction[0] = x1 - x2;
  direction[1] = y1 - y2;
  direction[2] = z1 - z2;
...
bool checkIfRecompute(nrs_t *nrs, int tstep) {
  ...
  platform->linAlg->axpbyzMany(mesh->Nlocal, nPropertyFields, nrs->fieldOffset,
                               1.0, nrs->o_prop, -1.0, nrs->o_prevProp,
                               platform->o_mempool.slice0);

  const dfloat delta = platform->linAlg->norm2Many(
...
bool apply(nrs_t *nrs, int tstep, dfloat time) {
  ...
  nrs->computeFieldDotNormalKernel(mesh->Nlocal, nrs->fieldOffset,
                                   nrs->flowDirection[0], nrs->flowDirection[1],
                                   nrs->flowDirection[2], nrs->o_U,
                                   o_currentFlowRate);
  ...
  const dfloat currentFlowRate =
      platform->linAlg->sum(mesh->Nlocal, o_currentFlowRate,
...
void compute(nrs_t *nrs, double lengthScale, dfloat time) {
  ...
  platform->timer.tic("pressureSolve", 1);
  {
    ...
    nrs->gradientVolumeKernel(mesh->Nelements, mesh->o_vgeo, mesh->o_D,
                              nrs->fieldOffset, nrs->o_ellipticCoeff,
                              o_gradPCoeff);
    nrs->computeFieldDotNormalKernel(mesh->Nlocal, nrs->fieldOffset,
                                     flowDirection[0], flowDirection[1],
                                     flowDirection[2], o_gradPCoeff, o_Prhs);

    // enforce Dirichlet BCs
    platform->linAlg->fill(nrs->fieldOffset, std::numeric_limits<dfloat>::min(),
                           platform->o_mempool.slice6);
    for (int sweep = 0; sweep < 2; sweep++) {
      nrs->pressureDirichletBCKernel(
          mesh->Nelements, time, nrs->fieldOffset, mesh->o_sgeo, mesh->o_x,
          mesh->o_y, mesh->o_z, mesh->o_vmapM, mesh->o_EToB, nrs->o_EToB,
          nrs->o_usrwrk, nrs->o_U, platform->o_mempool.slice6);

      // take care of Neumann-Dirichlet shared edges across elements
      if (sweep == 0)
        oogs::startFinish(platform->o_mempool.slice6, 1, nrs->fieldOffset,
                          ogsDfloat, ogsMax, nrs->gsh);
      if (sweep == 1)
        oogs::startFinish(platform->o_mempool.slice6, 1, nrs->fieldOffset,
                          ogsDfloat, ogsMin, nrs->gsh);
    }

    if (nrs->pSolver->Nmasked)
      nrs->maskCopyKernel(nrs->pSolver->Nmasked, 0, nrs->pSolver->o_maskIds,
                          platform->o_mempool.slice6, nrs->o_Pc);
  ...
  platform->timer.tic("velocitySolve", 1);
  {
    ...
    o_BF.copyFrom(mesh->o_LMM, mesh->Nlocal * sizeof(dfloat),
                  0 * nrs->fieldOffset * sizeof(dfloat), 0);
    o_BF.copyFrom(mesh->o_LMM, mesh->Nlocal * sizeof(dfloat),
                  1 * nrs->fieldOffset * sizeof(dfloat), 0);
    o_BF.copyFrom(mesh->o_LMM, mesh->Nlocal * sizeof(dfloat),
                  2 * nrs->fieldOffset * sizeof(dfloat), 0);
    nrs->gradientVolumeKernel(mesh->Nelements, mesh->o_vgeo, mesh->o_D,
                              nrs->fieldOffset, nrs->o_Pc,
                              o_RhsVel // <- rhs = -\grad{P_c}
                              );

    // rhs = -\grad{P_c} + BF n_i
    platform->linAlg->scaleMany(mesh->Nlocal, nrs->NVfields, nrs->fieldOffset,
                                -1.0, o_RhsVel);

    for (int dim = 0; dim < ndim; ++dim) {
      const dlong offset = dim * nrs->fieldOffset;
      const dfloat n_dim = nrs->flowDirection[dim];
      platform->linAlg->axpby(mesh->Nlocal, n_dim, o_BF, 1.0, o_RhsVel, offset,
                              offset);
    }

    if (nrs->uvwSolver) {
      nrs->maskKernel(nrs->uvwSolver->Nmasked, nrs->uvwSolver->o_maskIds,
                      o_RhsVel);
    } else {
      if (nrs->uSolver)
        nrs->maskKernel(nrs->uSolver->Nmasked, nrs->uSolver->o_maskIds,
                        o_RhsVel + 0 * nrs->fieldOffset * sizeof(dfloat));
      if (nrs->vSolver)
        nrs->maskKernel(nrs->vSolver->Nmasked, nrs->vSolver->o_maskIds,
                        o_RhsVel + 1 * nrs->fieldOffset * sizeof(dfloat));
      if (nrs->wSolver)
        nrs->maskKernel(nrs->wSolver->Nmasked, nrs->wSolver->o_maskIds,
    ...
    for (int sweep = 0; sweep < 2; sweep++) {
      ...
      nrs->velocityDirichletBCKernel(
          mesh->Nelements, nrs->fieldOffset, time, mesh->o_sgeo, mesh->o_x,
          mesh->o_y, mesh->o_z, mesh->o_vmapM, mesh->o_EToB, nrs->o_EToB,
          nrs->o_VmapB, nrs->o_usrwrk, nrs->o_Uc, platform->o_mempool.slice3);

      nrs->velocityNeumannBCKernel(
          mesh->Nelements, nrs->fieldOffset, mesh->o_sgeo, mesh->o_vmapM,
          mesh->o_EToB, nrs->o_EToB, time, mesh->o_x, mesh->o_y, mesh->o_z,
          nrs->o_usrwrk, nrs->o_U, platform->o_mempool.slice3);

      // take care of Neumann-Dirichlet shared edges across elements
      if (sweep == 0)
        oogs::startFinish(platform->o_mempool.slice3, nrs->NVfields,
                          nrs->fieldOffset, ogsDfloat, ogsMax, nrs->gsh);
      if (sweep == 1)
        oogs::startFinish(platform->o_mempool.slice3, nrs->NVfields,
                          nrs->fieldOffset, ogsDfloat, ogsMin, nrs->gsh);
    }
    if (nrs->uvwSolver) {
      if (nrs->uvwSolver->Nmasked)
        nrs->maskCopyKernel(nrs->uvwSolver->Nmasked, 0 * nrs->fieldOffset,
                            nrs->uvwSolver->o_maskIds,
                            platform->o_mempool.slice3, nrs->o_Uc);
    } else {
      if (nrs->uSolver->Nmasked)
        nrs->maskCopyKernel(nrs->uSolver->Nmasked, 0 * nrs->fieldOffset,
                            nrs->uSolver->o_maskIds, platform->o_mempool.slice3,
                            nrs->o_Uc);
      if (nrs->vSolver->Nmasked)
        nrs->maskCopyKernel(nrs->vSolver->Nmasked, 1 * nrs->fieldOffset,
                            nrs->vSolver->o_maskIds, platform->o_mempool.slice3,
                            nrs->o_Uc);
      if (nrs->wSolver->Nmasked)
        nrs->maskCopyKernel(nrs->wSolver->Nmasked, 2 * nrs->fieldOffset,
                            nrs->wSolver->o_maskIds, platform->o_mempool.slice3,
                            nrs->o_Uc);
    }
