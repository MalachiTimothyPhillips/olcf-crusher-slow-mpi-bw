...
occa::memory velocitySolve(nrs_t* nrs, dfloat time, int stage)
{
  ...
  if(nrs->uvwSolver) {
    const occa::memory& o_U0 = (nrs->uvwSolver->options.compareArgs("INITIAL GUESS", "EXTRAPOLATION") && stage == 1) ?
                               nrs->o_Ue : nrs->o_U;
    ...
    const occa::memory& o_U0 = (nrs->uSolver->options.compareArgs("INITIAL GUESS", "EXTRAPOLATION") && stage == 1) ?
    ...
    const occa::memory& o_V0 = (nrs->vSolver->options.compareArgs("INITIAL GUESS", "EXTRAPOLATION") && stage == 1) ?
    ...
    const occa::memory& o_W0 = (nrs->wSolver->options.compareArgs("INITIAL GUESS", "EXTRAPOLATION") && stage == 1) ?
