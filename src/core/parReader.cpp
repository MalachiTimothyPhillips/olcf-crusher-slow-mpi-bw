...
void parsePreconditioner(const int rank, setupAide &options,
  ...
  if(p_preconditioner.find("auto") != std::string::npos){
    ...
    options.setArgs(parSection + " AUTO PRECONDITIONER MAX TRIALS", std::to_string(maxTrials));
    options.setArgs(parSection + " AUTO PRECONDITIONER SAMPLING", sampling);
    ...
    for (std::string s : list) {
      if(s.find("frequency") != std::string::npos){
        ...
        const int value = std::stoi(params[1]);
        options.setArgs(parSection + " AUTO PRECONDITIONER TRIAL FREQUENCY",
                        std::to_string(value));
      }
      if(s.find("start") != std::string::npos){
        ...
        const int value = std::stoi(params[1]);
        options.setArgs(parSection + " AUTO PRECONDITIONER START",
                        std::to_string(value));
      }
      if(s.find("mindegree") != std::string::npos){
        ...
        const int value = std::stoi(params[1]);
        options.setArgs(parSection + " AUTO PRECONDITIONER MIN CHEBY ORDER",
                        std::to_string(value));
      }
      if(s.find("maxdegree") != std::string::npos){
        ...
        const int value = std::stoi(params[1]);
        options.setArgs(parSection + " AUTO PRECONDITIONER MAX CHEBY ORDER",
                        std::to_string(value));
      }
