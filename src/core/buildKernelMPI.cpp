...
int fileBcastNodes(const fs::path srcPath, const fs::path dstPath, 
                   int rankCompile, MPI_Comm comm, int verbose)
{
  int rank;
  ...
  if(rank == rankCompile && !fs::exists(srcPath)) {
    err++;
    std::cout << __func__ << ": cannot stat " << "" << srcPath << ":"
  ...
  MPI_Comm commLocal; 
  {
    ...
    if(localRank == localRankRoot) color = 1;
    if(rank == rankCompile) color = 1;
  }
  MPI_Comm commNode;
  ...
  int nodeRank = -1;
  int nodeRankRoot = -1;
  if(color != MPI_UNDEFINED) MPI_Comm_rank(commNode, &nodeRank);
  if(rank == rankCompile) nodeRankRoot = nodeRank;
  ...
  for(int i=0; i < nFiles; i++) {
    ...
    if(rank == rankCompile) std::strncpy(buf, fileList.at(i).c_str(), bufSize); 
    MPI_Bcast(buf, bufSize, MPI_CHAR, rankCompile, comm);
    if(rank != rankCompile) fileList.push_back(std::string(buf, 0, bufSize));
    free(buf);
  }
  ...
  for(const auto& file : fileList) {
    ...
    if(color != MPI_UNDEFINED) {
      if(nodeRank == nodeRankRoot) bufSize = fs::file_size(file);
      MPI_Bcast(&bufSize, 1, MPI_INT, nodeRankRoot, commNode);

      if(bufSize > std::numeric_limits<int>::max()) {
        if(rank == rankCompile) std::cout << __func__ << ": file size of " << "" << file << " too large!\n";
        return EXIT_FAILURE;
      }
    ...
    MPI_File fh;
    MPI_File_open(commLocal, filePath.c_str(), 
...
occa::kernel buildKernelFromStringMPI(const std::string& kernelSource, const std::string& kernelName, 
                                      const occa::properties& props, MPI_Comm comm, int rankCompile = 0)
{
  int rank;
  MPI_Comm_rank(comm, &rank);
  ...
  std::string srcPath;
  if(rank == rankCompile) {
    occa::kernel knl = platform->device.occaDevice().buildKernelFromString(kernelSource, kernelName, props);
    srcPath = fs::path(knl.sourceFilename()).parent_path();
  }

  // bcast srcPath
  int srcPathSize = (rank == rankCompile) ? srcPath.size() : 0;
  MPI_Bcast(&srcPathSize, 1, MPI_INT, rankCompile, comm);
  auto buf = (char*) std::malloc(srcPathSize * sizeof(char));
  if(rank == rankCompile) std::strncpy(buf, srcPath.c_str(), srcPathSize);
  MPI_Bcast(buf, srcPathSize, MPI_CHAR, rankCompile, comm);
  ...
  std::string uniquePath;
  {
    ...
    MPI_Bcast(&uniquePathSize, 1, MPI_INT, rankCompile, comm);
    auto buf = (char*) std::malloc(uniquePathSize * sizeof(char));
    if(rank == rankCompile) std::strncpy(buf, uniquePath.c_str(), uniquePathSize);
    MPI_Bcast(buf, uniquePathSize, MPI_CHAR, rankCompile, comm);
...

  {
    ...
    int err = fileBcastNodes(fs::relative(srcPath, fs::current_path()), dstPath, rankCompile, comm, true); 
    MPI_Allreduce(MPI_IN_PLACE, &err, 1, MPI_INT, MPI_SUM, comm);
...
occa::kernel buildKernelMPI(const std::string& kernelFile, const std::string& kernelName, 
                            const occa::properties& props, MPI_Comm comm, int rankCompile = 0)
{
  int rank;
  MPI_Comm_rank(comm, &rank);
  ...
  int err = 0;
  if(rank == rankCompile && !fs::exists(kernelFile)) {
    err++;
    std::cout << __func__ << ": cannot stat " << "" << kernelFile << ":"
              << " No such file or directory\n"; 
  }
  MPI_Allreduce(MPI_IN_PLACE, &err, 1, MPI_INT, MPI_SUM, comm);
  if(err) MPI_Abort(comm, EXIT_FAILURE);

  int kernelContentSize = (rank == rankCompile) ? fs::file_size(kernelFile) : 0;
  MPI_Bcast(&kernelContentSize, 1, MPI_INT, rankCompile, comm);
  ...
  MPI_Bcast(buf, kernelContentSize, MPI_CHAR, rankCompile, comm);
  const std::string kernelFileContent(buf, kernelContentSize); // buf not null-terminated 
  free(buf);

  return buildKernelFromStringMPI(kernelFileContent, kernelName, props, comm, rankCompile);
}
