...
void comm_t::reallocScratch(int Nbytes) const
{
...
{
  int sizeBytes;
  MPI_Type_size(datatype, &sizeBytes);

  const int Nbytes = sizeBytes * count;

  reallocScratch(Nbytes);

  if(useGPUAware){
    return MPI_Allreduce((void*) sendbuf.ptr(), (void*) recvbuf.ptr(), count, datatype, op, comm);
  } else {
    int retVal = 0;

    sendbuf.copyTo(send, Nbytes);
    retVal = MPI_Allreduce(send, recv, count, datatype, op, comm);
                ...
                MPI_Datatype datatype, MPI_Op op, MPI_Comm comm) const
{
  ...
  if(useGPUAware){
    return MPI_Allreduce(MPI_IN_PLACE, (void*) recvbuf.ptr(), count, datatype, op, comm);
  } else {
    int retVal = 0;

    recvbuf.copyTo(recv, Nbytes);
    retVal = MPI_Allreduce(MPI_IN_PLACE, recv, count, datatype, op, comm);
    recvbuf.copyFrom(recv, Nbytes);
