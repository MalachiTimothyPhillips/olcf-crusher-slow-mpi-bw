...
cds_t* cdsSetup(nrs_t* nrs, setupAide options, occa::properties& kernelInfoBC)
{
  ...
  for (int is = 0; is < cds->NSfields; is++) {
    ...
    cds->options[is].setArgs("SCALING COEFF", options.getArgs("SCALAR" + sid + " SCALING COEFF"));
    cds->options[is].setArgs("HPF RESIDUAL", options.getArgs("SCALAR" + sid + " HPF RESIDUAL"));
