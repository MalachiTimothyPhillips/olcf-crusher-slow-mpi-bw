...
std::vector<int> determineMGLevels(std::string section, int version)
{
  ...
  if (platform->options.compareArgs("PRESSURE AUTO PRECONDITIONER", "TRUE")) {
    ...
    // TODO: ensure that levels are shared between each type...
...
void nrsSetup(MPI_Comm comm, setupAide &options, nrs_t *nrs)
{
  ...
  *(nrs->kernelInfo) = platform->kernelInfo;
  ...
  nrs->o_ellipticCoeff = device.malloc(2 * nrs->fieldOffset * sizeof(dfloat), nrs->ellipticCoeff);
...

  {
    ...
    nrs->nStagesSum3Kernel = platform->kernels.getKernel(section + kernelName);

    kernelName = "computeFieldDotNormal";
    nrs->computeFieldDotNormalKernel = platform->kernels.getKernel(section + kernelName);

    kernelName = "computeFaceCentroid";
    nrs->computeFaceCentroidKernel = platform->kernels.getKernel(section + kernelName);

    {
      kernelName = "strongAdvectionVolume" + suffix;
      nrs->advectionStrongVolumeKernel = platform->kernels.getKernel(section + kernelName);
      kernelName = "strongAdvectionCubatureVolume" + suffix;
      nrs->advectionStrongCubatureVolumeKernel = platform->kernels.getKernel(section + kernelName);
    }

    kernelName = "curl" + suffix;
    nrs->curlKernel = platform->kernels.getKernel(section + kernelName);
    ...
    nrs->wgradientVolumeKernel = platform->kernels.getKernel(section + kernelName);
    ...
    nrs->wDivergenceVolumeKernel = platform->kernels.getKernel(section + kernelName);
    kernelName = "divergenceVolume" + suffix;
    nrs->divergenceVolumeKernel = platform->kernels.getKernel(section + kernelName);

    kernelName = "divergenceSurfaceTOMBO" + suffix;
    nrs->divergenceSurfaceKernel = platform->kernels.getKernel(section + kernelName);

    kernelName = "advectMeshVelocityHex3D";
    nrs->advectMeshVelocityKernel = platform->kernels.getKernel(section + kernelName);

    kernelName = "pressureRhsTOMBO" + suffix;
    nrs->pressureRhsKernel = platform->kernels.getKernel(section + kernelName);

    kernelName = "pressureStress" + suffix;
    nrs->pressureStressKernel = platform->kernels.getKernel(section + kernelName);

    kernelName = "pressureDirichletBC" + suffix;
    nrs->pressureDirichletBCKernel = platform->kernels.getKernel(section + kernelName);

    kernelName = "velocityRhsTOMBO" + suffix;
    nrs->velocityRhsKernel = platform->kernels.getKernel(section + kernelName);

    kernelName = "velocityDirichletBC" + suffix;
    nrs->velocityDirichletBCKernel = platform->kernels.getKernel(section + kernelName);

    kernelName = "velocityNeumannBC" + suffix;
    nrs->velocityNeumannBCKernel = platform->kernels.getKernel(section + kernelName);

    kernelName = "UrstCubature" + suffix;
    nrs->UrstCubatureKernel = platform->kernels.getKernel(section + kernelName);

    kernelName = "Urst" + suffix;
    nrs->UrstKernel = platform->kernels.getKernel(section + kernelName);

    if (nrs->Nsubsteps) {
      kernelName = "subCycleStrongCubatureVolume" + suffix;
      nrs->subCycleStrongCubatureVolumeKernel = platform->kernels.getKernel(section + kernelName);
      kernelName = "subCycleStrongVolume" + suffix;
      nrs->subCycleStrongVolumeKernel = platform->kernels.getKernel(section + kernelName);

      kernelName = "subCycleERKUpdate";
      nrs->subCycleRKUpdateKernel = platform->kernels.getKernel(section + kernelName);
      kernelName = "subCycleRK";
      nrs->subCycleRKKernel = platform->kernels.getKernel(section + kernelName);
    ...
    nrs->extrapolateKernel = platform->kernels.getKernel(section + kernelName);

    kernelName = "maskCopy";
    nrs->maskCopyKernel = platform->kernels.getKernel(section + kernelName);
    kernelName = "mask";
    nrs->maskKernel = platform->kernels.getKernel(section + kernelName);

    kernelName = "filterRT" + suffix;
    nrs->filterRTKernel = platform->kernels.getKernel(section + kernelName);

    kernelName = "cfl" + suffix;
    nrs->cflKernel = platform->kernels.getKernel(section + kernelName);

    kernelName = "pressureAddQtl";
    nrs->pressureAddQtlKernel = platform->kernels.getKernel(section + kernelName);

    kernelName = "setEllipticCoeff";
    nrs->setEllipticCoeffKernel = platform->kernels.getKernel(section + kernelName);
    kernelName = "setEllipticCoeffPressure";
    nrs->setEllipticCoeffPressureKernel = platform->kernels.getKernel(section + kernelName);
  }
  ...
  if (nrs->Nscalar) {
    ...
    for (int is = 0; is < cds->NSfields; is++) {
      ...
      for (int i = 0; i < 2 * nrs->fieldOffset; i++)
        nrs->ellipticCoeff[i] = 1;
...
cds_t *cdsSetup(nrs_t *nrs, setupAide options)
{
  ...
  {
    kernelName = "strongAdvectionVolume" + suffix;
    cds->advectionStrongVolumeKernel = platform->kernels.getKernel(section + kernelName);

    kernelName = "strongAdvectionCubatureVolume" + suffix;
    cds->advectionStrongCubatureVolumeKernel = platform->kernels.getKernel(section + kernelName);

    kernelName = "advectMeshVelocityHex3D";
    cds->advectMeshVelocityKernel = platform->kernels.getKernel(section + kernelName);

    kernelName = "maskCopy";
    cds->maskCopyKernel = platform->kernels.getKernel(section + kernelName);
    ...
    kernelName = "setEllipticCoeff";
    cds->setEllipticCoeffKernel = platform->kernels.getKernel(section + kernelName);

    kernelName = "filterRT" + suffix;
    cds->filterRTKernel = platform->kernels.getKernel(section + kernelName);

    kernelName = "nStagesSum3";
    cds->nStagesSum3Kernel = platform->kernels.getKernel(section + kernelName);

    if (cds->Nsubsteps) {
      kernelName = "subCycleStrongCubatureVolume" + suffix;
      cds->subCycleStrongCubatureVolumeKernel = platform->kernels.getKernel(section + kernelName);
      kernelName = "subCycleStrongVolume" + suffix;
      cds->subCycleStrongVolumeKernel = platform->kernels.getKernel(section + kernelName);
