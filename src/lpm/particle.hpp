...
template <class Extra = char> struct particle_set {
  ...
  template <typename fieldPtr> void interp(fieldPtr field, dfloat *out[], dlong nFields)
  {
  ...
  //   out            ... array of pointers to the output arrays (dfloat[n][3])
  //   nfld           ... number of fields
  template <class fieldPtr> void interpLocal(fieldPtr field, dfloat *out[], dlong nFields)
  {
    dlong pn = size();
    dlong offset = 0;
