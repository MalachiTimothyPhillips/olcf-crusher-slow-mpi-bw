...
void lowMach::qThermalIdealGasSingleComponent(dfloat time, occa::memory o_div)
{
  ...
  platform->linAlg->fill(mesh->Nelements * mesh->Np,
                         0.0,
  ...
  if (nrs->pSolver->allNeumann) {
    ...
    const dfloat termQ =
        linAlg->sum(Nlocal, platform->o_mempool.slice0, platform->comm.mpiComm);
