...
SEMFEMData *ellipticBuildSEMFEM(const int N_,
                                const int n_elem_,
                                occa::memory _o_x,
                                occa::memory _o_y,
                                occa::memory _o_z,
                                ...
                                long long int *gatherGlobalNodes)
{
...

  {
    ...
    HYPRE_IJMatrixGetRowCounts(A_bc, numRows, ownedRows, ncols);
    ...
    HYPRE_IJMatrixGetValues(A_bc, -numRows, ncols, ownedRows, &hAj[0], &hAv[0]);
