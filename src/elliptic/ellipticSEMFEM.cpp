...
void ellipticSEMFEMSetup(elliptic_t *elliptic)
{
  ...
  if (platform->comm.mpiRank == 0)
    printf("setup SEMFEM preconditioner ... \n");
  fflush(stdout);
  ...
  SEMFEMData *data = ellipticBuildSEMFEM(mesh->Nq,
                                         mesh->Nelements,
                                         mesh->o_x,
                                         mesh->o_y,
                                         mesh->o_z,
  ...
  if (elliptic->options.compareArgs("SEMFEM SOLVER", "BOOMERAMG")) {
    ...
    if (useFP32) {
      if (platform->comm.mpiRank == 0)
        printf("HYPRE does not support FP32!\n");
      MPI_Barrier(platform->comm.mpiComm);
      ABORT(1);
    }
    ...
    if (platform->device.mode() != "Serial" && useDevice) {
      if (platform->comm.mpiRank == 0)
        printf("HYPRE has not been built with GPU support!\n");
      MPI_Barrier(platform->comm.mpiComm);
      ABORT(1);
    }
                         ...
                         occa::memory &o_z)
{
  ...
  gatherKernel(numRowsSEMFEM, o_dofMap, o_r, o_buffer);
