...
int SUNMemoryHelper_Alloc_Hip(SUNMemoryHelper helper,
                              SUNMemory *memptr,
                              size_t mem_size,
...
int SUNMemoryHelper_Dealloc_Hip(SUNMemoryHelper helper, SUNMemory mem, void *queue)
{
  ...
  if (mem->ptr != NULL && mem->own) {
    ...
    else if (mem->type == SUNMEMTYPE_DEVICE || mem->type == SUNMEMTYPE_UVM) {
...
int SUNMemoryHelper_Copy_Hip(SUNMemoryHelper helper,
                             SUNMemory dst,
                             SUNMemory src,
                             size_t memory_size,
                             void *queue)
{
  ...
  case SUNMEMTYPE_DEVICE:
    if (dst->type == SUNMEMTYPE_HOST || dst->type == SUNMEMTYPE_PINNED) {
      ...
      cuerr = hipMemcpy(dst->ptr, src->ptr, memory_size, hipMemcpyDeviceToDevice);
    }
    if (!SUNDIALS_HIP_VERIFY(cuerr))
      retval = -1;
    break;
...
int SUNMemoryHelper_CopyAsync_Hip(SUNMemoryHelper helper,
                                  SUNMemory dst,
                                  SUNMemory src,
                                  size_t memory_size,
                                  void *queue)
{
  ...
  case SUNMEMTYPE_PINNED:
    if (dst->type == SUNMEMTYPE_HOST || dst->type == SUNMEMTYPE_PINNED) {
      ...
      cuerr = hipMemcpyAsync(dst->ptr, src->ptr, memory_size, hipMemcpyHostToDevice, stream);
    }
    if (!SUNDIALS_HIP_VERIFY(cuerr))
      retval = -1;
    break;
  ...
  case SUNMEMTYPE_DEVICE:
    if (dst->type == SUNMEMTYPE_HOST || dst->type == SUNMEMTYPE_PINNED) {
      cuerr = hipMemcpyAsync(dst->ptr, src->ptr, memory_size, hipMemcpyDeviceToHost, stream);
    }
    else if (dst->type == SUNMEMTYPE_DEVICE || dst->type == SUNMEMTYPE_UVM) {
      cuerr = hipMemcpyAsync(dst->ptr, src->ptr, memory_size, hipMemcpyDeviceToDevice, stream);
    }
    if (!SUNDIALS_HIP_VERIFY(cuerr))
      retval = -1;
    break;
