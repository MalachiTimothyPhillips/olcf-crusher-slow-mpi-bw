...
SUNMatrix SUNMatrix_SLUNRloc(SuperMatrix *A_super, gridinfo_t *grid)
{
  ...
  if (A_super->Stype != SLU_NR_loc || A_super->Dtype != SLU_D || A_super->Mtype != SLU_GE)
    return (NULL);
  ...
  /* Create content */
  content = NULL;
  content = (SUNMatrixContent_SLUNRloc)malloc(sizeof(struct _SUNMatrixContent_SLUNRloc));
  if (content == NULL) {
    free(ops);
    free(A);
    return (NULL);
...
SuperMatrix *SUNMatrix_SLUNRloc_SuperMatrix(SUNMatrix A) { return (SM_SUPERMATRIX_SLUNRLOC(A)); }

gridinfo_t *SUNMatrix_SLUNRloc_ProcessGrid(SUNMatrix A) { return (SM_GRID_SLUNRLOC(A)); }

booleantype SUNMatrix_SLUNRloc_OwnData(SUNMatrix A) { return (SM_OWNDATA_SLUNRLOC(A)); }
...
int SUNMatCopy_SLUNRloc(SUNMatrix A, SUNMatrix B)
{
  ...
  dCopy_CompRowLoc_Matrix_dist(SM_SUPERMATRIX_SLUNRLOC(A), SM_SUPERMATRIX_SLUNRLOC(B));
  return (SUNMAT_SUCCESS);
}
...
int SUNMatScaleAdd_SLUNRloc(realtype c, SUNMatrix A, SUNMatrix B)
{
  ...
  if (!SMCompatible_SLUNRloc(A, B))
    return (SUNMAT_ILL_INPUT);

  /* call SuperLU-DIST ScaleAdd function */
  dScaleAdd_CompRowLoc_Matrix_dist(SM_SUPERMATRIX_SLUNRLOC(A), SM_SUPERMATRIX_SLUNRLOC(B), c);
  return (SUNMAT_SUCCESS);
}
...
int SUNMatMatvec_SLUNRloc(SUNMatrix A, N_Vector x, N_Vector y)
{
  ...
  if (ACS == NULL || SM_ROWTOPROC_SLUNRLOC(A) == NULL || SM_COMMPATTERN_SLUNRLOC(A) == NULL)
...
int SUNMatMatvecSetup_SLUNRloc(SUNMatrix A)
{
  ...
  if (ACS == NULL) {
    ...
    SM_ROWTOPROC_SLUNRLOC(A) = (sunindextype *)malloc(SM_GLOBALROWS_SLUNRLOC(A) * sizeof(sunindextype));
  ...
  MPI_Allgather(&SM_FSTROW_SLUNRLOC(A),
                1,
                MPI_SUNINDEXTYPE,
                temp,
                1,
                MPI_SUNINDEXTYPE,
                SM_GRID_SLUNRLOC(A)->comm);
  ...
  pdgsmv_init(ACS, SM_ROWTOPROC_SLUNRLOC(A), SM_GRID_SLUNRLOC(A), SM_COMMPATTERN_SLUNRLOC(A));
...
static booleantype SMCompatible_SLUNRloc(SUNMatrix A, SUNMatrix B)
{
  ...
  if ((SUNMatGetID(A) != SUNMATRIX_SLUNRLOC) || (SUNMatGetID(B) != SUNMATRIX_SLUNRLOC))
    return SUNFALSE;
