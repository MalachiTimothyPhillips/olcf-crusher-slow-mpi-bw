...
void SUNSparseMatrix_Print(SUNMatrix A, FILE *outfile)
{
  ...
  fprintf(outfile, "\n");
  fprintf(outfile,
          "%ld by %ld %s matrix, NNZ: %ld \n",
          (long int)SM_ROWS_S(A),
          (long int)SM_COLUMNS_S(A),
          matrixtype,
          (long int)SM_NNZ_S(A));
  for (j = 0; j < SM_NP_S(A); j++) {
    fprintf(outfile,
            "%s %ld : locations %ld to %ld\n",
            indexname,
            (long int)j,
    ...
    for (i = (SM_INDEXPTRS_S(A))[j]; i < (SM_INDEXPTRS_S(A))[j + 1]; i++) {
      ...
      fprintf(outfile, "%ld: %.32Lg   ", (long int)(SM_INDEXVALS_S(A))[i], (SM_DATA_S(A))[i]);
#elif defined(SUNDIALS_DOUBLE_PRECISION)
      fprintf(outfile, "%ld: %.16g   ", (long int)(SM_INDEXVALS_S(A))[i], (SM_DATA_S(A))[i]);
#else
      fprintf(outfile, "%ld: %.8g   ", (long int)(SM_INDEXVALS_S(A))[i], (SM_DATA_S(A))[i]);
#endif
    }
    fprintf(outfile, "\n");
  }
...
{
  SUNMatrix B = SUNSparseMatrix(SM_ROWS_S(A), SM_COLUMNS_S(A), SM_NNZ_S(A), SM_SPARSETYPE_S(A), A->sunctx);
  return (B);
}
...
int SUNMatScaleAddI_Sparse(realtype c, SUNMatrix A)
{
  ...
  if (newvals == 0) {
    ...
    C = SUNSparseMatrix(SM_ROWS_S(A), SM_COLUMNS_S(A), Ap[N] + newvals, SM_SPARSETYPE_S(A), A->sunctx);
...
static booleantype SMCompatible_Sparse(SUNMatrix A, SUNMatrix B)
{
  ...
  if ((SUNMatGetID(A) != SUNMATRIX_SPARSE) || (SUNMatGetID(B) != SUNMATRIX_SPARSE))
    return SUNFALSE;
...
static booleantype SMCompatible2_Sparse(SUNMatrix A, N_Vector x, N_Vector y)
{
  ...
  if ((SUNSparseMatrix_Columns(A) != N_VGetLength(x)) || (SUNSparseMatrix_Rows(A) != N_VGetLength(y)))
    return SUNFALSE;
