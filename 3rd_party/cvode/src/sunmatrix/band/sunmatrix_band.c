...
SUNMatrix
SUNBandMatrixStorage(sunindextype N, sunindextype mu, sunindextype ml, sunindextype smu, SUNContext sunctx)
{
  SUNMatrix A;
...
{
  SUNMatrix B =
      SUNBandMatrixStorage(SM_COLUMNS_B(A), SM_UBAND_B(A), SM_LBAND_B(A), SM_SUBAND_B(A), A->sunctx);
  return (B);
}
...
int SUNMatCopy_Band(SUNMatrix A, SUNMatrix B)
{
  ...
  if ((SM_UBAND_B(A) > SM_UBAND_B(B)) || (SM_LBAND_B(A) > SM_LBAND_B(B))) {
    ...
    SM_CONTENT_B(B)->data =
        (realtype *)realloc(SM_CONTENT_B(B)->data, SM_COLUMNS_B(B) * colSize * sizeof(realtype));
...
static booleantype SMCompatible2_Band(SUNMatrix A, N_Vector x, N_Vector y)
{
  ...
  if ((N_VGetVectorID(x) != SUNDIALS_NVEC_SERIAL) && (N_VGetVectorID(x) != SUNDIALS_NVEC_OPENMP) &&
