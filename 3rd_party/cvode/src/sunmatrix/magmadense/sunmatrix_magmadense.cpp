...
SUNMatrix SUNMatrix_MagmaDense(sunindextype M,
                               sunindextype N,
...
SUNMatrix SUNMatrix_MagmaDenseBlock(sunindextype nblocks,
                                    sunindextype M,
                                    sunindextype N,
                                    SUNMemoryType memtype,
                                    SUNMemoryHelper memhelper,
                                    ...
                                    SUNContext sunctx)
{
  ...
  SUNDIALS_HIP_OR_CUDA(magma_queue_create_from_hip(A->device_id, (hipStream_t)queue, NULL, NULL, &A->q);
                       , magma_queue_create_from_cuda(A->device_id, (cudaStream_t)queue, NULL, NULL, &A->q);)

  /* Allocate data */
  retval = SUNMemoryHelper_Alloc(A->memhelp, &A->data, sizeof(realtype) * A->ldata, memtype, nullptr);
  if (retval) {
    SUNMatDestroy(Amat);
    return (NULL);
  }
  ...
  if (A->nblocks > 1) {
    ...
    retval = SUNMemoryHelper_Alloc(A->memhelp, &A->blocks, sizeof(realtype *) * A->nblocks, memtype, nullptr);
    if (retval) {
      SUNMatDestroy(Amat);
      return (NULL);
    }
...
int SUNMatrix_MagmaDense_CopyToDevice(SUNMatrix Amat, realtype *h_data)
{
  ...
  retval =
      SUNMemoryHelper_CopyAsync(A->memhelp, A->data, _h_data, sizeof(realtype) * A->ldata, (void *)&stream);
...
int SUNMatrix_MagmaDense_CopyFromDevice(SUNMatrix Amat, realtype *h_data)
{
  ...
  retval =
      SUNMemoryHelper_CopyAsync(A->memhelp, _h_data, A->data, sizeof(realtype) * A->ldata, (void *)&stream);
  magma_queue_sync(A->q); /* sync with respect to host, but only this stream */
...
SUNMatrix SUNMatClone_MagmaDense(SUNMatrix Amat)
{
  ...
  if (A->nblocks > 1)
    B = SUNMatrix_MagmaDenseBlock(A->nblocks, A->M, A->N, A->data->type, A->memhelp, stream, Amat->sunctx);
  else
    B = SUNMatrix_MagmaDense(A->M, A->N, A->data->type, A->memhelp, stream, Amat->sunctx);
...
void SUNMatDestroy_MagmaDense(SUNMatrix Amat)
{
  ...
  if (A) {
    /* Free data array(s) */
    if (A->data)
      SUNMemoryHelper_Dealloc(A->memhelp, A->data, nullptr);
    if (A->blocks)
      SUNMemoryHelper_Dealloc(A->memhelp, A->blocks, nullptr);
    if (A->xblocks)
...
int SUNMatZero_MagmaDense(SUNMatrix Amat)
{
  ...
  /* Zero out matrix */
  SUNDIALS_LAUNCH_KERNEL(
      SUNDIALS_KERNEL_NAME(zeroKernel<realtype, sunindextype>),
      dim3(std::min<sunindextype>(A->nblocks, INT_MAX), 1, 1),
      SUNDIALS_HIP_OR_CUDA(dim3(1, 16, 16), dim3(1, 16, 32)), /* We choose slightly larger thread blocks when
...
int SUNMatCopy_MagmaDense(SUNMatrix Amat, SUNMatrix Bmat)
{
  ...
  /* Copy A into B */
  SUNDIALS_LAUNCH_KERNEL(
      SUNDIALS_KERNEL_NAME(copyKernel<realtype, sunindextype>),
      dim3(std::min<sunindextype>(A->nblocks, INT_MAX), 1, 1),
...
int SUNMatScaleAddI_MagmaDense(realtype c, SUNMatrix Amat)
{
  ...
  SUNDIALS_LAUNCH_KERNEL(
      SUNDIALS_KERNEL_NAME(scaleAddIKernel<realtype, sunindextype>),
      dim3(std::min<sunindextype>(A->nblocks, INT_MAX), 1, 1),
      SUNDIALS_HIP_OR_CUDA(dim3(1, 16, 16), dim3(1, 16, 32)),
      0,
      SUNDIALS_HIP_OR_CUDA(magma_queue_get_hip_stream(A->q), magma_queue_get_cuda_stream(A->q)),
...
int SUNMatScaleAdd_MagmaDense(realtype c, SUNMatrix Amat, SUNMatrix Bmat)
{
  ...
  if ((SUNMatGetID(Amat) != SUNMATRIX_MAGMADENSE) || !SMCompatible_MagmaDense(Amat, Bmat))
    return (SUNMAT_ILL_INPUT);
  ...
  SUNDIALS_LAUNCH_KERNEL(
      SUNDIALS_KERNEL_NAME(scaleAddKernel<realtype, sunindextype>),
      dim3(std::min<sunindextype>(A->nblocks, INT_MAX), 1, 1),
      SUNDIALS_HIP_OR_CUDA(dim3(1, 16, 16), dim3(1, 16, 32)),
      0,
      SUNDIALS_HIP_OR_CUDA(magma_queue_get_hip_stream(A->q), magma_queue_get_cuda_stream(A->q)),
...
int SUNMatMatvecSetup_MagmaDense(SUNMatrix Amat)
{
  ...
  if (A->nblocks > 1) {
    ...
    if (A->xblocks == NULL)
      retval = SUNMemoryHelper_Alloc(A->memhelp,
                                     &A->xblocks,
...
int SUNMatMatvec_MagmaDense(SUNMatrix Amat, N_Vector x, N_Vector y)
{
  ...
  if ((SUNMatGetID(Amat) != SUNMATRIX_MAGMADENSE) || !SMCompatible2_MagmaDense(Amat, x, y))
    return (SUNMAT_ILL_INPUT);
  ...
  if (A->nblocks > 1) {
    ...
    SUNDIALS_LAUNCH_KERNEL(
        SUNDIALS_KERNEL_NAME(getBlockPointers<realtype, sunindextype>),
        A->nblocks,
        256,
        0,
        SUNDIALS_HIP_OR_CUDA(magma_queue_get_hip_stream(A->q), magma_queue_get_cuda_stream(A->q)),
...
static booleantype SMCompatible2_MagmaDense(SUNMatrix Amat, N_Vector x, N_Vector y)
{
  ...
  if (x->ops->nvgetdevicearraypointer == NULL || y->ops->nvgetdevicearraypointer == NULL)
    return (SUNFALSE);
