...
static int
GetKernelParameters(SUNMatrix A, booleantype reduction, size_t &nthreads_total, size_t &nthreads_per_block);

/* --------------------------------------------------------------------------
 * Constructors
 * -------------------------------------------------------------------------- */

SUNMatrix SUNMatrix_OneMklDense(sunindextype M,
                                sunindextype N,
                                     ...
                                     SUNContext sunctx)
{
  ...
  // Allocate data
  retval = SUNMemoryHelper_Alloc(MAT_MEMHELPER(A),
                                 &(MAT_DATA(A)),
...
extern realtype *SUNMatrix_OneMklDense_BlockColumn(SUNMatrix A, sunindextype k, sunindextype j);
...
int SUNMatMatvec_OneMklDense(SUNMatrix A, N_Vector x, N_Vector y)
{
  ...
  if (MAT_NBLOCKS(A) > 1) {
    ...
    for (sunindextype i = 0; i < MAT_NBLOCKS(A); i++) {
      ...
      // Copmute y = a * A * x + b * y
      oneapi::mkl::blas::gemv(*Q, oneapi::mkl::transpose::N, M, N, ONE, Adata, M, xdata, 1, ZERO, ydata, 1);
    }
  }
  else {
    sycl::queue *Q = MAT_QUEUE(A);
