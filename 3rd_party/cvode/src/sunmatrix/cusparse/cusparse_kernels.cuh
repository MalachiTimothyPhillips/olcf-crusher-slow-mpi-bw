...
__global__ void scaleAddIKernelCSR(I m, T c, T *A, const I *rowptr, const I *colind)
{
  ...
  // more than one row.
  for (I row = blockIdx.x * blockDim.x + threadIdx.x; row < m; row += blockDim.x * gridDim.x) {
    I tmp = rowptr[row];
    I rownnz = rowptr[row + 1] - tmp;
    I idx = tmp;
    for (I j = 0; j < rownnz; j++) {
      if (colind[idx + j] == row)
        A[idx + j] = c * A[idx + j] + 1.0;
      else
        A[idx + j] = c * A[idx + j];
    }
  }
...
template <typename T, typename I>
__global__ void scaleAddIKernelBCSR(I m, I nblocks, I blocknnz, T c, T *A, const I *rowptr, const I *colind)
{
  // REQUIRES THE DIAGONAL TO BE PRESENT!
  ...
  for (I block = blockIdx.x; block < nblocks; block += gridDim.x) {
    ...
    for (I row = threadIdx.x; row < m; row += blockDim.x) {
      ...
      for (I j = 0; j < rownnz; j++) {
        if (colind[idxl + j] == row)
          A[idxg + j] = c * A[idxg + j] + 1.0;
        else
          A[idxg + j] = c * A[idxg + j];
      }
    }
...
template <typename T, typename I> __global__ void scaleAddKernelCSR(I nnz, T c, T *A, const T *B)
{
  // REQUIRES A AND B TO HAVE THE SAME SPARSITY PATTERN
  for (I i = blockIdx.x * blockDim.x + threadIdx.x; i < nnz; i += blockDim.x * gridDim.x) {
    A[i] = c * A[i] + B[i];
  }
}
...
matvecBCSR(I m, I nblocks, I blocknnz, const T *A, const I *rowptr, const I *colind, const T *x, T *y)
{
  ...
  for (I i = blockIdx.x * blockDim.x + threadIdx.x; i < nblocks * blocknnz; i += blockDim.x * gridDim.x) {
    y[i] = 0.0;
  }
...
template <typename T, typename I>
__global__ void print_kernel(I m, I nnz, I blocknnz, T *A, const I *rowptr, const I *colind)
{
  for (I i = blockIdx.x * blockDim.x + threadIdx.x; i < nnz; i += blockDim.x * gridDim.x) {
    printf("A[%d] = %f\n", i, A[i]);
  }
  for (I i = blockIdx.x * blockDim.x + threadIdx.x; i < m + 1; i += blockDim.x * gridDim.x) {
    printf("rowptr[%d] = %d\n", i, rowptr[i]);
  }
  for (I i = blockIdx.x * blockDim.x + threadIdx.x; i < blocknnz; i += blockDim.x * gridDim.x) {
    printf("colind[%d] = %d\n", i, colind[i]);
  }
}
