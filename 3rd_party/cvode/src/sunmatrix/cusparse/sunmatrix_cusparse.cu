...
public:
  SUNCuSparseMatrixExecPolicy(const cudaStream_t stream = 0) : ExecPolicy(stream) {}

  SUNCuSparseMatrixExecPolicy(const SUNCuSparseMatrixExecPolicy &ex) : ExecPolicy(ex.stream_) {}
  ...
  static size_t max_block_size(int val) { return ((val > MAX_BLOCK_SIZE) ? MAX_BLOCK_SIZE : val); }
};
...
SUNMatrix SUNMatrix_cuSparse_NewCSR(int M, int N, int NNZ, cusparseHandle_t cusp, SUNContext sunctx)
{
  ...
  alloc_fail +=
      SUNMemoryHelper_Alloc(SMCU_MEMHELP(A), &d_colind, sizeof(int) * NNZ, SUNMEMTYPE_DEVICE, nullptr);
  alloc_fail +=
      SUNMemoryHelper_Alloc(SMCU_MEMHELP(A), &d_rowptr, sizeof(int) * (M + 1), SUNMEMTYPE_DEVICE, nullptr);
  alloc_fail +=
      SUNMemoryHelper_Alloc(SMCU_MEMHELP(A), &d_values, sizeof(realtype) * NNZ, SUNMEMTYPE_DEVICE, nullptr);
  if (alloc_fail) {
...
SUNMatrix SUNMatrix_cuSparse_MakeCSR(cusparseMatDescr_t mat_descr,
                                     int M,
                                     int N,
                                     int NNZ,
                                     int *rowptrs,
                                     ...
                                     SUNContext sunctx)
{
  ...
  if (cusparseGetMatIndexBase(mat_descr) != CUSPARSE_INDEX_BASE_ZERO) {
    SUNDIALS_DEBUG_PRINT("ERROR in SUNMatrix_MakeCSR_cuSparse: the cusparseMatDescr_t must have index base "
  ...
  if (SMCU_CONTENT(A)->colind == NULL || SMCU_CONTENT(A)->rowptrs == NULL || SMCU_CONTENT(A)->data == NULL) {
    SUNMatDestroy(A);
    return (NULL);
  }
...
SUNMatrix SUNMatrix_cuSparse_NewBlockCSR(int nblocks,
                                         int blockrows,
                                         int blockcols,
                                         int blocknnz,
                                         cusparseHandle_t cusp,
                                         SUNContext sunctx)
{
  ...
  if (blockrows != blockcols) {
    SUNDIALS_DEBUG_PRINT(
        "ERROR in SUNMatrix_cuSparse_NewBlockCSR: matrix must be square for the BCSR format\n");
    return (NULL);
  }
  ...
  if (A == NULL) {
    SUNDIALS_DEBUG_PRINT(
        "ERROR in SUNMatrix_cuSparse_NewBlockCSR: SUNMatrix_cuSparse_NewEmpty returned NULL\n");
    return (NULL);
  }
  ...
  /* Allocate device memory for the matrix */
  alloc_fail +=
      SUNMemoryHelper_Alloc(SMCU_MEMHELP(A), &d_colind, sizeof(int) * blocknnz, SUNMEMTYPE_DEVICE, nullptr);
  alloc_fail += SUNMemoryHelper_Alloc(SMCU_MEMHELP(A),
                                      &d_rowptr,
...
int SUNMatrix_cuSparse_CopyToDevice(SUNMatrix dA, realtype *h_data, int *h_idxptrs, int *h_idxvals)
{
  int retval;
...
int SUNMatrix_cuSparse_CopyFromDevice(SUNMatrix dA, realtype *h_data, int *h_idxptrs, int *h_idxvals)
{
  int retval;
  SUNMemory _h_data, _h_idxptrs, _h_idxvals;
  const cudaStream_t *stream;
...
SUNMatrix SUNMatClone_cuSparse(SUNMatrix A)
{
  ...
  case SUNMAT_CUSPARSE_CSR:
    B = SUNMatrix_cuSparse_NewCSR(SMCU_ROWS(A), SMCU_COLUMNS(A), SMCU_NNZ(A), SMCU_CUSPHANDLE(A), A->sunctx);
    break;
  case SUNMAT_CUSPARSE_BCSR:
    B = SUNMatrix_cuSparse_NewBlockCSR(SMCU_NBLOCKS(A),
                                       SMCU_BLOCKROWS(A),
                                       SMCU_BLOCKCOLS(A),
                                       SMCU_BLOCKNNZ(A),
                                       SMCU_CUSPHANDLE(A),
...
void SUNMatDestroy_cuSparse(SUNMatrix A)
{
  ...
  if (A->content != NULL) {
    if (SMCU_MEMHELP(A)) {
      ...
      SUNDIALS_DEBUG_PRINT("WARNING in SUNMatDestroy_cuSparse: mem_helper was NULL when trying to dealloc "
    ...
    SUNMemoryHelper_Dealloc(SMCU_MEMHELP(A), SMCU_CONTENT(A)->dBufferMem, nullptr);
#endif
...
int SUNMatZero_cuSparse(SUNMatrix A)
{
  ...
  if (!SMCU_FIXEDPATTERN(A)) {
    cuerr = cudaMemsetAsync(SMCU_INDEXPTRSp(A), 0, (SMCU_BLOCKROWS(A) + 1) * sizeof(int), stream);
    if (!SUNDIALS_CUDA_VERIFY(cuerr))
      return (SUNMAT_OPERATION_FAIL);

    /* set all colind to zero */
    cuerr = cudaMemsetAsync(SMCU_INDEXVALSp(A), 0, SMCU_BLOCKNNZ(A) * sizeof(int), stream);
    if (!SUNDIALS_CUDA_VERIFY(cuerr))
      return (SUNMAT_OPERATION_FAIL);
  }
...
int SUNMatCopy_cuSparse(SUNMatrix src, SUNMatrix dst)
{
  ...
  if (SMCU_NNZ(dst) < SMCU_NNZ(src)) {
    SUNDIALS_DEBUG_PRINT(
        "ERROR in SUNMatCopy_cuSparse: the destination matrix has less nonzeros than the source\n");
    return (SUNMAT_ILL_INPUT);
  }
...
int SUNMatScaleAddI_cuSparse(realtype c, SUNMatrix A)
{
  ...
  case SUNMAT_CUSPARSE_CSR:
    ...
    scaleAddIKernelCSR<realtype, int><<<gridSize, threadsPerBlock, 0, stream>>>(SMCU_ROWS(A),
                                                                                c,
                                                                                SMCU_DATAp(A),
                                                                                SMCU_INDEXPTRSp(A),
                                                                                SMCU_INDEXVALSp(A));
  ...
  case SUNMAT_CUSPARSE_BCSR:
    ...
    scaleAddIKernelBCSR<realtype, int><<<gridSize, threadsPerBlock, 0, stream>>>(SMCU_BLOCKROWS(A),
                                                                                 SMCU_NBLOCKS(A),
                                                                                 SMCU_BLOCKNNZ(A),
                                                                                 c,
                                                                                 SMCU_DATAp(A),
  ...
  if (!SUNDIALS_CUDA_VERIFY(cudaGetLastError()))
    return (SUNMAT_OPERATION_FAIL);
#endif
...
int SUNMatScaleAdd_cuSparse(realtype c, SUNMatrix A, SUNMatrix B)
{
  ...
  case SUNMAT_CUSPARSE_CSR:
    ...
    scaleAddKernelCSR<realtype, int>
        <<<gridSize, threadsPerBlock, 0, stream>>>(SMCU_NNZ(A), c, SMCU_DATAp(A), SMCU_DATAp(B));
    break;
  case SUNMAT_CUSPARSE_BCSR:
    /* Choose the grid size to be the number of blocks in the matrix,
       and then choose threadsPerBlock to be a multiple of the warp size
...
int SUNMatMatvecSetup_cuSparse(SUNMatrix A)
{
  ...
  if (!(SMCU_CONTENT(A)->matvec_issetup)) {
    SUNDIALS_CUSPARSE_VERIFY(
        cusparseCreateDnVec(&SMCU_CONTENT(A)->vecX, SMCU_COLUMNS(A), placeholder, CUDA_R_XF));
    SUNDIALS_CUSPARSE_VERIFY(
        cusparseCreateDnVec(&SMCU_CONTENT(A)->vecY, SMCU_ROWS(A), placeholder, CUDA_R_XF));

    SUNDIALS_CUSPARSE_VERIFY(cusparseSpMV_bufferSize(SMCU_CUSPHANDLE(A),
                                                     CUSPARSE_OPERATION_NON_TRANSPOSE,
                                                     &one,
                                                     SMCU_CONTENT(A)->spmat_descr,
                                                     SMCU_CONTENT(A)->vecX,
                                                     &one,
                                                     SMCU_CONTENT(A)->vecY,
                                                     CUDA_R_XF,
    ...
    if (SUNMemoryHelper_Alloc(SMCU_MEMHELP(A),
                              &SMCU_CONTENT(A)->dBufferMem,
                              SMCU_CONTENT(A)->bufferSize,
                              SUNMEMTYPE_DEVICE,
                              nullptr))
      return (SUNMAT_OPERATION_FAIL);
...
int SUNMatMatvec_cuSparse(SUNMatrix A, N_Vector x, N_Vector y)
{
  ...
  if (SMCU_SPARSETYPE(A) == SUNMAT_CUSPARSE_CSR) {
...
#if CUDART_VERSION >= 11000
    {
      ...
      SUNDIALS_CUSPARSE_VERIFY(cusparseDnVecSetValues(SMCU_CONTENT(A)->vecX, d_xdata));
      SUNDIALS_CUSPARSE_VERIFY(cusparseDnVecSetValues(SMCU_CONTENT(A)->vecY, d_ydata));
...
#else
    SUNDIALS_CUSPARSE_VERIFY(cusparseXcsrmv(SMCU_CUSPHANDLE(A),
                                            CUSPARSE_OPERATION_NON_TRANSPOSE,
                                            SMCU_ROWS(A),
                                            SMCU_COLUMNS(A),
                                            SMCU_NNZ(A),
    ...
    threadsPerBlock = SMCU_EXECPOLICY(A)->blockSize(SMCU_COLUMNS(A) / 2);
    gridSize = SMCU_EXECPOLICY(A)->gridSize(SMCU_ROWS(A) * SMCU_COLUMNS(A) / 2, threadsPerBlock);
    matvecBCSR<realtype, int><<<gridSize, threadsPerBlock, 0, stream>>>(SMCU_BLOCKROWS(A),
                                                                        SMCU_NBLOCKS(A),
                                                                        SMCU_BLOCKNNZ(A),
                                                                        SMCU_DATAp(A),
                                                                        SMCU_INDEXPTRSp(A),
...
static booleantype SMCompatible_cuSparse(SUNMatrix A, SUNMatrix B)
{
  ...
  if ((SUNMatGetID(A) != SUNMATRIX_CUSPARSE) || (SUNMatGetID(B) != SUNMATRIX_CUSPARSE))
    return (SUNFALSE);
...
cusparseStatus_t CreateSpMatDescr(SUNMatrix A, cusparseSpMatDescr_t *spmat_descr)
{
  ...
  return (cusparseCreateCsr(spmat_descr,
                            SMCU_ROWS(A),
                            SMCU_COLUMNS(A),
                            SMCU_NNZ(A),
                            SMCU_INDEXPTRSp(A),
