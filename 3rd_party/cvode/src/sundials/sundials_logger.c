...
static void sunCreateLogMessage(SUNLogLevel lvl,
                                int rank,
                                const char *scope,
                                const char *label,
                                const char *txt,
...
int SUNLogger_SetErrorFilename(SUNLogger logger, const char *error_filename)
{
  ...
  if (error_filename && strcmp(error_filename, "")) {
    ...
    if (!SUNHashMap_GetValue(logger->filenames, error_filename, (void *)&fp)) {
      ...
      if (logger->error_fp) {
        SUNHashMap_Insert(logger->filenames, error_filename, (void *)logger->error_fp);
      }
      else {
        return -1;
      }
  ...
  if (warning_filename && strcmp(warning_filename, "")) {
    ...
    if (!SUNHashMap_GetValue(logger->filenames, warning_filename, (void *)&fp)) {
      ...
      if (logger->warning_fp) {
        SUNHashMap_Insert(logger->filenames, warning_filename, (void *)logger->warning_fp);
      }
      else {
        return -1;
      }
...
int SUNLogger_SetDebugFilename(SUNLogger logger, const char *debug_filename)
{
  ...
  if (debug_filename && strcmp(debug_filename, "")) {
    ...
    if (!SUNHashMap_GetValue(logger->filenames, debug_filename, (void *)&fp)) {
      ...
      if (logger->debug_fp) {
        SUNHashMap_Insert(logger->filenames, debug_filename, (void *)logger->debug_fp);
      }
      else {
        return -1;
      }
...
int SUNLogger_QueueMsg(SUNLogger logger,
                       SUNLogLevel lvl,
                       const char *scope,
                       const char *label,
                       const char *msg_txt,
