...
#define GRID_STRIDE_XLOOP(type, iter, max)                                                                   \
  for (type iter = blockDim.x * blockIdx.x + threadIdx.x; iter < max; iter += blockDim.x * gridDim.x)
...
template <typename T> __forceinline__ __device__ T shfl_xor_sync(T var, int laneMask);

template <typename T> __forceinline__ __device__ T shfl_sync(T var, int srcLane);

template <typename T> __forceinline__ __device__ T shfl_down_sync(T var, int srcLane);

template <> __forceinline__ __device__ int shfl_xor_sync<int>(int var, int laneMask)
{
  return ::__shfl_xor(var, laneMask);
}

template <> __forceinline__ __device__ float shfl_xor_sync<float>(float var, int laneMask)
{
  return ::__shfl_xor(var, laneMask);
}

template <> __forceinline__ __device__ double shfl_xor_sync<double>(double var, int laneMask)
{
  return ::__shfl_xor(var, laneMask);
}

template <> __forceinline__ __device__ int shfl_sync<int>(int var, int srcLane)
{
  return ::__shfl(var, srcLane);
}

template <> __forceinline__ __device__ float shfl_sync<float>(float var, int srcLane)
{
  return ::__shfl(var, srcLane);
}

template <> __forceinline__ __device__ double shfl_sync<double>(double var, int srcLane)
{
  return ::__shfl(var, srcLane);
}

template <> __forceinline__ __device__ float shfl_down_sync(float val, int srcLane)
{
  return ::__shfl_down(val, srcLane);
}

template <> __forceinline__ __device__ double shfl_down_sync(double val, int srcLane)
{
  return ::__shfl_down(val, srcLane);
}
 ...
 */
__forceinline__ __device__ void atomicMax(double *const address, const double value)
{
  if (*address >= value) {
    return;
  }
 ...
 */
__forceinline__ __device__ void atomicMin(double *const address, const double value)
{
  if (*address <= value) {
    return;
  }
...
template <typename BinaryReductionOp> struct atomic;

template <typename T> struct atomic<sundials::reductions::impl::plus<T>> {
  __device__ __forceinline__ void operator()(T *out, const T val) { atomicAdd(out, val); }
};

template <typename T> struct atomic<sundials::reductions::impl::maximum<T>> {
  __device__ __forceinline__ void operator()(T *out, const T val) { atomicMax(out, val); }
};

template <typename T> struct atomic<sundials::reductions::impl::minimum<T>> {
  __device__ __forceinline__ void operator()(T *out, const T val) { atomicMin(out, val); }
};

/*
 * Perform a reduce on the warp to get the operation result.
 */
template <typename T, typename BinaryReductionOp> __inline__ __device__ T warpReduceShflDown(T val)
{
 ...
 */
template <typename T, typename op> __inline__ __device__ T blockReduceShflDown(T val, T identity)
{
 ...
 */
template <typename T, typename BinaryReductionOp> __inline__ __device__ T blockReduceShfl(T val, T identity)
{
  int numThreads = blockDim.x * blockDim.y * blockDim.z;
...
__device__ __forceinline__ void gridReduce(T val, T identity, T *device_mem, unsigned int *device_count)
{
...
template <typename T, typename BinaryReductionOp> struct GridReducerLDS {
  __device__ __forceinline__ void operator()(T val, T identity, T *device_mem, unsigned int *device_count)
  {
    return impl::gridReduce<T, BinaryReductionOp>(val, identity, device_mem, device_count);
  }
};

template <typename T, typename BinaryReductionOp> struct GridReducerAtomic {
  __device__ __forceinline__ void operator()(T val, T identity, T *device_mem, unsigned int *device_count)
  {
