...
template <typename Arg1, typename Arg2, typename Result> struct BinaryOperator {
...
struct plus : public BinaryOperator<Arg1, Arg2, Ret> {
  SUNDIALS_HOST_DEVICE constexpr Ret operator()(const Arg1 &lhs, const Arg2 &rhs) const
  {
    return Ret{lhs} + rhs;
  }

  static SUNDIALS_HOST_DEVICE SUNDIALS_DEVICE_INLINE constexpr Ret identity() { return Ret{0}; }
};
