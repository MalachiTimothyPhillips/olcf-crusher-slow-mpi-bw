...
void BandScale(realtype c, SUNDlsMat A) { SUNDlsMat_bandScale(c, A->cols, A->M, A->mu, A->ml, A->s_mu); }
...
sunindextype
bandGBTRF(realtype **a, sunindextype n, sunindextype mu, sunindextype ml, sunindextype smu, sunindextype *p)
{
  return (SUNDlsMat_bandGBTRF(a, n, mu, ml, smu, p));
}

sunindextype SUNDlsMat_bandGBTRF(realtype **a,
                                 sunindextype n,
                                 sunindextype mu,
                                 sunindextype ml,
                                 sunindextype smu,
  ...
  for (k = 0; k < n - 1; k++, p++) {
    ...
    *p = l;
    ...
    for (j = k + 1; j <= last_col_k; j++) {
      ...
      if (a_kj != ZERO) {
        for (i = k + 1, kptr = sub_diag_k, jptr = col_j + ROW(k + 1, j, smu); i <= last_row_k;
             i++, kptr++, jptr++)
          (*jptr) += a_kj * (*kptr);
      }
    }
...
void SUNDlsMat_bandGBTRS(realtype **a,
                         sunindextype n,
                         sunindextype smu,
                         sunindextype ml,
                         sunindextype *p,
...
void SUNDlsMat_bandCopy(realtype **a,
                        realtype **b,
                        sunindextype n,
                        sunindextype a_smu,
                        sunindextype b_smu,
...
void SUNDlsMat_bandScale(realtype c,
                         realtype **a,
                         sunindextype n,
                         sunindextype mu,
                         sunindextype ml,
...
void bandAddIdentity(realtype **a, sunindextype n, sunindextype smu) { SUNDlsMat_bandAddIdentity(a, n, smu); }
...
void SUNDlsMat_bandMatvec(realtype **a,
                          realtype *x,
                          realtype *y,
                          sunindextype n,
                          sunindextype mu,
