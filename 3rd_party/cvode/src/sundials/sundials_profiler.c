  ...
  if (SUNHashMap_GetValue(p->map, name, (void **)&timer)) {
    ...
    if (ier) {
      ...
      snprintf(
          errmsg,
          128 + slen,
          "(((( [ERROR] in SUNProfilerBegin: SUNHashMapInsert failed with code %d while inserting %s))))\n",
          ier,
          name);
...
int SUNProfiler_Print(SUNProfiler p, FILE *fp)
{
  ...
  if (rank == 0) {
    ...
    for (i = 0; i < p->map->size; i++)
      if (sorted[i])
        sunPrintTimers(i, sorted[i], fp, (void *)p);
    free(sorted);
  }
...
int sunCollectTimers(SUNProfiler p)
{
  ...
  const MPI_Aint displ[2] = {offsetof(sunTimerStruct, tic), offsetof(sunTimerStruct, count)};
  MPI_Aint lb, extent;
  ...
  if (rank == 0) {
    MPI_Reduce(MPI_IN_PLACE,
               ...
               p->map->size,
               MPI_sunTimerStruct,
               MPI_sunTimerStruct_MAXANDSUM,
               0,
               comm);
  }
  else {
    MPI_Reduce(reduced, reduced, p->map->size, MPI_sunTimerStruct, MPI_sunTimerStruct_MAXANDSUM, 0, comm);
  }
