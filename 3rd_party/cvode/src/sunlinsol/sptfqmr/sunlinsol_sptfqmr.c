...
SUNLinearSolver SUNLinSol_SPTFQMR(N_Vector y, int pretype, int maxl, SUNContext sunctx)
{
  ...
  if ((pretype != SUN_PREC_NONE) && (pretype != SUN_PREC_LEFT) && (pretype != SUN_PREC_RIGHT) &&
  ...
  if ((y->ops->nvclone == NULL) || (y->ops->nvdestroy == NULL) || (y->ops->nvlinearsum == NULL) ||
      (y->ops->nvconst == NULL) || (y->ops->nvprod == NULL) || (y->ops->nvdiv == NULL) ||
...
SUNLinearSolver_Type SUNLinSolGetType_SPTFQMR(SUNLinearSolver S) { return (SUNLINEARSOLVER_ITERATIVE); }

SUNLinearSolver_ID SUNLinSolGetID_SPTFQMR(SUNLinearSolver S) { return (SUNLINEARSOLVER_SPTFQMR); }
...
int SUNLinSolInitialize_SPTFQMR(SUNLinearSolver S)
{
  ...
  if ((content->pretype != SUN_PREC_LEFT) && (content->pretype != SUN_PREC_RIGHT) &&
...
int SUNLinSolSetPreconditioner_SPTFQMR(SUNLinearSolver S, void *PData, SUNPSetupFn Psetup, SUNPSolveFn Psolve)
{
...
int SUNLinSolSolve_SPTFQMR(SUNLinearSolver S, SUNMatrix A, N_Vector x, N_Vector b, realtype delta)
{
  ...
  *nli = 0;
  ...
  preOnLeft =
      ((SPTFQMR_CONTENT(S)->pretype == SUN_PREC_LEFT) || (SPTFQMR_CONTENT(S)->pretype == SUN_PREC_BOTH));
  preOnRight =
      ((SPTFQMR_CONTENT(S)->pretype == SUN_PREC_RIGHT) || (SPTFQMR_CONTENT(S)->pretype == SUN_PREC_BOTH));
  scale_x = (sx != NULL);
  scale_b = (sb != NULL);
  ...
  *res_norm = r_init_norm = SUNRsqrt(rho[0]);
  ...
  if (SPTFQMR_CONTENT(S)->print_level && SPTFQMR_CONTENT(S)->info_file &&
      (SPTFQMR_CONTENT(S)->info_file != S->sunctx->logger->info_fp)) {
    fprintf(SPTFQMR_CONTENT(S)->info_file, SUNLS_MSG_RESIDUAL, (long int)0, *res_norm);
  }
  SUNLogger_QueueMsg(S->sunctx->logger,
                     SUN_LOGLEVEL_INFO,
                     "SUNLinSolSolve_SPFTQMR",
                     "initial-residual",
                     "nli = %li, resnorm = %.16g",
                     (long int)0,
                     *res_norm);
  ...
  for (n = 0; n < l_max; ++n) {
    ...
    for (m = 0; m < 2; ++m) {
      ...
      *res_norm = r_curr_norm = tau * SUNRsqrt(m + 1);
      ...
      if (SPTFQMR_CONTENT(S)->print_level && SPTFQMR_CONTENT(S)->info_file &&
          (SPTFQMR_CONTENT(S)->info_file != S->sunctx->logger->info_fp)) {
        fprintf(SPTFQMR_CONTENT(S)->info_file, SUNLS_MSG_RESIDUAL, (long int)*nli, *res_norm);
      }
      SUNLogger_QueueMsg(S->sunctx->logger,
                         SUN_LOGLEVEL_INFO,
      ...
      if ((r_curr_norm > delta) || (r_curr_norm >= r_init_norm && m == 1 && n == l_max)) {
        ...
        if (preOnRight) {
          ...
          if (ier != 0) {
            *zeroguess = SUNFALSE;
            LASTFLAG(S) = (ier < 0) ? SUNLS_PSOLVE_FAIL_UNREC : SUNLS_PSOLVE_FAIL_UNREC;
            return (LASTFLAG(S));
          }
...
int SUNLinSolSetInfoFile_SPTFQMR(SUNLinearSolver S, FILE *info_file)
{
...
int SUNLinSolSetPrintLevel_SPTFQMR(SUNLinearSolver S, int print_level)
{
  /* check that the linear solver is non-null */
  if (S == NULL)
    return (SUNLS_MEM_NULL);
