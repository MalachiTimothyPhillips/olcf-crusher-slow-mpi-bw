...
SUNLinearSolver SUNLinSol_OneMklDense(N_Vector y, SUNMatrix Amat, SUNContext sunctx)
{
  ...
  if (!(y->ops->nvgetlength) || !(y->ops->nvgetdevicearraypointer) || !(Amat->ops->getid)) {
  ...
  // Allocate data
  retval = SUNMemoryHelper_Alloc(LS_MEM_HELPER(S),
                                 &(LS_PIVOTS(S)),
  ...
  if (num_blocks > 1) {
    LS_F_SCRATCH_SIZE(S) = getrf_batch_scratchpad_size<realtype>(*queue,      // device queue
                                                                 M,           // rows in A_i
                                                                 N,           // columns in A_i
                                                                 M,           // leading dimension
    ...
    LS_S_SCRATCH_SIZE(S) = getrs_batch_scratchpad_size<realtype>(*queue, // device queue
                                                                 ...
                                                                 num_blocks); // number of blocks
#else
    LS_S_SCRATCH_SIZE(S) = getrs_scratchpad_size<realtype>(*queue, // device queue
                                                           oneapi::mkl::transpose::nontrans,
    ...
    LS_F_SCRATCH_SIZE(S) = getrf_scratchpad_size<realtype>(*queue, // device queue
                                                           M,      // rows in A_i
                                                           N,      // columns in A_i
                                                           M);     // leading dimension
...
int SUNLinSolSolve_OneMklDense(SUNLinearSolver S, SUNMatrix A, N_Vector x, N_Vector b, realtype tol)
{
  // Check for valid inputs
  if (!S) {
    SUNDIALS_DEBUG_ERROR("Linear solver is NULL\n");
...
int SUNLinSolSpace_OneMklDense(SUNLinearSolver S, long int *lenrwLS, long int *leniwLS)
{
  if (!S) {
    SUNDIALS_DEBUG_ERROR("Linear solver is NULL\n");
    return SUNLS_MEM_NULL;
