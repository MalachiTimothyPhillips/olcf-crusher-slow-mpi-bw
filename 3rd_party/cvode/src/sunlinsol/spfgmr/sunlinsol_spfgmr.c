...
SUNLinearSolver SUNLinSol_SPFGMR(N_Vector y, int pretype, int maxl, SUNContext sunctx)
{
  ...
  pretype = ((pretype == SUN_PREC_LEFT) || (pretype == SUN_PREC_RIGHT) || (pretype == SUN_PREC_BOTH))
                ? SUN_PREC_RIGHT
                : SUN_PREC_NONE;
...
SUNLinearSolver_Type SUNLinSolGetType_SPFGMR(SUNLinearSolver S) { return (SUNLINEARSOLVER_ITERATIVE); }

SUNLinearSolver_ID SUNLinSolGetID_SPFGMR(SUNLinearSolver S) { return (SUNLINEARSOLVER_SPFGMR); }
...
int SUNLinSolSetPreconditioner_SPFGMR(SUNLinearSolver S, void *PData, SUNPSetupFn Psetup, SUNPSolveFn Psolve)
{
...
int SUNLinSolSolve_SPFGMR(SUNLinearSolver S, SUNMatrix A, N_Vector x, N_Vector b, realtype delta)
{
  ...
  *nli = 0;
  ...
  preOnRight =
      ((SPFGMR_CONTENT(S)->pretype == SUN_PREC_LEFT) || (SPFGMR_CONTENT(S)->pretype == SUN_PREC_RIGHT) ||
  ...
  *res_norm = r_norm = beta = SUNRsqrt(N_VDotProd(V[0], V[0]));
  ...
  if (SPFGMR_CONTENT(S)->print_level && SPFGMR_CONTENT(S)->info_file &&
      (SPFGMR_CONTENT(S)->info_file != S->sunctx->logger->info_fp)) {
    fprintf(SPFGMR_CONTENT(S)->info_file, SUNLS_MSG_RESIDUAL, (long int)0, *res_norm);
  }
  ...
  for (ntries = 0; ntries <= max_restarts; ntries++) {
    ...
    for (l = 0; l < l_max; l++) {
      ...
      *res_norm = rho = SUNRabs(rotation_product * r_norm);
      ...
      if (SPFGMR_CONTENT(S)->print_level && SPFGMR_CONTENT(S)->info_file &&
          (SPFGMR_CONTENT(S)->info_file != S->sunctx->logger->info_fp)) {
        fprintf(SPFGMR_CONTENT(S)->info_file, SUNLS_MSG_RESIDUAL, (long int)*nli, *res_norm);
      }
      SUNLogger_QueueMsg(S->sunctx->logger,
                         SUN_LOGLEVEL_INFO,
                         "SUNLinSolSolve_SPFGMR",
...
int SUNLinSolSetInfoFile_SPFGMR(SUNLinearSolver S, FILE *info_file)
{
...
int SUNLinSolSetPrintLevel_SPFGMR(SUNLinearSolver S, int print_level)
{
  /* check that the linear solver is non-null */
  if (S == NULL)
    return (SUNLS_MEM_NULL);
