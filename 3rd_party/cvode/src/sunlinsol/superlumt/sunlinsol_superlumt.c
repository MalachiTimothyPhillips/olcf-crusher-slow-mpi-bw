...
SUNLinearSolver_Type SUNLinSolGetType_SuperLUMT(SUNLinearSolver S) { return (SUNLINEARSOLVER_DIRECT); }

SUNLinearSolver_ID SUNLinSolGetID_SuperLUMT(SUNLinearSolver S) { return (SUNLINEARSOLVER_SUPERLUMT); }
...
int SUNLinSolSetup_SuperLUMT(SUNLinearSolver S, SUNMatrix A)
{
  ...
  pxgstrf(OPTIONS(S), SM_AC(S), (int_t *)PERMR(S), SM_L(S), SM_U(S), GSTAT(S), &retval);
  if (retval != 0) {
    LASTFLAG(S) = (retval < 0) ? SUNLS_PACKAGE_FAIL_UNREC : SUNLS_PACKAGE_FAIL_REC;
    return (LASTFLAG(S));
  }
...
int SUNLinSolSolve_SuperLUMT(SUNLinearSolver S, SUNMatrix A, N_Vector x, N_Vector b, realtype tol)
{
  int_t retval;
  realtype *xdata;
  DNformat *Bstore;
...
int SUNLinSolSpace_SuperLUMT(SUNLinearSolver S, long int *lenrwLS, long int *leniwLS)
{
