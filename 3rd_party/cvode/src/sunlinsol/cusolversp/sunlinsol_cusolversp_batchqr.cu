...
SUNLinearSolver
SUNLinSol_cuSolverSp_batchQR(N_Vector y, SUNMatrix A, cusolverSpHandle_t cusol_handle, SUNContext sunctx)
{
...
int SUNLinSolSolve_cuSolverSp_batchQR(SUNLinearSolver S, SUNMatrix A, N_Vector x, N_Vector b, realtype tol)
{
  cusolverStatus_t status;
  int blockrows, blockcols, blocknnz, nblock;
  int *d_rowptr, *d_colind;
