...
SUNLinearSolver_Type SUNLinSolGetType_PCG(SUNLinearSolver S) { return (SUNLINEARSOLVER_ITERATIVE); }

SUNLinearSolver_ID SUNLinSolGetID_PCG(SUNLinearSolver S) { return (SUNLINEARSOLVER_PCG); }
...
int SUNLinSolInitialize_PCG(SUNLinearSolver S)
{
  ...
  if ((PRETYPE(S) != SUN_PREC_LEFT) && (PRETYPE(S) != SUN_PREC_RIGHT) && (PRETYPE(S) != SUN_PREC_BOTH))
...
int SUNLinSolSetPreconditioner_PCG(SUNLinearSolver S, void *PData, SUNPSetupFn Psetup, SUNPSolveFn Psolve)
{
...
int SUNLinSolSolve_PCG(SUNLinearSolver S, SUNMatrix nul, N_Vector x, N_Vector b, realtype delta)
{
  ...
  *nli = 0;
  ...
  UsePrec = ((pretype == SUN_PREC_BOTH) || (pretype == SUN_PREC_LEFT) || (pretype == SUN_PREC_RIGHT));
  UseScaling = (w != NULL);
  ...
  *res_norm = r0_norm = rho = SUNRsqrt(N_VDotProd(Ap, Ap));
  ...
  if (PCG_CONTENT(S)->print_level && PCG_CONTENT(S)->info_file &&
      (PCG_CONTENT(S)->info_file != S->sunctx->logger->info_fp)) {
    fprintf(PCG_CONTENT(S)->info_file, SUNLS_MSG_RESIDUAL, (long int)0, *res_norm);
  }
  ...
  for (l = 0; l < l_max; l++) {
    ...
    *res_norm = rho = SUNRsqrt(N_VDotProd(Ap, Ap));
    ...
    if (PCG_CONTENT(S)->print_level && PCG_CONTENT(S)->info_file &&
        (PCG_CONTENT(S)->info_file != S->sunctx->logger->info_fp)) {
      fprintf(PCG_CONTENT(S)->info_file, SUNLS_MSG_RESIDUAL, (long int)*nli, *res_norm);
    }
    SUNLogger_QueueMsg(S->sunctx->logger,
                       SUN_LOGLEVEL_INFO,
                       "SUNLinSolSolve_PCG",
...
int SUNLinSolSetInfoFile_PCG(SUNLinearSolver S, FILE *info_file)
{
...
int SUNLinSolSetPrintLevel_PCG(SUNLinearSolver S, int print_level)
{
  /* check that the linear solver is non-null */
  if (S == NULL)
    return (SUNLS_MEM_NULL);
