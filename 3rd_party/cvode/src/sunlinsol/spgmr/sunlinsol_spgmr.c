...
SUNLinearSolver_Type SUNLinSolGetType_SPGMR(SUNLinearSolver S) { return (SUNLINEARSOLVER_ITERATIVE); }

SUNLinearSolver_ID SUNLinSolGetID_SPGMR(SUNLinearSolver S) { return (SUNLINEARSOLVER_SPGMR); }
...
int SUNLinSolSetPreconditioner_SPGMR(SUNLinearSolver S, void *PData, SUNPSetupFn Psetup, SUNPSolveFn Psolve)
{
...
int SUNLinSolSolve_SPGMR(SUNLinearSolver S, SUNMatrix A, N_Vector x, N_Vector b, realtype delta)
{
  ...
  *nli = 0;
  ...
  preOnLeft = ((SPGMR_CONTENT(S)->pretype == SUN_PREC_LEFT) || (SPGMR_CONTENT(S)->pretype == SUN_PREC_BOTH));
  preOnRight =
      ((SPGMR_CONTENT(S)->pretype == SUN_PREC_RIGHT) || (SPGMR_CONTENT(S)->pretype == SUN_PREC_BOTH));
  scale1 = (s1 != NULL);
  scale2 = (s2 != NULL);
  ...
  *res_norm = r_norm = beta = SUNRsqrt(N_VDotProd(V[0], V[0]));
  ...
  if (SPGMR_CONTENT(S)->print_level && SPGMR_CONTENT(S)->info_file &&
      (SPGMR_CONTENT(S)->info_file != S->sunctx->logger->info_fp)) {
    fprintf(SPGMR_CONTENT(S)->info_file, SUNLS_MSG_RESIDUAL, (long int)0, *res_norm);
  }
  ...
  for (ntries = 0; ntries <= max_restarts; ntries++) {
    ...
    for (l = 0; l < l_max; l++) {
      ...
      if (gstype == SUN_CLASSICAL_GS) {
        if (SUNClassicalGS(V, Hes, l_plus_1, l_max, &(Hes[l_plus_1][l]), cv, Xv) != 0) {
          *zeroguess = SUNFALSE;
      ...
      *res_norm = rho = SUNRabs(rotation_product * r_norm);
      ...
      if (SPGMR_CONTENT(S)->print_level && SPGMR_CONTENT(S)->info_file &&
          (SPGMR_CONTENT(S)->info_file != S->sunctx->logger->info_fp)) {
        fprintf(SPGMR_CONTENT(S)->info_file, SUNLS_MSG_RESIDUAL, (long int)*nli, *res_norm);
      }
      SUNLogger_QueueMsg(S->sunctx->logger,
                         SUN_LOGLEVEL_INFO,
                         "SUNLinSolSolve_SPGMR",
...
int SUNLinSolSetInfoFile_SPGMR(SUNLinearSolver S, FILE *info_file)
{
