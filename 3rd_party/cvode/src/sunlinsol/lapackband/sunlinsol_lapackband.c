...
SUNLinearSolver_Type SUNLinSolGetType_LapackBand(SUNLinearSolver S) { return (SUNLINEARSOLVER_DIRECT); }

SUNLinearSolver_ID SUNLinSolGetID_LapackBand(SUNLinearSolver S) { return (SUNLINEARSOLVER_LAPACKBAND); }
...
int SUNLinSolSolve_LapackBand(SUNLinearSolver S, SUNMatrix A, N_Vector x, N_Vector b, realtype tol)
{
  ...
  ldim = SUNBandMatrix_LDim(A);
  one = 1;
  xgbtrs_f77("N", &n, &ml, &mu, &one, SUNBandMatrix_Data(A), &ldim, PIVOTS(S), xdata, &n, &ier);
  LASTFLAG(S) = ier;
  if (ier < 0)
    return (SUNLS_PACKAGE_FAIL_UNREC);
...
int SUNLinSolSpace_LapackBand(SUNLinearSolver S, long int *lenrwLS, long int *leniwLS)
{
  *lenrwLS = 0;
