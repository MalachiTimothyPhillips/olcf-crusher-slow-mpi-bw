...
SUNLinearSolver SUNLinSol_KLU(N_Vector y, SUNMatrix A, SUNContext sunctx)
{
  ...
  if ((N_VGetVectorID(y) != SUNDIALS_NVEC_SERIAL) && (N_VGetVectorID(y) != SUNDIALS_NVEC_OPENMP) &&
...
sun_klu_symbolic *SUNLinSol_KLUGetSymbolic(SUNLinearSolver S) { return (SYMBOLIC(S)); }

sun_klu_numeric *SUNLinSol_KLUGetNumeric(SUNLinearSolver S) { return (NUMERIC(S)); }

sun_klu_common *SUNLinSol_KLUGetCommon(SUNLinearSolver S) { return (&(COMMON(S))); }
...
SUNLinearSolver_Type SUNLinSolGetType_KLU(SUNLinearSolver S) { return (SUNLINEARSOLVER_DIRECT); }

SUNLinearSolver_ID SUNLinSolGetID_KLU(SUNLinearSolver S) { return (SUNLINEARSOLVER_KLU); }
...
int SUNLinSolSolve_KLU(SUNLinearSolver S, SUNMatrix A, N_Vector x, N_Vector b, realtype tol)
{
  int flag;
  realtype *xdata;
  ...
  flag = SOLVE(S)(SYMBOLIC(S), NUMERIC(S), SUNSparseMatrix_NP(A), 1, xdata, &COMMON(S));
  if (flag == 0) {
    LASTFLAG(S) = SUNLS_PACKAGE_FAIL_REC;
    return (LASTFLAG(S));
  }
