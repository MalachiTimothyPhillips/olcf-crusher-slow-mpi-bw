...
SUNLinearSolver SUNLinSol_SPBCGS(N_Vector y, int pretype, int maxl, SUNContext sunctx)
{
  ...
  if ((y->ops->nvclone == NULL) || (y->ops->nvdestroy == NULL) || (y->ops->nvlinearsum == NULL) ||
      (y->ops->nvprod == NULL) || (y->ops->nvdiv == NULL) || (y->ops->nvscale == NULL) ||
...
SUNLinearSolver_Type SUNLinSolGetType_SPBCGS(SUNLinearSolver S) { return (SUNLINEARSOLVER_ITERATIVE); }

SUNLinearSolver_ID SUNLinSolGetID_SPBCGS(SUNLinearSolver S) { return (SUNLINEARSOLVER_SPBCGS); }
...
int SUNLinSolSetPreconditioner_SPBCGS(SUNLinearSolver S, void *PData, SUNPSetupFn Psetup, SUNPSolveFn Psolve)
{
...
int SUNLinSolSolve_SPBCGS(SUNLinearSolver S, SUNMatrix A, N_Vector x, N_Vector b, realtype delta)
{
  ...
  *res_norm = r_norm = rho = SUNRsqrt(beta_denom);
  ...
  if (SPBCGS_CONTENT(S)->print_level && SPBCGS_CONTENT(S)->info_file &&
      (SPBCGS_CONTENT(S)->info_file != S->sunctx->logger->info_fp)) {
    fprintf(SPBCGS_CONTENT(S)->info_file, SUNLS_MSG_RESIDUAL, (long int)0, *res_norm);
  }
  ...
  for (l = 0; l < l_max; l++) {
    ...
    *res_norm = rho = SUNRsqrt(N_VDotProd(r, r));
    ...
    if (SPBCGS_CONTENT(S)->print_level && SPBCGS_CONTENT(S)->info_file &&
        (SPBCGS_CONTENT(S)->info_file != S->sunctx->logger->info_fp)) {
      fprintf(SPBCGS_CONTENT(S)->info_file, SUNLS_MSG_RESIDUAL, (long int)*nli, *res_norm);
    }
    SUNLogger_QueueMsg(S->sunctx->logger,
                       SUN_LOGLEVEL_INFO,
                       "SUNLinSolSolve_SPBCGS",
...
int SUNLinSolSetInfoFile_SPBCGS(SUNLinearSolver S, FILE *info_file)
{
...
int SUNLinSolSetPrintLevel_SPBCGS(SUNLinearSolver S, int print_level)
{
  /* check that the linear solver is non-null */
  if (S == NULL)
    return (SUNLS_MEM_NULL);
