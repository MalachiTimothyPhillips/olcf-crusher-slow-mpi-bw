...
SUNLinearSolver SUNLinSol_MagmaDense(N_Vector y, SUNMatrix Amat, SUNContext sunctx)
{
  ...
  if (y->ops->nvgetlength == NULL || y->ops->nvgetdevicearraypointer == NULL || Amat->ops->getid == NULL)
    return (NULL);
  ...
  retval = SUNMemoryHelper_Alloc(content->memhelp,
                                 &content->pivots,
...
SUNLinearSolver_Type SUNLinSolGetType_MagmaDense(SUNLinearSolver S) { return (SUNLINEARSOLVER_DIRECT); }

SUNLinearSolver_ID SUNLinSolGetID_MagmaDense(SUNLinearSolver S) { return (SUNLINEARSOLVER_MAGMADENSE); }
...
int SUNLinSolSolve_MagmaDense(SUNLinearSolver S, SUNMatrix A, N_Vector x, N_Vector b, realtype tol)
{
  /* Check for valid inputs */
  if (S == NULL)
    return (SUNLS_MEM_NULL);
...
int SUNLinSolSpace_MagmaDense(SUNLinearSolver S, long int *lenrwLS, long int *leniwLS)
{
  *lenrwLS = 0;
...
int SUNLinSolFree_MagmaDense(SUNLinearSolver S)
{
  ...
  if (S->content) {
    ...
    if (MAGMADENSE_CONTENT(S)->pivotsarr)
      SUNMemoryHelper_Dealloc(MHELP(S), MAGMADENSE_CONTENT(S)->pivotsarr, nullptr);
