...
N_Vector N_VMake_Raja(sunindextype length, realtype *h_vdata, realtype *d_vdata, SUNContext sunctx)
{
  N_Vector v;
  ...
  if (NVEC_RAJA_CONTENT(v)->device_data == NULL || NVEC_RAJA_CONTENT(v)->host_data == NULL) {
 ...
 */
booleantype N_VIsManagedMemory_Raja(N_Vector x) { return NVEC_RAJA_PRIVATE(x)->use_managed_mem; }
...
void N_VDestroy_Raja(N_Vector v)
{
  ...
  if (NVEC_RAJA_MEMHELP(v)) {
    ...
    SUNDIALS_DEBUG_PRINT("WARNING in N_VDestroy_Raja: mem_helper was NULL when trying to dealloc data, this "
...
void N_VConst_Raja(realtype c, N_Vector Z)
{
  ...
  RAJA::forall<SUNDIALS_RAJA_EXEC_STREAM>(RAJA::RangeSegment(zeroIdx, N),
                                          [=] RAJA_DEVICE(sunindextype i) { zdata[i] = c; });
}
...
void N_VScale_Raja(realtype c, N_Vector X, N_Vector Z)
{
  ...
  RAJA::forall<SUNDIALS_RAJA_EXEC_STREAM>(RAJA::RangeSegment(zeroIdx, N),
                                          [=] RAJA_DEVICE(sunindextype i) { zdata[i] = c * xdata[i]; });
}
...
void N_VAbs_Raja(N_Vector X, N_Vector Z)
{
  ...
  RAJA::forall<SUNDIALS_RAJA_EXEC_STREAM>(RAJA::RangeSegment(zeroIdx, N),
                                          [=] RAJA_DEVICE(sunindextype i) { zdata[i] = abs(xdata[i]); });
}
...
void N_VInv_Raja(N_Vector X, N_Vector Z)
{
  ...
  RAJA::forall<SUNDIALS_RAJA_EXEC_STREAM>(RAJA::RangeSegment(zeroIdx, N),
                                          [=] RAJA_DEVICE(sunindextype i) { zdata[i] = ONE / xdata[i]; });
}
...
void N_VAddConst_Raja(N_Vector X, realtype b, N_Vector Z)
{
  ...
  RAJA::forall<SUNDIALS_RAJA_EXEC_STREAM>(RAJA::RangeSegment(zeroIdx, N),
                                          [=] RAJA_DEVICE(sunindextype i) { zdata[i] = xdata[i] + b; });
}
...
realtype N_VDotProd_Raja(N_Vector X, N_Vector Y)
{
  ...
  RAJA::forall<SUNDIALS_RAJA_EXEC_REDUCE>(RAJA::RangeSegment(zeroIdx, N), [=] RAJA_DEVICE(sunindextype i) {
...
realtype N_VMaxNorm_Raja(N_Vector X)
{
  ...
  RAJA::forall<SUNDIALS_RAJA_EXEC_REDUCE>(RAJA::RangeSegment(zeroIdx, N),
                                          [=] RAJA_DEVICE(sunindextype i) { gpu_result.max(abs(xdata[i])); });
...
realtype N_VMin_Raja(N_Vector X)
{
  ...
  RAJA::forall<SUNDIALS_RAJA_EXEC_REDUCE>(RAJA::RangeSegment(zeroIdx, N),
                                          [=] RAJA_DEVICE(sunindextype i) { gpu_result.min(xdata[i]); });

  return (static_cast<realtype>(gpu_result));
}

realtype N_VWL2Norm_Raja(N_Vector X, N_Vector W) { return std::sqrt(N_VWSqrSumLocal_Raja(X, W)); }
...
realtype N_VL1Norm_Raja(N_Vector X)
{
  ...
  RAJA::ReduceSum<SUNDIALS_RAJA_REDUCE, realtype> gpu_result(0.0);
  RAJA::forall<SUNDIALS_RAJA_EXEC_REDUCE>(RAJA::RangeSegment(zeroIdx, N),
                                          [=] RAJA_DEVICE(sunindextype i) { gpu_result += (abs(xdata[i])); });
...
int N_VLinearSumVectorArray_Raja(int nvec, realtype a, N_Vector *X, realtype b, N_Vector *Y, N_Vector *Z)
{
...
int N_VScaleAddMultiVectorArray_Raja(int nvec, int nsum, realtype *c, N_Vector *X, N_Vector **Y, N_Vector **Z)
{
...
int N_VLinearCombinationVectorArray_Raja(int nvec, int nsum, realtype *c, N_Vector **X, N_Vector *Z)
{
  const sunindextype N = NVEC_RAJA_CONTENT(Z[0])->length;
