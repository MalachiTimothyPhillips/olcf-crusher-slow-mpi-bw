...
static int
VScaleSumVectorArray_Parallel(int nvec, realtype c, N_Vector *X, N_Vector *Y, N_Vector *Z); /* Z=c(X+Y)  */
static int
VScaleDiffVectorArray_Parallel(int nvec, realtype c, N_Vector *X, N_Vector *Y, N_Vector *Z); /* Z=c(X-Y)  */
static int
VLin1VectorArray_Parallel(int nvec, realtype a, N_Vector *X, N_Vector *Y, N_Vector *Z); /* Z=aX+Y    */
static int
VLin2VectorArray_Parallel(int nvec, realtype a, N_Vector *X, N_Vector *Y, N_Vector *Z); /* Z=aX-Y    */
...
N_Vector_ID N_VGetVectorID_Parallel(N_Vector v) { return SUNDIALS_NVEC_PARALLEL; }
...
N_Vector
N_VNewEmpty_Parallel(MPI_Comm comm, sunindextype local_length, sunindextype global_length, SUNContext sunctx)
{
  N_Vector v;
...
N_Vector
N_VNew_Parallel(MPI_Comm comm, sunindextype local_length, sunindextype global_length, SUNContext sunctx)
{
  N_Vector v;
  realtype *data;
...
N_Vector *N_VCloneVectorArray_Parallel(int count, N_Vector w) { return (N_VCloneVectorArray(count, w)); }
...
sunindextype N_VGetLocalLength_Parallel(N_Vector v) { return NV_LOCLENGTH_P(v); }
...
realtype *N_VGetArrayPointer_Parallel(N_Vector v) { return ((realtype *)NV_DATA_P(v)); }

void N_VSetArrayPointer_Parallel(realtype *v_data, N_Vector v)
{
  if (NV_LOCLENGTH_P(v) > 0)
    NV_DATA_P(v) = v_data;

  return;
}

void *N_VGetCommunicator_Parallel(N_Vector v) { return ((void *)&(NV_COMM_P(v))); }
...
booleantype N_VConstrMaskLocal_Parallel(N_Vector c, N_Vector x, N_Vector m)
{
  ...
  for (i = 0; i < N; i++) {
    ...
    test =
        (SUNRabs(cd[i]) > ONEPT5 && xd[i] * cd[i] <= ZERO) || (SUNRabs(cd[i]) > HALF && xd[i] * cd[i] < ZERO);
    if (test) {
      temp = md[i] = ONE;
    }
  }
...
int N_VDotProdMultiLocal_Parallel(int nvec, N_Vector x, N_Vector *Y, realtype *dotprods)
{
  int i;
  sunindextype j, N;
  realtype *xd = NULL;
...
int N_VDotProdMultiAllReduce_Parallel(int nvec_total, N_Vector x, realtype *sum)
{
  ...
  /* perform reduction */
  retval = MPI_Allreduce(MPI_IN_PLACE, sum, nvec_total, MPI_SUNREALTYPE, MPI_SUM, comm);
...
int N_VLinearSumVectorArray_Parallel(int nvec, realtype a, N_Vector *X, realtype b, N_Vector *Y, N_Vector *Z)
{
  int i;
  sunindextype j, N;
  realtype *xd = NULL;
...
int N_VWrmsNormMaskVectorArray_Parallel(int nvec, N_Vector *X, N_Vector *W, N_Vector id, realtype *nrm)
{
  int i, retval;
  sunindextype j, Nl, Ng;
  realtype *wd = NULL;
...
int N_VScaleAddMultiVectorArray_Parallel(int nvec,
                                         int nsum,
                                         realtype *a,
                                         N_Vector *X,
                                         N_Vector **Y,
...
int N_VLinearCombinationVectorArray_Parallel(int nvec, int nsum, realtype *c, N_Vector **X, N_Vector *Z)
{
