...
static int
GetKernelParameters(N_Vector v, booleantype reduction, size_t &nthreads_total, size_t &nthreads_per_block);
...
N_Vector N_VNewManaged_Sycl(sunindextype length, ::sycl::queue *Q, SUNContext sunctx)
{
  /* Check inputs */
  if (Q == NULL) {
...
N_Vector
N_VMake_Sycl(sunindextype length, realtype *h_vdata, realtype *d_vdata, ::sycl::queue *Q, SUNContext sunctx)
{
  /* Check inputs */
  if (Q == NULL) {
  ...
  if (NVEC_SYCL_CONTENT(v)->device_data == NULL || NVEC_SYCL_CONTENT(v)->host_data == NULL) {
...
booleantype N_VIsManagedMemory_Sycl(N_Vector x) { return NVEC_SYCL_PRIVATE(x)->use_managed_mem; }
...
void N_VDestroy_Sycl(N_Vector v)
{
  ...
  if (NVEC_SYCL_MEMHELP(v)) {
    SUNMemoryHelper_Dealloc(NVEC_SYCL_MEMHELP(v), vc->host_data, NVEC_SYCL_QUEUE(v));
    vc->host_data = NULL;
    SUNMemoryHelper_Dealloc(NVEC_SYCL_MEMHELP(v), vc->device_data, NVEC_SYCL_QUEUE(v));
    vc->device_data = NULL;
    if (vc->own_helper)
    ...
    SUNDIALS_DEBUG_PRINT("WARNING in N_VDestroy_Sycl: mem_helper was NULL when trying to dealloc data, this "
...
void N_VLinearSum_Sycl(realtype a, N_Vector x, realtype b, N_Vector y, N_Vector z)
{
  ...
  SYCL_FOR(
      ...
      nthreads_per_block,
      item,
      GRID_STRIDE_XLOOP(item, i, N) { zdata[i] = (a * xdata[i]) + (b * ydata[i]); });
}
...
realtype N_VWSqrSumLocal_Sycl(N_Vector x, N_Vector w)
{
  ...
  SYCL_FOR_REDUCE(
      ...
      sum,
      ::sycl::plus<realtype>(),
      GRID_STRIDE_XLOOP(item, i, N) { sum += xdata[i] * wdata[i] * xdata[i] * wdata[i]; });
...
realtype N_VWL2Norm_Sycl(N_Vector x, N_Vector w) { return std::sqrt(N_VWSqrSumLocal_Sycl(x, w)); }
...
void N_VCompare_Sycl(realtype c, N_Vector x, N_Vector z)
{
  ...
  SYCL_FOR(
      ...
      nthreads_per_block,
      item,
      GRID_STRIDE_XLOOP(item, i, N) { zdata[i] = abs(xdata[i]) >= c ? ONE : ZERO; });
}
...
int N_VScaleAddMulti_Sycl(int nvec, realtype *c, N_Vector x, N_Vector *Y, N_Vector *Z)
{
  const sunindextype N = NVEC_SYCL_LENGTH(x);
  const realtype *xdata = NVEC_SYCL_DDATAp(x);
  ::sycl::queue *Q = NVEC_SYCL_QUEUE(x);
...
int N_VLinearSumVectorArray_Sycl(int nvec, realtype a, N_Vector *X, realtype b, N_Vector *Y, N_Vector *Z)
{
  ...
  if (FusedBuffer_CopyPtrArray1D(Z[0], X, nvec, &xdata)) {
    SUNDIALS_DEBUG_PRINT(
        "ERROR in N_VLinearSumVectorArray_Sycl: FusedBuffer_CopyPtrArray1D returned nonzero\n");
    return -1;
  }
  ...
  if (FusedBuffer_CopyToDevice(Z[0])) {
    SUNDIALS_DEBUG_PRINT(
        "ERROR in N_VLinaerSumVectorArray_Sycl: FusedBuffer_CopyToDevice returned nonzero\n");
    return -1;
  }
...
int N_VScaleAddMultiVectorArray_Sycl(int nvec, int nsum, realtype *c, N_Vector *X, N_Vector **Y, N_Vector **Z)
{
  ...
  if (FusedBuffer_CopyPtrArray1D(X[0], X, nvec, &xdata)) {
    SUNDIALS_DEBUG_PRINT(
        "ERROR in N_VScaleAddMultiVectorArray_Sycl: FusedBuffer_CopyPtrArray1D returned nonzero\n");
    return -1;
  }

  if (FusedBuffer_CopyPtrArray2D(X[0], Y, nvec, nsum, &ydata)) {
    SUNDIALS_DEBUG_PRINT(
        "ERROR in N_VScaleAddMultiVectorArray_Sycl: FusedBuffer_CopyPtrArray2D returned nonzero\n");
    return -1;
  }
      ...
      GRID_STRIDE_XLOOP(item, i, N) {
        for (int j = 0; j < nvec; j++) {
          for (int k = 0; k < nsum; k++) {
            zdata[j * nsum + k][i] = cdata[k] * xdata[j][i] + ydata[j * nsum + k][i];
          }
        }
      });
...
int N_VLinearCombinationVectorArray_Sycl(int nvec, int nsum, realtype *c, N_Vector **X, N_Vector *Z)
{
  const sunindextype N = NVEC_SYCL_LENGTH(Z[0]);
  ::sycl::queue *Q = NVEC_SYCL_QUEUE(Z[0]);
  size_t nthreads_total, nthreads_per_block;
  ...
  if (FusedBuffer_Init(Z[0], nsum, nvec + nvec * nsum)) {
    SUNDIALS_DEBUG_PRINT(
        "ERROR in N_VLinearCombinationVectorArray_Sycl: FusedBuffer_Init returned nonzero\n");
    return -1;
  }

  if (FusedBuffer_CopyRealArray(Z[0], c, nsum, &cdata)) {
    SUNDIALS_DEBUG_PRINT(
        "ERROR in N_VLinearCombinationVectorArray_Sycl: FusedBuffer_CopyRealArray returned nonzero\n");
    return -1;
  }

  if (FusedBuffer_CopyPtrArray2D(Z[0], X, nvec, nsum, &xdata)) {
    SUNDIALS_DEBUG_PRINT(
        "ERROR in N_VLinearCombinationVectorArray_Sycl: FusedBuffer_CopyPtrArray2D returned nonzero\n");
    return -1;
  }

  if (FusedBuffer_CopyPtrArray1D(Z[0], Z, nvec, &zdata)) {
    SUNDIALS_DEBUG_PRINT(
        "ERROR in N_VLinearCombinationVectorArray_Sycl: FusedBuffer_CopyPtrArray1D returned nonzero\n");
    return -1;
  }

  if (FusedBuffer_CopyToDevice(Z[0])) {
    SUNDIALS_DEBUG_PRINT(
        "ERROR in N_VLinearCombinationVectorArray_Sycl: FusedBuffer_CopyToDevice returned nonzero\n");
    return -1;
  }

  if (GetKernelParameters(Z[0], SUNFALSE, nthreads_total, nthreads_per_block)) {
    SUNDIALS_DEBUG_PRINT(
        "ERROR in N_VLinearCombinationVectorArray_Sycl: GetKernelParameters returned nonzero\n");
    return -1;
  }
...
int N_VEnableScaleAddMultiVectorArray_Sycl(N_Vector v, booleantype tf)
{
  ...
  if (v->ops == NULL)
    return -1;
  v->ops->nvscaleaddmultivectorarray = tf ? N_VScaleAddMultiVectorArray_Sycl : NULL;
  return 0;
}
...
int N_VEnableLinearCombinationVectorArray_Sycl(N_Vector v, booleantype tf)
{
  ...
  if (v->ops == NULL)
    return -1;
  v->ops->nvlinearcombinationvectorarray = tf ? N_VLinearCombinationVectorArray_Sycl : NULL;
  return 0;
}
...
static void FreeReductionBuffer(N_Vector v)
{
  ...
  if (vcp->reduce_buffer_dev != NULL)
    SUNMemoryHelper_Dealloc(NVEC_SYCL_MEMHELP(v), vcp->reduce_buffer_dev, NVEC_SYCL_QUEUE(v));
  vcp->reduce_buffer_dev = NULL;

  /* free host mem */
  if (vcp->reduce_buffer_host != NULL)
    SUNMemoryHelper_Dealloc(NVEC_SYCL_MEMHELP(v), vcp->reduce_buffer_host, NVEC_SYCL_QUEUE(v));
...
static int FusedBuffer_Free(N_Vector v)
{
  ...
  if (vcp->fused_buffer_host) {
    SUNMemoryHelper_Dealloc(NVEC_SYCL_MEMHELP(v), vcp->fused_buffer_host, NVEC_SYCL_QUEUE(v));
    vcp->fused_buffer_host = NULL;
  }

  if (vcp->fused_buffer_dev) {
    SUNMemoryHelper_Dealloc(NVEC_SYCL_MEMHELP(v), vcp->fused_buffer_dev, NVEC_SYCL_QUEUE(v));
...
GetKernelParameters(N_Vector v, booleantype reduction, size_t &nthreads_total, size_t &nthreads_per_block)
{
  ...
  exec_policy =
      reduction ? NVEC_SYCL_CONTENT(v)->reduce_exec_policy : NVEC_SYCL_CONTENT(v)->stream_exec_policy;
  ...
  nthreads_total = nthreads_per_block * exec_policy->gridSize(NVEC_SYCL_LENGTH(v));
