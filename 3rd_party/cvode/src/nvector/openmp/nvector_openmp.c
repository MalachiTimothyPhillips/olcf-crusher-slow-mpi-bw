...
static int
VScaleSumVectorArray_OpenMP(int nvec, realtype c, N_Vector *X, N_Vector *Y, N_Vector *Z); /* Z=c(X+Y)  */
static int
VScaleDiffVectorArray_OpenMP(int nvec, realtype c, N_Vector *X, N_Vector *Y, N_Vector *Z); /* Z=c(X-Y)  */
...
N_Vector *N_VCloneVectorArray_OpenMP(int count, N_Vector w) { return (N_VCloneVectorArray(count, w)); }
...
realtype *N_VGetArrayPointer_OpenMP(N_Vector v) { return ((realtype *)NV_DATA_OMP(v)); }
...
#pragma omp parallel for default(none) private(i, test) shared(N, xd, cd, md, temp) schedule(static)         \
    num_threads(NV_NUM_THREADS_OMP(x))
  for (i = 0; i < N; i++) {
    md[i] = ZERO;
...
int N_VLinearSumVectorArray_OpenMP(int nvec, realtype a, N_Vector *X, realtype b, N_Vector *Y, N_Vector *Z)
{
  int i;
  sunindextype j, N;
  realtype *xd = NULL;
...
int N_VWrmsNormMaskVectorArray_OpenMP(int nvec, N_Vector *X, N_Vector *W, N_Vector id, realtype *nrm)
{
  int i;
  sunindextype j, N;
  realtype sum;
...
int N_VScaleAddMultiVectorArray_OpenMP(int nvec,
                                       int nsum,
                                       realtype *a,
                                       N_Vector *X,
                                       N_Vector **Y,
...
int N_VLinearCombinationVectorArray_OpenMP(int nvec, int nsum, realtype *c, N_Vector **X, N_Vector *Z)
{
