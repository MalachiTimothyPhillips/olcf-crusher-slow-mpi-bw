 ...
 */
template <typename T, typename I> __global__ void linearCombinationKernel(int nv, T *c, T **xd, T *zd, I n)
{
...
__global__ void dotProdMultiKernel(int nv, const T *xd, T **yd, T *out, I n)
{
  ...
  // Initialize to zero.
  T sum = Id;
  for (I i = threadIdx.x; i < n; i += blockDim.x) { // each thread computes n/blockDim.x elements
    sum += xd[i] * yd[k][i];
  }
 ...
 */
template <typename T, typename I>
__global__ void linearSumVectorArrayKernel(int nv, T a, T **xd, T b, T **yd, T **zd, I n)
{
  GRID_STRIDE_XLOOP(I, i, n)
  {
    for (int j = 0; j < nv; j++)
 ...
 */
template <typename T, typename I> __global__ void scaleVectorArrayKernel(int nv, T *c, T **xd, T **zd, I n)
{
  GRID_STRIDE_XLOOP(I, i, n)
  {
    for (int j = 0; j < nv; j++)
 ...
 */
template <typename T, typename I> __global__ void constVectorArrayKernel(int nv, T c, T **zd, I n)
{
  GRID_STRIDE_XLOOP(I, i, n)
  {
    for (int j = 0; j < nv; j++)
 ...
 */
template <typename T, typename I, template <typename, typename> class GridReducer>
__global__ void wL2NormSquareMaskVectorArrayKernel(int nv, T **xd, T **wd, T *id, T *out, I n)
{
  // REQUIRES nv blocks (i.e. gridDim.x == nv)
  using op = sundials::reductions::impl::plus<T>;
  constexpr T Id = op::identity();
 ...
 */
template <typename T, typename I>
__global__ void scaleAddMultiVectorArrayKernel(int nv, int ns, T *c, T **xd, T **yd, T **zd, I n)
{
  GRID_STRIDE_XLOOP(I, i, n)
  {
    for (int k = 0; k < nv; k++)
 ...
 */
template <typename T, typename I>
__global__ void linearCombinationVectorArrayKernel(int nv, int ns, T *c, T **xd, T **zd, I n)
{
  GRID_STRIDE_XLOOP(I, i, n)
  {
    for (int k = 0; k < nv; k++) {
