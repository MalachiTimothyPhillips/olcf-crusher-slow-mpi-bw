...
N_Vector N_VMake_Hip(sunindextype length, realtype *h_vdata, realtype *d_vdata, SUNContext sunctx)
{
  ...
  if (NVEC_HIP_CONTENT(v)->device_data == NULL || NVEC_HIP_CONTENT(v)->host_data == NULL) {
...
booleantype N_VIsManagedMemory_Hip(N_Vector x) { return NVEC_HIP_PRIVATE(x)->use_managed_mem; }
...
void N_VConst_Hip(realtype a, N_Vector X)
{
  ...
  setConstKernel<<<grid, block, shMemSize, stream>>>(a, NVEC_HIP_DDATAp(X), NVEC_HIP_CONTENT(X)->length);
  PostKernelLaunch();
}
...
int N_VScaleAddMulti_Hip(int nvec, realtype *c, N_Vector X, N_Vector *Y, N_Vector *Z)
{
  hipError_t err;
...
int N_VLinearSumVectorArray_Hip(int nvec, realtype a, N_Vector *X, realtype b, N_Vector *Y, N_Vector *Z)
{
  hipError_t err;
...
int N_VConstVectorArray_Hip(int nvec, realtype c, N_Vector *Z)
{
  ...
  if (GetKernelParameters(Z[0], false, grid, block, shMemSize, stream))
    return (-1);
  constVectorArrayKernel<<<grid, block, shMemSize, stream>>>(nvec, c, d_Zd, NVEC_HIP_CONTENT(Z[0])->length);
  PostKernelLaunch();
...
int N_VWrmsNormVectorArray_Hip(int nvec, N_Vector *X, N_Vector *W, realtype *norms)
{
  hipError_t err;
  ...
  wL2NormSquareVectorArrayKernel<realtype, sunindextype, GridReducerAtomic>
      <<<grid, block, shMemSize, stream>>>(nvec, d_Xd, d_Wd, d_buff, NVEC_HIP_CONTENT(X[0])->length);
  PostKernelLaunch();
...
int N_VWrmsNormMaskVectorArray_Hip(int nvec, N_Vector *X, N_Vector *W, N_Vector id, realtype *norms)
{
  hipError_t err;
...
int N_VScaleAddMultiVectorArray_Hip(int nvec, int nsum, realtype *c, N_Vector *X, N_Vector **Y, N_Vector **Z)
{
  hipError_t err;
...
int N_VLinearCombinationVectorArray_Hip(int nvec, int nsum, realtype *c, N_Vector **X, N_Vector *Z)
{
  hipError_t err;
...
static void FreeReductionBuffer(N_Vector v)
{
  ...
  if (vcp->reduce_buffer_dev != NULL)
    SUNMemoryHelper_Dealloc(NVEC_HIP_MEMHELP(v), vcp->reduce_buffer_dev, (void *)NVEC_HIP_STREAM(v));
  vcp->reduce_buffer_dev = NULL;

  // Free host mem
  if (vcp->reduce_buffer_host != NULL)
    SUNMemoryHelper_Dealloc(NVEC_HIP_MEMHELP(v), vcp->reduce_buffer_host, (void *)NVEC_HIP_STREAM(v));
