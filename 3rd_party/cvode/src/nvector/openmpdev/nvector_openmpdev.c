...
static int
VScaleSumVectorArray_OpenMPDEV(int nvec, realtype c, N_Vector *X, N_Vector *Y, N_Vector *Z); /* Z=c(X+Y)  */
static int
VScaleDiffVectorArray_OpenMPDEV(int nvec, realtype c, N_Vector *X, N_Vector *Y, N_Vector *Z); /* Z=c(X-Y)  */
static int
VLin1VectorArray_OpenMPDEV(int nvec, realtype a, N_Vector *X, N_Vector *Y, N_Vector *Z); /* Z=aX+Y    */
static int
VLin2VectorArray_OpenMPDEV(int nvec, realtype a, N_Vector *X, N_Vector *Y, N_Vector *Z); /* Z=aX-Y    */
 ...
 */
N_Vector_ID N_VGetVectorID_OpenMPDEV(N_Vector v) { return SUNDIALS_NVEC_OPENMPDEV; }
...
N_Vector *N_VCloneVectorArray_OpenMPDEV(int count, N_Vector w) { return (N_VCloneVectorArray(count, w)); }
 ...
 */
realtype *N_VGetHostArrayPointer_OpenMPDEV(N_Vector v) { return ((realtype *)NV_DATA_HOST_OMPDEV(v)); }

/* ----------------------------------------------------------------------------
 * Function to return a pointer to the data array on the device.
 */
realtype *N_VGetDeviceArrayPointer_OpenMPDEV(N_Vector v) { return ((realtype *)NV_DATA_DEV_OMPDEV(v)); }
...
#pragma omp teams distribute parallel for reduction(min : min) schedule(static, 1)
  for (i = 0; i < N; i++)
    if (dd_dev[i] != ZERO)
      min = SUNMIN(nd_dev[i] / dd_dev[i], min);
...
#pragma omp target map(to                                                                                    \
                       : N, nvec, a[:nvec], yd_dev_ptrs                                                      \
                       [:nvec], zd_dev_ptrs                                                                  \
                       [:nvec]) is_device_ptr(xd_dev, yd_dev, zd_dev) device(dev)
#pragma omp teams distribute
  {
    for (i = 0; i < nvec; i++) {
      yd_dev = yd_dev_ptrs[i];
...
#pragma omp target map(to                                                                                    \
                       : N, nvec, yd_dev_ptrs[:nvec]) map(tofrom                                             \
                                                          : dotprods[:nvec]) is_device_ptr(xd_dev, yd_dev)   \
    device(dev)
#pragma omp teams distribute
  for (i = 0; i < nvec; i++) {
    yd_dev = yd_dev_ptrs[i];
...
int N_VLinearSumVectorArray_OpenMPDEV(int nvec, realtype a, N_Vector *X, realtype b, N_Vector *Y, N_Vector *Z)
{
  int i, dev;
  sunindextype j, N;
  N_Vector *V1;
...
#pragma omp target map(to                                                                                    \
                       : N, nvec, a, b, xd_dev_ptrs[:nvec], yd_dev_ptrs                                      \
                       [:nvec], zd_dev_ptrs                                                                  \
                       [:nvec]) is_device_ptr(xd_dev, yd_dev, zd_dev) device(dev)
#pragma omp teams distribute
  {
...
#pragma omp parallel for schedule(static, 1)
  ...
  for (i = 0; i < nvec; i++)
     ...
     */
#pragma omp target map(to                                                                                    \
                       : N, nvec, c[:nvec], xd_dev_ptrs                                                      \
                       [:nvec], zd_dev_ptrs                                                                  \
                       [:nvec]) is_device_ptr(xd_dev, zd_dev) device(dev)
#pragma omp teams distribute
  {
    for (i = 0; i < nvec; i++) {
      xd_dev = xd_dev_ptrs[i];
...
#pragma omp target map(to                                                                                    \
                       : N, nvec, xd_dev_ptrs[:nvec], wd_dev_ptrs                                            \
                       [:nvec]) map(tofrom                                                                   \
                                    : nrm[:nvec]) is_device_ptr(xd_dev, wd_dev) device(dev)
#pragma omp teams distribute
  {
    for (i = 0; i < nvec; i++) {
      xd_dev = xd_dev_ptrs[i];
...
int N_VWrmsNormMaskVectorArray_OpenMPDEV(int nvec, N_Vector *X, N_Vector *W, N_Vector id, realtype *nrm)
{
  int i, dev;
  sunindextype j, N;
  realtype sum;
...
#pragma omp target map(to                                                                                    \
                       : N, nvec, xd_dev_ptrs[:nvec], wd_dev_ptrs                                            \
                       [:nvec]) map(tofrom                                                                   \
                                    : nrm[:nvec]) is_device_ptr(idd_dev, xd_dev, wd_dev) device(dev)
#pragma omp teams distribute
  {
    for (i = 0; i < nvec; i++) {
      xd_dev = xd_dev_ptrs[i];
...
int N_VScaleAddMultiVectorArray_OpenMPDEV(int nvec,
                                          int nsum,
                                          realtype *a,
                                          N_Vector *X,
                                          N_Vector **Y,
                                          N_Vector **Z)
{
  ...
  if (Y == Z) {
#pragma omp target map(to                                                                                    \
                       : N, nvec, nsum, a[:nsum], xd_dev_ptrs                                                \
                       [:nvec], yd_dev_ptrs                                                                  \
                       [:nvec * nsum]) is_device_ptr(xd_dev, yd_dev) device(dev)
#pragma omp teams distribute
    {
      for (i = 0; i < nvec; i++) {
        xd_dev = xd_dev_ptrs[i];
...
#pragma omp target map(to                                                                                    \
                       : N, nvec, nsum, a[:nsum], xd_dev_ptrs                                                \
                       [:nvec], yd_dev_ptrs                                                                  \
                       [:nvec * nsum], zd_dev_ptrs                                                           \
                       [:nvec * nsum]) is_device_ptr(xd_dev, yd_dev, zd_dev) device(dev)
#pragma omp teams distribute
  {
    for (i = 0; i < nvec; i++) {
      xd_dev = xd_dev_ptrs[i];
...
int N_VLinearCombinationVectorArray_OpenMPDEV(int nvec, int nsum, realtype *c, N_Vector **X, N_Vector *Z)
{
...
#pragma omp target map(to                                                                                    \
                       : N, nvec, c[:nsum], xd_dev_ptrs                                                      \
                       [:nvec * nsum], zd_dev_ptrs                                                           \
                       [:nvec]) is_device_ptr(zd_dev) device(dev)
#pragma omp teams distribute
    {
      for (j = 0; j < nvec; j++) {
        zd_dev = zd_dev_ptrs[j];
...
#pragma omp target map(to                                                                                    \
                       : N, xd_dev_ptrs[:nvec], yd_dev_ptrs                                                  \
                       [:nvec], zd_dev_ptrs                                                                  \
                       [:nvec]) is_device_ptr(xd_dev, yd_dev, zd_dev) device(dev)
#pragma omp teams distribute
  {
...
#pragma omp target map(to                                                                                    \
                       : N, xd_dev_ptrs[:nvec], yd_dev_ptrs                                                  \
                       [:nvec]) is_device_ptr(xd_dev, yd_dev) device(dev)
#pragma omp teams distribute
    {
      for (i = 0; i < nvec; i++) {
        xd_dev = xd_dev_ptrs[i];
