...
static int GetKernelParameters(N_Vector v,
                               booleantype reduction,
                               size_t &grid,
                               size_t &block,
                               size_t &shMemSize,
                               cudaStream_t &stream,
...
N_Vector N_VMake_Cuda(sunindextype length, realtype *h_vdata, realtype *d_vdata, SUNContext sunctx)
{
  ...
  if (NVEC_CUDA_CONTENT(v)->device_data == NULL || NVEC_CUDA_CONTENT(v)->host_data == NULL) {
...
booleantype N_VIsManagedMemory_Cuda(N_Vector x) { return NVEC_CUDA_PRIVATE(x)->use_managed_mem; }
...
void N_VConst_Cuda(realtype a, N_Vector X)
{
  ...
  setConstKernel<<<grid, block, shMemSize, stream>>>(a, NVEC_CUDA_DDATAp(X), NVEC_CUDA_CONTENT(X)->length);
  PostKernelLaunch();
}
...
void N_VLinearSum_Cuda(realtype a, N_Vector X, realtype b, N_Vector Y, N_Vector Z)
{
  ...
  linearSumKernel<<<grid, block, shMemSize, stream>>>(a,
                                                      NVEC_CUDA_DDATAp(X),
                                                      b,
                                                      NVEC_CUDA_DDATAp(Y),
                                                      NVEC_CUDA_DDATAp(Z),
...
void N_VScale_Cuda(realtype a, N_Vector X, N_Vector Z)
{
  ...
  scaleKernel<<<grid, block, shMemSize, stream>>>(a,
                                                  NVEC_CUDA_DDATAp(X),
                                                  NVEC_CUDA_DDATAp(Z),
                                                  NVEC_CUDA_CONTENT(X)->length);
  PostKernelLaunch();
...
void N_VAddConst_Cuda(N_Vector X, realtype b, N_Vector Z)
{
  ...
  addConstKernel<<<grid, block, shMemSize, stream>>>(b,
                                                     NVEC_CUDA_DDATAp(X),
                                                     NVEC_CUDA_DDATAp(Z),
                                                     NVEC_CUDA_CONTENT(X)->length);
  PostKernelLaunch();
...
realtype N_VMin_Cuda(N_Vector X)
{
  ...
  if (atomic) {
    findMinKernel<realtype, sunindextype, GridReducerAtomic>
        <<<grid, block, shMemSize, stream>>>(gpu_result,
                                             NVEC_CUDA_DDATAp(X),
                                             NVEC_CUDA_DBUFFERp(X),
                                             NVEC_CUDA_CONTENT(X)->length,
                                             nullptr);
...
void N_VCompare_Cuda(realtype c, N_Vector X, N_Vector Z)
{
  ...
  compareKernel<<<grid, block, shMemSize, stream>>>(c,
                                                    NVEC_CUDA_DDATAp(X),
                                                    NVEC_CUDA_DDATAp(Z),
                                                    NVEC_CUDA_CONTENT(X)->length);
  PostKernelLaunch();
...
int N_VLinearCombination_Cuda(int nvec, realtype *c, N_Vector *X, N_Vector z)
{
  ...
  linearCombinationKernel<<<grid, block, shMemSize, stream>>>(nvec,
                                                              cdata,
                                                              xdata,
                                                              NVEC_CUDA_DDATAp(z),
...
int N_VScaleAddMulti_Cuda(int nvec, realtype *c, N_Vector x, N_Vector *Y, N_Vector *Z)
{
  ...
  scaleAddMultiKernel<<<grid, block, shMemSize, stream>>>(nvec,
                                                          cdata,
                                                          NVEC_CUDA_DDATAp(x),
                                                          ydata,
                                                          zdata,
...
int N_VDotProdMulti_Cuda(int nvec, N_Vector x, N_Vector *Y, realtype *dots)
{
  ...
  dotProdMultiKernel<realtype, sunindextype, GridReducerAtomic>
      <<<grid, block, shMemSize, stream>>>(nvec,
                                           NVEC_CUDA_DDATAp(x),
                                           ydata,
                                           NVEC_CUDA_DBUFFERp(x),
                                           NVEC_CUDA_CONTENT(x)->length);
...
int N_VLinearSumVectorArray_Cuda(int nvec, realtype a, N_Vector *X, realtype b, N_Vector *Y, N_Vector *Z)
{
  // Shortcuts to the fused op workspace
  realtype **xdata = NULL;
  realtype **ydata = NULL;
  ...
  if (FusedBuffer_CopyPtrArray1D(Z[0], X, nvec, &xdata)) {
    SUNDIALS_DEBUG_PRINT(
        "ERROR in N_VLinearSumVectorArray_Cuda: FusedBuffer_CopyPtrArray1D returned nonzero\n");
    return -1;
  }
  ...
  if (FusedBuffer_CopyToDevice(Z[0])) {
    SUNDIALS_DEBUG_PRINT(
        "ERROR in N_VLinaerSumVectorArray_Cuda: FusedBuffer_CopyToDevice returned nonzero\n");
    return -1;
  }
  ...
  linearSumVectorArrayKernel<<<grid, block, shMemSize, stream>>>(nvec,
                                                                 a,
                                                                 xdata,
                                                                 b,
                                                                 ydata,
...
int N_VScaleVectorArray_Cuda(int nvec, realtype *c, N_Vector *X, N_Vector *Z)
{
  ...
  scaleVectorArrayKernel<<<grid, block, shMemSize, stream>>>(nvec,
                                                             cdata,
                                                             xdata,
                                                             zdata,
                                                             NVEC_CUDA_CONTENT(Z[0])->length);
...
int N_VConstVectorArray_Cuda(int nvec, realtype c, N_Vector *Z)
{
  ...
  constVectorArrayKernel<<<grid, block, shMemSize, stream>>>(nvec, c, zdata, NVEC_CUDA_CONTENT(Z[0])->length);
  PostKernelLaunch();

  return 0;
}

int N_VWrmsNormVectorArray_Cuda(int nvec, N_Vector *X, N_Vector *W, realtype *norms)
{
  // Fused op workspace shortcuts
  realtype **xdata = NULL;
  realtype **wdata = NULL;
  ...
  if (FusedBuffer_CopyPtrArray1D(W[0], X, nvec, &xdata)) {
    SUNDIALS_DEBUG_PRINT(
        "ERROR in N_VWrmsNormVectorArray_Cuda: FusedBuffer_CopyPtrArray1D returned nonzero\n");
    return -1;
  }
  ...
  if (InitializeReductionBuffer(W[0], ZERO, nvec)) {
    SUNDIALS_DEBUG_PRINT(
        "ERROR in N_VWrmsNormVectorArray_Cuda: InitializeReductionBuffer returned nonzero\n");
  }
...
int N_VWrmsNormMaskVectorArray_Cuda(int nvec, N_Vector *X, N_Vector *W, N_Vector id, realtype *norms)
{
  // Fused op workspace shortcuts
  realtype **xdata = NULL;
  realtype **wdata = NULL;
...
int N_VScaleAddMultiVectorArray_Cuda(int nvec, int nsum, realtype *c, N_Vector *X, N_Vector **Y, N_Vector **Z)
{
  // Shortcuts to the fused op workspace
  realtype *cdata = NULL;
  realtype **xdata = NULL;
  ...
  if (FusedBuffer_CopyPtrArray1D(X[0], X, nvec, &xdata)) {
    SUNDIALS_DEBUG_PRINT(
        "ERROR in N_VScaleAddMultiVectorArray_Cuda: FusedBuffer_CopyPtrArray1D returned nonzero\n");
    return -1;
  }

  if (FusedBuffer_CopyPtrArray2D(X[0], Y, nvec, nsum, &ydata)) {
    SUNDIALS_DEBUG_PRINT(
        "ERROR in N_VScaleAddMultiVectorArray_Cuda: FusedBuffer_CopyPtrArray2D returned nonzero\n");
    return -1;
  }
  ...
  scaleAddMultiVectorArrayKernel<<<grid, block, shMemSize, stream>>>(nvec,
                                                                     nsum,
                                                                     cdata,
                                                                     xdata,
                                                                     ydata,
...
int N_VLinearCombinationVectorArray_Cuda(int nvec, int nsum, realtype *c, N_Vector **X, N_Vector *Z)
{
  // Shortcuts to the fused op workspace arrays
  realtype *cdata = NULL;
  realtype **xdata = NULL;
  ...
  if (FusedBuffer_Init(Z[0], nsum, nvec + nvec * nsum)) {
    SUNDIALS_DEBUG_PRINT(
        "ERROR in N_VLinearCombinationVectorArray_Cuda: FusedBuffer_Init returned nonzero\n");
    return -1;
  }

  if (FusedBuffer_CopyRealArray(Z[0], c, nsum, &cdata)) {
    SUNDIALS_DEBUG_PRINT(
        "ERROR in N_VLinearCombinationVectorArray_Cuda: FusedBuffer_CopyRealArray returned nonzero\n");
    return -1;
  }

  if (FusedBuffer_CopyPtrArray2D(Z[0], X, nvec, nsum, &xdata)) {
    SUNDIALS_DEBUG_PRINT(
        "ERROR in N_VLinearCombinationVectorArray_Cuda: FusedBuffer_CopyPtrArray2D returned nonzero\n");
    return -1;
  }

  if (FusedBuffer_CopyPtrArray1D(Z[0], Z, nvec, &zdata)) {
    SUNDIALS_DEBUG_PRINT(
        "ERROR in N_VLinearCombinationVectorArray_Cuda: FusedBuffer_CopyPtrArray1D returned nonzero\n");
    return -1;
  }

  if (FusedBuffer_CopyToDevice(Z[0])) {
    SUNDIALS_DEBUG_PRINT(
        "ERROR in N_VLinearCombinationVectorArray_Cuda: FusedBuffer_CopyToDevice returned nonzero\n");
    return -1;
  }
  ...
  if (GetKernelParameters(Z[0], false, grid, block, shMemSize, stream)) {
    SUNDIALS_DEBUG_PRINT(
        "ERROR in N_VLinearCombinationVectorArray_Cuda: GetKernelParameters returned nonzero\n");
    return -1;
  }
...
int N_VEnableScaleAddMultiVectorArray_Cuda(N_Vector v, booleantype tf)
{
  ...
  if (v->ops == NULL)
    return -1;
  v->ops->nvscaleaddmultivectorarray = tf ? N_VScaleAddMultiVectorArray_Cuda : NULL;
  return 0;
}
...
int N_VEnableLinearCombinationVectorArray_Cuda(N_Vector v, booleantype tf)
{
  ...
  if (v->ops == NULL)
    return -1;
  v->ops->nvlinearcombinationvectorarray = tf ? N_VLinearCombinationVectorArray_Cuda : NULL;
  return 0;
}
...
static int AllocateData(N_Vector v)
{
  ...
  if (vcp->use_managed_mem) {
    ...
    if (alloc_fail) {
      SUNDIALS_DEBUG_PRINT(
          "ERROR in AllocateData: SUNMemoryHelper_Alloc failed to alloc SUNMEMTYPE_DEVICE\n");
    }
  }
...
static int InitializeReductionBuffer(N_Vector v, realtype value, size_t n)
{
  ...
  if (alloc_mem) {
    ...
    alloc_fail = SUNMemoryHelper_Alloc(NVEC_CUDA_MEMHELP(v),
                                       ...
                                       (void *)NVEC_CUDA_STREAM(v));
    if (alloc_fail) {
      SUNDIALS_DEBUG_PRINT("WARNING in InitializeReductionBuffer: SUNMemoryHelper_Alloc failed to alloc "
      ...
      if (alloc_fail) {
        SUNDIALS_DEBUG_PRINT(
            "ERROR in InitializeReductionBuffer: SUNMemoryHelper_Alloc failed to alloc SUNMEMTYPE_HOST\n");
      }
    }
    ...
    if (alloc_fail) {
      SUNDIALS_DEBUG_PRINT(
          "ERROR in InitializeReductionBuffer: SUNMemoryHelper_Alloc failed to alloc SUNMEMTYPE_DEVICE\n");
    }
  }
...
static void FreeReductionBuffer(N_Vector v)
{
  ...
  if (vcp->reduce_buffer_dev != NULL)
    SUNMemoryHelper_Dealloc(NVEC_CUDA_MEMHELP(v), vcp->reduce_buffer_dev, (void *)NVEC_CUDA_STREAM(v));
  vcp->reduce_buffer_dev = NULL;

  // Free host mem
  if (vcp->reduce_buffer_host != NULL)
    SUNMemoryHelper_Dealloc(NVEC_CUDA_MEMHELP(v), vcp->reduce_buffer_host, (void *)NVEC_CUDA_STREAM(v));
...
static int CopyReductionBufferFromDevice(N_Vector v, size_t n)
{
  ...
  if (copy_fail) {
    SUNDIALS_DEBUG_PRINT(
        "ERROR in CopyReductionBufferFromDevice: SUNMemoryHelper_CopyAsync returned nonzero\n");
  }
...
#error Incompatible precision for CUDA
  ...
  if (alloc_mem) {
    ...
    alloc_fail = SUNMemoryHelper_Alloc(NVEC_CUDA_MEMHELP(v),
                                       ...
                                       (void *)NVEC_CUDA_STREAM(v));
    if (alloc_fail) {
      SUNDIALS_DEBUG_PRINT("WARNING in FusedBuffer_Init: SUNMemoryHelper_Alloc failed to alloc "
                           "SUNMEMTYPE_PINNED, using SUNMEMTYPE_HOST instead\n");
      ...
      if (alloc_fail) {
        SUNDIALS_DEBUG_PRINT(
            "ERROR in FusedBuffer_Init: SUNMemoryHelper_Alloc failed to alloc SUNMEMTYPE_HOST\n");
        return -1;
      }
    }
    ...
    if (alloc_fail) {
      SUNDIALS_DEBUG_PRINT(
          "ERROR in FusedBuffer_Init: SUNMemoryHelper_Alloc failed to alloc SUNMEMTYPE_DEVICE\n");
      return -1;
    }
...
static int FusedBuffer_CopyRealArray(N_Vector v, realtype *rdata, int nval, realtype **shortcut)
{
  // Get the vector private memory structure
  N_PrivateVectorContent_Cuda vcp = NVEC_CUDA_PRIVATE(v);
  ...
  realtype *h_buffer = (realtype *)((char *)(vcp->fused_buffer_host->ptr) + vcp->fused_buffer_offset);
  ...
  *shortcut = (realtype *)((char *)(vcp->fused_buffer_dev->ptr) + vcp->fused_buffer_offset);
...
static int FusedBuffer_Free(N_Vector v)
{
  ...
  if (vcp->fused_buffer_host) {
    SUNMemoryHelper_Dealloc(NVEC_CUDA_MEMHELP(v), vcp->fused_buffer_host, (void *)NVEC_CUDA_STREAM(v));
    vcp->fused_buffer_host = NULL;
  }

  if (vcp->fused_buffer_dev) {
    SUNMemoryHelper_Dealloc(NVEC_CUDA_MEMHELP(v), vcp->fused_buffer_dev, (void *)NVEC_CUDA_STREAM(v));
