...
template <typename T, typename I> __global__ void setConstKernel(T a, T *X, I n)
{
...
template <typename T, typename I> __global__ void linearSumKernel(T a, const T *X, T b, const T *Y, T *Z, I n)
{
...
template <typename T, typename I> __global__ void prodKernel(const T *X, const T *Y, T *Z, I n)
{
...
template <typename T, typename I> __global__ void divKernel(const T *X, const T *Y, T *Z, I n)
{
  GRID_STRIDE_XLOOP(I, i, n) { Z[i] = X[i] / Y[i]; }
}
...
template <typename T, typename I> __global__ void scaleKernel(T a, const T *X, T *Z, I n)
{
...
template <typename T, typename I> __global__ void absKernel(const T *X, T *Z, I n)
{
...
template <typename T, typename I> __global__ void invKernel(const T *X, T *Z, I n)
{
...
template <typename T, typename I> __global__ void addConstKernel(T a, const T *X, T *Z, I n)
{
  GRID_STRIDE_XLOOP(I, i, n) { Z[i] = a + X[i]; }
}
...
template <typename T, typename I> __global__ void compareKernel(T c, const T *X, T *Z, I n)
{
...
__global__ void dotProdKernel(const T *x, const T *y, T *out, I n, unsigned int *device_count)
{
 ...
 */
template <typename T, typename I, template <typename, typename> class GridReducer>
__global__ void wL2NormSquareKernel(const T *x, const T *w, T *out, I n, unsigned int *device_count)
{
  using op = sundials::reductions::impl::plus<T>;
  const T Id = op::identity();
 ...
 */
template <typename T, typename I, template <typename, typename> class GridReducer>
__global__ void findMinKernel(T MAX_VAL, const T *x, T *out, I n, unsigned int *device_count)
{
 ...
 */
template <typename T, typename I, template <typename, typename> class GridReducer>
__global__ void invTestKernel(const T *x, T *z, T *out, I n, unsigned int *device_count)
{
  using op = sundials::reductions::impl::plus<T>;
  const T Id = op::identity();
 ...
 */
template <typename T, typename I, template <typename, typename> class GridReducer>
__global__ void constrMaskKernel(const T *c, const T *x, T *m, T *out, I n, unsigned int *device_count)
{
  using op = sundials::reductions::impl::plus<T>;
  const T Id = op::identity();
  ...
  GRID_STRIDE_XLOOP(I, i, n)
  {
    ...
    bool test = (std::abs(c[i]) > 1.5 && c[i] * x[i] <= 0.0) || (std::abs(c[i]) > 0.5 && c[i] * x[i] < 0.0);
    m[i] = test ? 1.0 : 0.0;
    sum = m[i];
  }
  GridReducer<T, op>{}(sum, Id, out, device_count);
 ...
 */
template <typename T, typename I, template <typename, typename> class GridReducer>
__global__ void minQuotientKernel(const T MAX_VAL,
                                  const T *num,
                                  const T *den,
                                  T *min_quotient,
                                  I n,
