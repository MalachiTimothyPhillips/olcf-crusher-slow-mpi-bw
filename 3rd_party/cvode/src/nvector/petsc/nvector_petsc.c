...
N_Vector
N_VNewEmpty_Petsc(MPI_Comm comm, sunindextype local_length, sunindextype global_length, SUNContext sunctx)
{
  N_Vector v;
...
N_Vector *N_VCloneVectorArray_Petsc(int count, N_Vector w) { return (N_VCloneVectorArray(count, w)); }
 ...
 */
realtype *N_VGetArrayPointer_Petsc(N_Vector v) { return NULL; }

/*
 * Not implemented for PETSc wrapper.
 */
void N_VSetArrayPointer_Petsc(realtype *v_data, N_Vector v) { return; }

void *N_VGetCommunicator_Petsc(N_Vector v) { return ((void *)&(NV_COMM_PTC(v))); }
...
realtype N_VMaxNormLocal_Petsc(N_Vector x)
{
  ...
  VecGetArray(xv, &xd);
  for (i = 0; i < N; i++) {
    if (PetscAbsScalar(xd[i]) > max)
      max = PetscAbsScalar(xd[i]);
  }
  VecRestoreArray(xv, &xd);
  return ((realtype)max);
}
...
booleantype N_VConstrMaskLocal_Petsc(N_Vector c, N_Vector x, N_Vector m)
{
  ...
  for (i = 0; i < N; i++) {
    ...
    test = (SUNRabs(cc) > ONEPT5 && xx * cc <= ZERO) || (SUNRabs(cc) > HALF && xx * cc < ZERO);
    if (test) {
      temp = md[i] = ONE;
    }
  }
...
int N_VDotProdMultiLocal_Petsc(int nvec, N_Vector x, N_Vector *Y, realtype *dotprods)
{
  int j;
  sunindextype i;
  sunindextype N = NV_LOCLENGTH_PTC(x);
...
{
  int retval;
  retval = MPI_Allreduce(MPI_IN_PLACE, dotprods, nvec, MPI_SUNREALTYPE, MPI_SUM, NV_COMM_PTC(x));
...
int N_VLinearSumVectorArray_Petsc(int nvec, realtype a, N_Vector *X, realtype b, N_Vector *Y, N_Vector *Z)
{
  int i;
  sunindextype j, N;
  PetscScalar *xd, *yd, *zd;
...
int N_VWrmsNormMaskVectorArray_Petsc(int nvec, N_Vector *X, N_Vector *W, N_Vector id, realtype *nrm)
{
  int i, retval;
  sunindextype j, Nl, Ng;
...
int N_VScaleAddMultiVectorArray_Petsc(int nvec,
                                      int nsum,
                                      realtype *a,
                                      N_Vector *X,
                                      N_Vector **Y,
...
int N_VLinearCombinationVectorArray_Petsc(int nvec, int nsum, realtype *c, N_Vector **X, N_Vector *Z)
{
