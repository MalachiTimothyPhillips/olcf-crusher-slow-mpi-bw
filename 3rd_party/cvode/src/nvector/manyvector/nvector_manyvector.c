...
N_Vector
N_VMake_MPIManyVector(MPI_Comm comm, sunindextype num_subvectors, N_Vector *vec_array, SUNContext sunctx)
{
  N_Vector v;
  ...
  if (content->comm != MPI_COMM_NULL) {
    retval =
        MPI_Allreduce(&local_length, &(content->global_length), 1, MPI_SUNINDEXTYPE, MPI_SUM, content->comm);
    if (retval != MPI_SUCCESS) {
      N_VDestroy(v);
      return (NULL);
    }
...
sunindextype MVAPPEND(N_VGetNumSubvectors)(N_Vector v) { return (MANYVECTOR_NUM_SUBVECS(v)); }
...
N_Vector MVAPPEND(N_VCloneEmpty)(N_Vector w) { return (ManyVectorClone(w, SUNTRUE)); }

/* Clones a ManyVector, calling Clone on subvectors. */
N_Vector MVAPPEND(N_VClone)(N_Vector w) { return (ManyVectorClone(w, SUNFALSE)); }
...
sunindextype MVAPPEND(N_VGetLength)(N_Vector v) { return (MANYVECTOR_GLOBLENGTH(v)); }
...
void MVAPPEND(N_VLinearSum)(realtype a, N_Vector x, realtype b, N_Vector y, N_Vector z)
{
  sunindextype i;
  for (i = 0; i < MANYVECTOR_NUM_SUBVECS(x); i++)
    N_VLinearSum(a, MANYVECTOR_SUBVEC(x, i), b, MANYVECTOR_SUBVEC(y, i), MANYVECTOR_SUBVEC(z, i));
  return;
}
...
void MVAPPEND(N_VProd)(N_Vector x, N_Vector y, N_Vector z)
{
  sunindextype i;
  for (i = 0; i < MANYVECTOR_NUM_SUBVECS(x); i++)
    N_VProd(MANYVECTOR_SUBVEC(x, i), MANYVECTOR_SUBVEC(y, i), MANYVECTOR_SUBVEC(z, i));
  return;
}
...
void MVAPPEND(N_VDiv)(N_Vector x, N_Vector y, N_Vector z)
{
  sunindextype i;
  for (i = 0; i < MANYVECTOR_NUM_SUBVECS(x); i++)
    N_VDiv(MANYVECTOR_SUBVEC(x, i), MANYVECTOR_SUBVEC(y, i), MANYVECTOR_SUBVEC(z, i));
  return;
}
...
realtype MVAPPEND(N_VWSqrSumMaskLocal)(N_Vector x, N_Vector w, N_Vector id)
{
  ...
  for (i = 0; i < MANYVECTOR_NUM_SUBVECS(x); i++) {
    ...
    if (MANYVECTOR_SUBVEC(x, i)->ops->nvwsqrsummasklocal) {
      ...
      sum += N_VWSqrSumMaskLocal(MANYVECTOR_SUBVEC(x, i), MANYVECTOR_SUBVEC(w, i), MANYVECTOR_SUBVEC(id, i));

      /* otherwise, call nvwrmsnormmask, and accumulate to overall sum on root task */
    }
    else {

      contrib = N_VWrmsNormMask(MANYVECTOR_SUBVEC(x, i), MANYVECTOR_SUBVEC(w, i), MANYVECTOR_SUBVEC(id, i));
...
booleantype MVAPPEND(N_VConstrMaskLocal)(N_Vector c, N_Vector x, N_Vector m)
{
  ...
  for (i = 0; i < MANYVECTOR_NUM_SUBVECS(x); i++) {
    ...
    if (MANYVECTOR_SUBVEC(x, i)->ops->nvconstrmasklocal) {
      ...
      subval = N_VConstrMaskLocal(MANYVECTOR_SUBVEC(c, i), MANYVECTOR_SUBVEC(x, i), MANYVECTOR_SUBVEC(m, i));
      val = (val && subval);
      ...
      subval = N_VConstrMask(MANYVECTOR_SUBVEC(c, i), MANYVECTOR_SUBVEC(x, i), MANYVECTOR_SUBVEC(m, i));
      val = (val && subval);
    }
  }
...
realtype MVAPPEND(N_VMinQuotientLocal)(N_Vector num, N_Vector denom)
{
  ...
  for (i = 0; i < MANYVECTOR_NUM_SUBVECS(num); i++) {
    ...
    if (MANYVECTOR_SUBVEC(num, i)->ops->nvminquotientlocal) {
      ...
      lmin = N_VMinQuotientLocal(MANYVECTOR_SUBVEC(num, i), MANYVECTOR_SUBVEC(denom, i));
      min = (min < lmin) ? min : lmin;
      ...
      lmin = N_VMinQuotient(MANYVECTOR_SUBVEC(num, i), MANYVECTOR_SUBVEC(denom, i));
      min = (min < lmin) ? min : lmin;
    }
  }
...
int MVAPPEND(N_VDotProdMultiLocal)(int nvec, N_Vector x, N_Vector *Y, realtype *dotprods)
{
  int j, retval;
  sunindextype i;
  N_Vector *Ysub;
...
int N_VDotProdMultiAllReduce_MPIManyVector(int nvec_total, N_Vector x, realtype *sum)
{
  ...
  if (MANYVECTOR_COMM(x) != MPI_COMM_NULL)
    return (MPI_Allreduce(MPI_IN_PLACE, sum, nvec_total, MPI_SUNREALTYPE, MPI_SUM, MANYVECTOR_COMM(x)));
...
int MVAPPEND(N_VDotProdMulti)(int nvec, N_Vector x, N_Vector *Y, realtype *dotprods)
{
  ...
  /* accumulate totals and return */
  if (MANYVECTOR_COMM(x) != MPI_COMM_NULL)
    return (MPI_Allreduce(MPI_IN_PLACE, dotprods, nvec, MPI_SUNREALTYPE, MPI_SUM, MANYVECTOR_COMM(x)));
#endif
...
int MVAPPEND(N_VLinearSumVectorArray)(int nvec, realtype a, N_Vector *X, realtype b, N_Vector *Y, N_Vector *Z)
{
  sunindextype i, j;
  int retval;
...
int MVAPPEND(N_VWrmsNormVectorArray)(int nvec, N_Vector *X, N_Vector *W, realtype *nrm)
{
  ...
  if (MANYVECTOR_COMM(X[0]) != MPI_COMM_NULL)
    retval = (MPI_Allreduce(MPI_IN_PLACE, nrm, nvec, MPI_SUNREALTYPE, MPI_SUM, MANYVECTOR_COMM(X[0])) ==
              MPI_SUCCESS)
                 ? 0
                 : -1;
#endif
...
int MVAPPEND(N_VWrmsNormMaskVectorArray)(int nvec, N_Vector *X, N_Vector *W, N_Vector id, realtype *nrm)
{
  sunindextype i;
  int retval;
