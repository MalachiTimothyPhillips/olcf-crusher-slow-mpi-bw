 ...
 */
N_Vector_ID N_VGetVectorID_Trilinos(N_Vector v) { return SUNDIALS_NVEC_TRILINOS; }
...
N_Vector N_VClone_Trilinos(N_Vector w)
{
  ...
  Teuchos::RCP<vector_type> tvec = Teuchos::rcp(new vector_type(*wvec, Teuchos::Copy));
...
void N_VDestroy_Trilinos(N_Vector v)
{
  ...
  if (v->content != NULL) {
    TpetraVectorInterface *iface = reinterpret_cast<TpetraVectorInterface *>(v->content);
