...
typedef struct CVodeMemRec {
  ...
  int (*cv_lsetup)(struct CVodeMemRec *cv_mem,
                   int convfail,
                   N_Vector ypred,
                   N_Vector fpred,
                   booleantype *jcurPtr,
  ...
  int (*cv_lsolve)(struct CVodeMemRec *cv_mem, N_Vector b, N_Vector weight, N_Vector ycur, N_Vector fcur);
...
#define MSGCV_BAD_TOUT                                                                                       \
  "Trouble interpolating at " MSG_TIME_TOUT ". tout too far back in direction of integration"
...
#define MSGCV_HNIL                                                                                           \
  "Internal " MSG_TIME_H " are such that t + h = t on the next step. The solver will continue anyway."
#define MSGCV_ERR_FAILS "At " MSG_TIME_H ", the error test failed repeatedly or with |h| = hmin."
#define MSGCV_CONV_FAILS                                                                                     \
  "At " MSG_TIME_H ", the corrector convergence test failed repeatedly or with |h| = hmin."
...
#define MSGCV_RHSFUNC_UNREC                                                                                  \
  "At " MSG_TIME ", the right-hand side failed in a recoverable manner, but no recovery is possible."
...
#define MSGCV_BAD_TSTOP                                                                                      \
  "The value " MSG_TIME_TSTOP " is behind current " MSG_TIME " in the direction of integration."
#define MSGCV_INACTIVE_ROOTS                                                                                 \
  "At the end of the first step, there are still some root functions identically 0. This warning will not "  \
  "be issued again."
