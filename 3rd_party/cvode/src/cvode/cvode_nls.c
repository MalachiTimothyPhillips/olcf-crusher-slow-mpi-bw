...
static int cvNlsConvTest(SUNNonlinearSolver NLS,
                         N_Vector ycor,
                         N_Vector del,
                         realtype tol,
                         N_Vector ewt,
...
int CVodeSetNonlinearSolver(void *cvode_mem, SUNNonlinearSolver NLS)
{
  ...
  if (NLS == NULL) {
    cvProcessError(NULL, CV_ILL_INPUT, "CVODE", "CVodeSetNonlinearSolver", "NLS must be non-NULL");
    return (CV_ILL_INPUT);
  }

  /* check for required nonlinear solver functions */
  if (NLS->ops->gettype == NULL || NLS->ops->solve == NULL || NLS->ops->setsysfn == NULL) {
  ...
  if (SUNNonlinSolGetType(NLS) == SUNNONLINEARSOLVER_ROOTFIND) {
    ...
    cvProcessError(cv_mem, CV_ILL_INPUT, "CVODE", "CVodeSetNonlinearSolver", "Invalid nonlinear solver type");
    return (CV_ILL_INPUT);
  }
  ...
  if (!(cv_mem->cv_f)) {
    cvProcessError(cv_mem, CV_ILL_INPUT, "CVODE", "CVodeSetNonlinearSolver", "The ODE RHS function is NULL");
    return (CV_ILL_INPUT);
  }
...
/*---------------------------------------------------------------
  ...
  ---------------------------------------------------------------*/
int CVodeGetNonlinearSystemData(void *cvode_mem,
                                realtype *tcur,
                                N_Vector *ypred,
                                N_Vector *yn,
                                N_Vector *fn,
...
int cvNlsInit(CVodeMem cvode_mem)
{
  ...
  if (retval != CV_SUCCESS) {
    cvProcessError(cvode_mem, CV_ILL_INPUT, "CVODE", "cvNlsInit", MSGCV_NLS_INIT_FAIL);
    return (CV_NLS_INIT_FAIL);
  }
...
static int cvNlsLSetup(booleantype jbad, booleantype *jcur, void *cvode_mem)
{
  ...
  retval = cv_mem->cv_lsetup(cv_mem,
                             cv_mem->convfail,
                             cv_mem->cv_y,
                             cv_mem->cv_ftemp,
                             &(cv_mem->cv_jcur),
...
static int cvNlsResidual(N_Vector ycor, N_Vector res, void *cvode_mem)
{
  ...
  retval = cv_mem->nls_f(cv_mem->cv_tn, cv_mem->cv_y, cv_mem->cv_ftemp, cv_mem->cv_user_data);
  cv_mem->cv_nfe++;
  if (retval < 0)
  ...
  if (cv_mem->cv_usefused) {
    cvNlsResid_fused(cv_mem->cv_rl1, -cv_mem->cv_gamma, cv_mem->cv_zn[1], ycor, cv_mem->cv_ftemp, res);
  }
  else
#endif
  {
