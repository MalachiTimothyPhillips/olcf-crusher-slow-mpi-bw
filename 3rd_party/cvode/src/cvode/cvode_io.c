...
int CVodeSetDeltaGammaMaxLSetup(void *cvode_mem, realtype dgmax_lsetup)
{
  ...
  if (cvode_mem == NULL) {
    cvProcessError(NULL, CV_MEM_NULL, "CVODE", "CVodeSetDeltaGammaMaxLSetup", MSGCV_NO_MEM);
    return (CV_MEM_NULL);
  }
...
int CVodeSetEtaFixedStepBounds(void *cvode_mem, realtype eta_min_fx, realtype eta_max_fx)
{
  ...
  if (cvode_mem == NULL) {
    cvProcessError(NULL, CV_MEM_NULL, "CVODE", "CVodeSetEtaFixedStepBounds", MSGCV_NO_MEM);
    return (CV_MEM_NULL);
  }
...
int CVodeSetEtaMaxFirstStep(void *cvode_mem, realtype eta_max_fs)
{
  ...
  if (cvode_mem == NULL) {
    cvProcessError(NULL, CV_MEM_NULL, "CVODE", "CVodeSetEtaMaxFirstStep", MSGCV_NO_MEM);
    return (CV_MEM_NULL);
  }
...
int CVodeSetEtaMaxEarlyStep(void *cvode_mem, realtype eta_max_es)
{
  ...
  if (cvode_mem == NULL) {
    cvProcessError(NULL, CV_MEM_NULL, "CVODE", "CVodeSetEtaMaxEarlyStep", MSGCV_NO_MEM);
    return (CV_MEM_NULL);
  }
...
int CVodeSetNumStepsEtaMaxEarlyStep(void *cvode_mem, long int small_nst)
{
  ...
  if (cvode_mem == NULL) {
    cvProcessError(NULL, CV_MEM_NULL, "CVODE", "CVodeSetNumStepsEtaMaxEarlyStep", MSGCV_NO_MEM);
    return (CV_MEM_NULL);
  }
...
int CVodeSetEtaMinErrFail(void *cvode_mem, realtype eta_min_ef)
{
  ...
  if (cvode_mem == NULL) {
    cvProcessError(NULL, CV_MEM_NULL, "CVODE", "CVodeSetEtaMinErrFail", MSGCV_NO_MEM);
    return (CV_MEM_NULL);
  }
...
int CVodeSetEtaMaxErrFail(void *cvode_mem, realtype eta_max_ef)
{
  ...
  if (cvode_mem == NULL) {
    cvProcessError(NULL, CV_MEM_NULL, "CVODE", "CVodeSetEtaMaxErrFail", MSGCV_NO_MEM);
    return (CV_MEM_NULL);
  }
...
int CVodeSetNumFailsEtaMaxErrFail(void *cvode_mem, int small_nef)
{
  ...
  if (cvode_mem == NULL) {
    cvProcessError(NULL, CV_MEM_NULL, "CVODE", "CVodeSetNumFailsEtaMaxErrFail", MSGCV_NO_MEM);
    return (CV_MEM_NULL);
  }
...
int CVodeSetEtaConvFail(void *cvode_mem, realtype eta_cf)
{
  ...
  if (cvode_mem == NULL) {
    cvProcessError(NULL, CV_MEM_NULL, "CVODE", "CVodeSetEtaConvFail", MSGCV_NO_MEM);
    return (CV_MEM_NULL);
  }
...
int CVodeSetLSetupFrequency(void *cvode_mem, long int msbp)
{
  ...
  if (cvode_mem == NULL) {
    cvProcessError(NULL, CV_MEM_NULL, "CVODE", "CVodeSetLSetupFrequency", MSGCV_NO_MEM);
    return (CV_MEM_NULL);
  }
...
int CVodeSetConstraints(void *cvode_mem, N_Vector constraints)
{
  ...
  if (constraints->ops->nvdiv == NULL || constraints->ops->nvmaxnorm == NULL ||
      constraints->ops->nvcompare == NULL || constraints->ops->nvconstrmask == NULL ||
...
int CVodeSetUseIntegratorFusedKernels(void *cvode_mem, booleantype onoff)
{
  ...
  if (!cv_mem->cv_MallocDone || (id != SUNDIALS_NVEC_CUDA && id != SUNDIALS_NVEC_HIP)) {
    cvProcessError(cv_mem,
                   CV_ILL_INPUT,
                   "CVODE",
                   "CVodeSetUseIntegratorFusedKernels",
...
int CVodeGetNumStepSolveFails(void *cvode_mem, long int *nncfails)
{
  ...
  if (cvode_mem == NULL) {
    cvProcessError(NULL, CV_MEM_NULL, "CVODE", "CVodeGetNumStepSolveFails", MSGCV_NO_MEM);
    return (CV_MEM_NULL);
  }
...
int CVodePrintAllStats(void *cvode_mem, FILE *outfile, SUNOutputFormat fmt)
{
  ...
  if (cvode_mem == NULL) {
    cvProcessError(NULL, CV_MEM_NULL, "CVODE", "CVodePrintAllStats", MSGCV_NO_MEM);
    return (CV_MEM_NULL);
  }
  ...
  case SUN_OUTPUTFORMAT_CSV:
    ...
    if (cv_mem->cv_nst > 0) {
      fprintf(outfile, ",NLS iters per step,%" RSYM, (realtype)cv_mem->cv_nni / (realtype)cv_mem->cv_nst);
    }
    else {
    ...
    if (cv_mem->cv_lmem) {
      ...
      if (cv_mem->cv_nni > 0) {
        fprintf(outfile, ",LS iters per NLS iter,%" RSYM, (realtype)cvls_mem->nli / (realtype)cv_mem->cv_nni);
        fprintf(outfile,
                ",Jac evals per NLS iter,%" RSYM,
                (realtype)cvls_mem->nje / (realtype)cv_mem->cv_nni);
        fprintf(outfile,
  ...
  default:
    cvProcessError(cv_mem, CV_ILL_INPUT, "CVODE", "CVodePrintAllStats", "Invalid formatting option.");
    return (CV_ILL_INPUT);
  }
