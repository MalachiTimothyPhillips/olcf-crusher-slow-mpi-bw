...
static int cvLsLinSys(realtype t,
                      N_Vector y,
                      N_Vector fy,
                      SUNMatrix A,
                      booleantype jok,
...
int CVodeSetLinearSolver(void *cvode_mem, SUNLinearSolver LS, SUNMatrix A)
{
  ...
  if (cvode_mem == NULL) {
    cvProcessError(NULL, CVLS_MEM_NULL, "CVLS", "CVodeSetLinearSolver", MSG_LS_CVMEM_NULL);
    return (CVLS_MEM_NULL);
  }
  if (LS == NULL) {
    cvProcessError(NULL, CVLS_ILL_INPUT, "CVLS", "CVodeSetLinearSolver", "LS must be non-NULL");
  ...
  matrixbased = ((LSType != SUNLINEARSOLVER_ITERATIVE) && (LSType != SUNLINEARSOLVER_MATRIX_EMBEDDED));

  /* Test if vector is compatible with LS interface */
  if ((cv_mem->cv_tempv->ops->nvconst == NULL) || (cv_mem->cv_tempv->ops->nvwrmsnorm == NULL)) {
    cvProcessError(cv_mem, CVLS_ILL_INPUT, "CVLS", "CVodeSetLinearSolver", MSG_LS_BAD_NVECTOR);
    return (CVLS_ILL_INPUT);
  }
  ...
  if (iterative) {
    ...
    if (!matrixbased && (LSType != SUNLINEARSOLVER_MATRIX_EMBEDDED) && (LS->ops->setatimes == NULL)) {
      cvProcessError(cv_mem,
                     CVLS_ILL_INPUT,
                     "CVLS",
                     "CVodeSetLinearSolver",
  ...
  if (cvls_mem == NULL) {
    cvProcessError(cv_mem, CVLS_MEM_FAIL, "CVLS", "CVodeSetLinearSolver", MSG_LS_MEM_FAIL);
...
int CVodeSetDeltaGammaMaxBadJac(void *cvode_mem, realtype dgmax_jbad)
{
  ...
  /* Access CVLsMem structure */
  retval = cvLs_AccessLMem(cvode_mem, "CVodeSetDeltaGammaMaxBadJac", &cv_mem, &cvls_mem);
  if (retval != CVLS_SUCCESS)
    return (retval);
...
int CVodeSetEpsLin(void *cvode_mem, realtype eplifac)
{
  ...
  if (eplifac < ZERO) {
    cvProcessError(cv_mem, CVLS_ILL_INPUT, "CVLS", "CVodeSetEpsLin", MSG_LS_BAD_EPLIN);
    return (CVLS_ILL_INPUT);
  }
...
int CVodeSetJacEvalFrequency(void *cvode_mem, long int msbj)
{
  ...
  retval = cvLs_AccessLMem(cvode_mem, "CVodeSetJacEvalFrequency", &cv_mem, &cvls_mem);
  if (retval != CVLS_SUCCESS)
    return (retval);
...
int CVodeSetLinearSolutionScaling(void *cvode_mem, booleantype onoff)
{
  ...
  /* access CVLsMem structure; store input and return */
  retval = cvLs_AccessLMem(cvode_mem, "CVodeSetLinearSolutionScaling", &cv_mem, &cvls_mem);
  if (retval != CVLS_SUCCESS)
    return (retval);
...
int CVodeSetPreconditioner(void *cvode_mem, CVLsPrecSetupFn psetup, CVLsPrecSolveFn psolve)
{
  ...
  /* access CVLsMem structure */
  retval = cvLs_AccessLMem(cvode_mem, "CVodeSetPreconditioner", &cv_mem, &cvls_mem);
  if (retval != CVLS_SUCCESS)
    return (retval);
  ...
  retval = SUNLinSolSetPreconditioner(cvls_mem->LS, cv_mem, cvls_psetup, cvls_psolve);
  if (retval != SUNLS_SUCCESS) {
    cvProcessError(cv_mem,
                   CVLS_SUNLS_FAIL,
                   "CVLS",
...
int CVodeSetJacTimesRhsFn(void *cvode_mem, CVRhsFn jtimesRhsFn)
{
  ...
  /* access CVLsMem structure */
  retval = cvLs_AccessLMem(cvode_mem, "CVodeSetJacTimesRhsFn", &cv_mem, &cvls_mem);
  if (retval != CVLS_SUCCESS)
    return (retval);
...
int CVodeGetNumLinRhsEvals(void *cvode_mem, long int *nfevalsLS)
{
  ...
  /* access CVLsMem structure; set output value and return */
  retval = cvLs_AccessLMem(cvode_mem, "CVodeGetNumLinRhsEvals", &cv_mem, &cvls_mem);
  if (retval != CVLS_SUCCESS)
    return (retval);
...
int CVodeGetNumPrecSolves(void *cvode_mem, long int *npsolves)
{
  ...
  /* access CVLsMem structure; set output value and return */
  retval = cvLs_AccessLMem(cvode_mem, "CVodeGetNumPrecSolves", &cv_mem, &cvls_mem);
  if (retval != CVLS_SUCCESS)
    return (retval);
...
int CVodeGetNumLinConvFails(void *cvode_mem, long int *nlcfails)
{
  ...
  /* access CVLsMem structure; set output value and return */
  retval = cvLs_AccessLMem(cvode_mem, "CVodeGetNumLinConvFails", &cv_mem, &cvls_mem);
  if (retval != CVLS_SUCCESS)
    return (retval);
...
int CVodeGetNumJTSetupEvals(void *cvode_mem, long int *njtsetups)
{
  ...
  /* access CVLsMem structure; set output value and return */
  retval = cvLs_AccessLMem(cvode_mem, "CVodeGetNumJTSetupEvals", &cv_mem, &cvls_mem);
  if (retval != CVLS_SUCCESS)
    return (retval);
...
int CVodeGetNumJtimesEvals(void *cvode_mem, long int *njvevals)
{
  ...
  /* access CVLsMem structure; set output value and return */
  retval = cvLs_AccessLMem(cvode_mem, "CVodeGetNumJtimesEvals", &cv_mem, &cvls_mem);
  if (retval != CVLS_SUCCESS)
    return (retval);
...
int CVodeGetLinSolveStats(void *cvode_mem,
                          long int *njevals,
                          long int *nfevalsLS,
                          long int *nliters,
                          long int *nlcfails,
                          ...
                          long int *njtimes)
{
  ...
  /* access CVLsMem structure; set output value and return */
  retval = cvLs_AccessLMem(cvode_mem, "CVodeGetLinSolveStats", &cv_mem, &cvls_mem);
  if (retval != CVLS_SUCCESS)
    return (retval);
...
int cvLsATimes(void *cvode_mem, N_Vector v, N_Vector z)
{
  ...
  retval =
      cvls_mem
          ->jtimes(v, z, cv_mem->cv_tn, cvls_mem->ycur, cvls_mem->fcur, cvls_mem->jt_data, cvls_mem->ytemp);
  cvls_mem->njtimes++;
  if (retval != 0)
    return (retval);
...
int cvLsPSetup(void *cvode_mem)
{
  ...
  retval = cvls_mem->pset(cv_mem->cv_tn,
                          cvls_mem->ycur,
                          cvls_mem->fcur,
                          !(cvls_mem->jbad),
                          &cv_mem->cv_jcur,
...
int cvLsPSolve(void *cvode_mem, N_Vector r, N_Vector z, realtype tol, int lr)
{
  ...
  retval = cvls_mem->psolve(cv_mem->cv_tn,
                            cvls_mem->ycur,
                            cvls_mem->fcur,
                            r,
                            z,
              ...
              N_Vector tmp3)
{
  ...
  if (cv_mem->cv_tempv->ops->nvcloneempty == NULL || cv_mem->cv_tempv->ops->nvwrmsnorm == NULL ||
      cv_mem->cv_tempv->ops->nvlinearsum == NULL || cv_mem->cv_tempv->ops->nvdestroy == NULL ||
      cv_mem->cv_tempv->ops->nvscale == NULL || cv_mem->cv_tempv->ops->nvgetarraypointer == NULL ||
      cv_mem->cv_tempv->ops->nvsetarraypointer == NULL) {
    cvProcessError(cv_mem, CVLS_ILL_INPUT, "CVLS", "cvLsDQJac", MSG_LS_BAD_NVECTOR);
    return (CVLS_ILL_INPUT);
  }
  ...
  if (SUNMatGetID(Jac) == SUNMATRIX_DENSE) {
    ...
    cvProcessError(cv_mem, CVLS_ILL_INPUT, "CVLS", "cvLsDQJac", "unrecognized matrix type for cvLsDQJac");
...
int cvLsDenseDQJac(realtype t, N_Vector y, N_Vector fy, SUNMatrix Jac, CVodeMem cv_mem, N_Vector tmp1)
{
  ...
  minInc = (fnorm != ZERO) ? (MIN_INC_MULT * SUNRabs(cv_mem->cv_h) * cv_mem->cv_uround * N * fnorm) : ONE;
...
/*-----------------------------------------------------------------
  ...
  -----------------------------------------------------------------*/
int cvLsDQJtimes(N_Vector v, N_Vector Jv, realtype t, N_Vector y, N_Vector fy, void *cvode_mem, N_Vector work)
{
  CVodeMem cv_mem;
  CVLsMem cvls_mem;
  realtype sig, siginv;
                      ...
                      N_Vector vtemp3)
{
  ...
  if (jok) {
    ...
    *jcur = SUNFALSE;
    ...
    if (retval) {
      cvProcessError(cv_mem, CVLS_SUNMAT_FAIL, "CVLS", "cvLsSetup", MSG_LS_SUNMAT_FAILED);
    ...
    *jcur = SUNTRUE;
    ...
    if (retval < 0) {
      cvProcessError(cv_mem, CVLS_JACFUNC_UNRECVR, "CVLS", "cvLsSetup", MSG_LS_JACFUNC_FAILED);
...
int cvLsInitialize(CVodeMem cv_mem)
{
  ...
  if (cv_mem->cv_lmem == NULL) {
    cvProcessError(cv_mem, CVLS_LMEM_NULL, "CVLS", "cvLsInitialize", MSG_LS_LMEM_NULL);
    return (CVLS_LMEM_NULL);
  }
  cvls_mem = (CVLsMem)cv_mem->cv_lmem;
    ...
    if (cvls_mem->user_linsys) {
      ...
      if (cvls_mem->jacDQ) {
        ...
        if (cvls_mem->A->ops->getid) {
          ...
          if ((SUNMatGetID(cvls_mem->A) == SUNMATRIX_DENSE) || (SUNMatGetID(cvls_mem->A) == SUNMATRIX_BAND)) {
            cvls_mem->jac = cvLsDQJac;
            cvls_mem->J_data = cv_mem;
          }
          else {
      ...
      if (cvls_mem->savedJ == NULL) {
        ...
        if (cvls_mem->savedJ == NULL) {
          cvProcessError(cv_mem, CVLS_MEM_FAIL, "CVLS", "cvLsInitialize", MSG_LS_MEM_FAIL);
              ...
              N_Vector vtemp3)
{
  ...
  cvls_mem->jbad = (cv_mem->cv_nst == 0) || (cv_mem->cv_nst >= cvls_mem->nstlj + cvls_mem->msbj) ||
  ...
  if (cvls_mem->A != NULL) {
    ...
    retval = cvls_mem->linsys(cv_mem->cv_tn,
                              ypred,
                              fpred,
                              cvls_mem->A,
                              !(cvls_mem->jbad),
...
int cvLsSolve(CVodeMem cv_mem, N_Vector b, N_Vector weight, N_Vector ynow, N_Vector fnow)
{
  ...
  if (cvls_mem->jtsetup) {
    cvls_mem->last_flag = cvls_mem->jtsetup(cv_mem->cv_tn, ynow, fnow, cvls_mem->jt_data);
    cvls_mem->njtsetup++;
  ...
  case SUNLS_ATIMES_FAIL_UNREC:
    cvProcessError(cv_mem, SUNLS_ATIMES_FAIL_UNREC, "CVLS", "cvLsSolve", MSG_LS_JTIMES_FAILED);
    return (-1);
    break;
  case SUNLS_PSOLVE_FAIL_UNREC:
    cvProcessError(cv_mem, SUNLS_PSOLVE_FAIL_UNREC, "CVLS", "cvLsSolve", MSG_LS_PSOLVE_FAILED);
    return (-1);
    break;
  }
...
/*---------------------------------------------------------------
  ...
  or CVLS_LMEM_NULL.
  ---------------------------------------------------------------*/
int cvLs_AccessLMem(void *cvode_mem, const char *fname, CVodeMem *cv_mem, CVLsMem *cvls_mem)
{
  if (cvode_mem == NULL) {
