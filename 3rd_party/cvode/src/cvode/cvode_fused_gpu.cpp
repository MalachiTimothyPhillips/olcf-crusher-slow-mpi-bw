...
extern "C" int cvEwtSetSS_fused(const booleantype atolMin0,
                                const realtype reltol,
                                const realtype Sabstol,
                                const N_Vector ycur,
                                N_Vector tempv,
                                N_Vector weight)
{
  ...
  cvEwtSetSS_kernel<<<grid, block, 0, *(exec_policy->stream())>>>(N,
                                                                  reltol,
                                                                  Sabstol,
  ...
  if (!gpuAssert(gpuGetLastError(), __FILE__, __LINE__))
    return -1;
#endif
...
extern "C" int cvEwtSetSV_fused(const booleantype atolMin0,
                                const realtype reltol,
                                ...
                                N_Vector weight)
{
  ...
  cvEwtSetSV_kernel<<<grid, block, 0, *(exec_policy->stream())>>>(N,
                                                                  reltol,
...
extern "C" int cvCheckConstraints_fused(const N_Vector c,
                                        const N_Vector ewt,
                                        const N_Vector y,
                                        const N_Vector mm,
                                        N_Vector tempv)
...
extern "C" int cvNlsResid_fused(const realtype rl1,
                                const realtype ngamma,
                                const N_Vector zn1,
                                const N_Vector ycor,
                                ...
                                N_Vector res)
{
  ...
  cvNlsResid_kernel<<<grid, block, 0, *(exec_policy->stream())>>>(N,
                                                                  rl1,
                                                                  ngamma,
...
extern "C" int cvDiagSetup_formY(const realtype h,
                                 const realtype r,
                                 ...
                                 N_Vector y)
{
  ...
  cvDiagSetup_formY_kernel<<<grid, block, 0, *(exec_policy->stream())>>>(N,
                                                                         h,
                                                                         r,
...
__global__ void cvDiagSetup_buildM_kernel(const sunindextype length,
...
extern "C" int cvDiagSetup_buildM(const realtype fract,
                                  const realtype uround,
                                  const realtype h,
                                  const N_Vector ftemp,
                                  const N_Vector fpred,
...
__global__ void cvDiagSolve_updateM_kernel(const sunindextype length, const realtype r, realtype *M)
{
  static const realtype one = 1.0;
  GRID_STRIDE_XLOOP(sunindextype, i, length)
  {
...
extern "C" int cvDiagSolve_updateM(const realtype r, N_Vector M)
{
  ...
  cvDiagSolve_updateM_kernel<<<grid, block, 0, *(exec_policy->stream())>>>(N, r, N_VGetDeviceArrayPointer(M));
