...
extern int cvEwtSetSS_fused(const booleantype atolmin0,
...
extern int cvEwtSetSV_fused(const booleantype atolmin0,
                            const realtype reltol,
...
static void cvSetTqBDF(CVodeMem cv_mem,
                       realtype hsum,
                       realtype alpha0,
                       realtype alpha0_hat,
                       realtype xi_inv,
...
#ifdef SUNDIALS_BUILD_PACKAGE_FUSED_KERNELS
extern int cvCheckConstraints_fused(const N_Vector c,
                                    const N_Vector ewt,
                                    const N_Vector y,
                                    const N_Vector mm,
                                    N_Vector tempv);
...
int CVodeInit(void *cvode_mem, CVRhsFn f, realtype t0, N_Vector y0)
{
  ...
  if (retval != CV_SUCCESS) {
    cvProcessError(cv_mem, retval, "CVODE", "CVodeInit", "Setting the nonlinear solver failed");
    cvFreeVectors(cv_mem);
...
int CVode(void *cvode_mem, realtype tout, N_Vector yout, realtype *tret, int itask)
{
  ...
  if (cv_mem->cv_nst == 0) {
    ...
    cv_mem->cv_nfe++;
    if (retval < 0) {
      cvProcessError(cv_mem, CV_RHSFUNC_FAIL, "CVODE", "CVode", MSGCV_RHSFUNC_FAILED, cv_mem->cv_tn);
      SUNDIALS_MARK_FUNCTION_END(CV_PROFILER);
    ...
    if (cv_mem->cv_nrtfn > 0) {
      ...
      if (retval == CV_RTFUNC_FAIL) {
        cvProcessError(cv_mem, CV_RTFUNC_FAIL, "CVODE", "cvRcheck1", MSGCV_RTFUNC_FAILED, cv_mem->cv_tn);
        SUNDIALS_MARK_FUNCTION_END(CV_PROFILER);
  ...
  if (cv_mem->cv_nst > 0) {
    ...
    if (cv_mem->cv_nrtfn > 0) {
      ...
      if (retval == CLOSERT) {
        cvProcessError(cv_mem, CV_ILL_INPUT, "CVODE", "cvRcheck2", MSGCV_CLOSE_ROOTS, cv_mem->cv_tlo);
        SUNDIALS_MARK_FUNCTION_END(CV_PROFILER);
        return (CV_ILL_INPUT);
      }
      else if (retval == CV_RTFUNC_FAIL) {
        cvProcessError(cv_mem, CV_RTFUNC_FAIL, "CVODE", "cvRcheck2", MSGCV_RTFUNC_FAILED, cv_mem->cv_tlo);
        SUNDIALS_MARK_FUNCTION_END(CV_PROFILER);
        return (CV_RTFUNC_FAIL);
      }
      ...
      if (SUNRabs(cv_mem->cv_tn - cv_mem->cv_tretlast) > troundoff) {
        ...
        if (retval == CV_SUCCESS) { /* no root found */
          ...
          cvProcessError(cv_mem, CV_RTFUNC_FAIL, "CVODE", "cvRcheck3", MSGCV_RTFUNC_FAILED, cv_mem->cv_tlo);
          SUNDIALS_MARK_FUNCTION_END(CV_PROFILER);
          return (CV_RTFUNC_FAIL);
        }
      }
  ...
  for (;;) {
    ...
    if (cv_mem->cv_nst > 0) {
      ...
      if (ewtsetOK != 0) {
        ...
        if (cv_mem->cv_itol == CV_WF)
          cvProcessError(cv_mem, CV_ILL_INPUT, "CVODE", "CVode", MSGCV_EWT_NOW_FAIL, cv_mem->cv_tn);
        else
          cvProcessError(cv_mem, CV_ILL_INPUT, "CVODE", "CVode", MSGCV_EWT_NOW_BAD, cv_mem->cv_tn);
    ...
    if ((cv_mem->cv_mxstep > 0) && (nstloc >= cv_mem->cv_mxstep)) {
      cvProcessError(cv_mem, CV_TOO_MUCH_WORK, "CVODE", "CVode", MSGCV_MAX_STEPS, cv_mem->cv_tn);
    ...
    if (cv_mem->cv_tolsf > ONE) {
      cvProcessError(cv_mem, CV_TOO_MUCH_ACC, "CVODE", "CVode", MSGCV_TOO_MUCH_ACC, cv_mem->cv_tn);
    ...
    if (cv_mem->cv_tn + cv_mem->cv_h == cv_mem->cv_tn) {
      ...
      if (cv_mem->cv_nhnil <= cv_mem->cv_mxhnil)
        cvProcessError(cv_mem, CV_WARNING, "CVODE", "CVode", MSGCV_HNIL, cv_mem->cv_tn, cv_mem->cv_h);
      if (cv_mem->cv_nhnil == cv_mem->cv_mxhnil)
    ...
    if (cv_mem->cv_nrtfn > 0) {
      ...
      if (cv_mem->cv_nst == 1) {
        ...
        if ((cv_mem->cv_mxgnull > 0) && inactive_roots) {
          cvProcessError(cv_mem, CV_WARNING, "CVODE", "CVode", MSGCV_INACTIVE_ROOTS);
        }
      }
    }
...
static booleantype cvCheckNvector(N_Vector tmpl)
{
  if ((tmpl->ops->nvclone == NULL) || (tmpl->ops->nvdestroy == NULL) || (tmpl->ops->nvlinearsum == NULL) ||
      (tmpl->ops->nvconst == NULL) || (tmpl->ops->nvprod == NULL) || (tmpl->ops->nvdiv == NULL) ||
      (tmpl->ops->nvscale == NULL) || (tmpl->ops->nvabs == NULL) || (tmpl->ops->nvinv == NULL) ||
      (tmpl->ops->nvaddconst == NULL) || (tmpl->ops->nvmaxnorm == NULL) || (tmpl->ops->nvwrmsnorm == NULL))
    return (SUNFALSE);
  else
    return (SUNTRUE);
}
...
static int cvInitialSetup(CVodeMem cv_mem)
{
  ...
  if ((!cv_mem->cv_user_efun) && (cv_mem->cv_atolmin0) && (!cv_mem->cv_tempv->ops->nvmin)) {
    cvProcessError(cv_mem, CV_ILL_INPUT, "CVODE", "cvInitialSetup", "Missing N_VMin routine from N_Vector");
    return (CV_ILL_INPUT);
  }
  ...
  if (cv_mem->proj_enabled && cv_mem->proj_mem == NULL) {
    cvProcessError(cv_mem, CV_PROJ_MEM_NULL, "CVODE", "cvInitialSetup", MSG_CV_PROJ_MEM_NULL);
  ...
  if (cv_mem->proj_mem != NULL) {
    ...
    if (ier != CV_SUCCESS) {
      cvProcessError(cv_mem, CV_MEM_FAIL, "CVODE", "cvInitialSetup", MSGCV_MEM_FAIL);
      return (CV_MEM_FAIL);
    }
    cv_mem->proj_applied = SUNFALSE;
  }
...
static int cvStep(CVodeMem cv_mem)
{
  ...
  if (cv_mem->proj_enabled)
    doProjection =
        ...
        (cv_mem->cv_nst == 0 || (cv_mem->cv_nst >= cv_mem->proj_mem->nstlprj + cv_mem->proj_mem->freq));
  ...
  cv_mem->cv_etamax =
      (cv_mem->cv_nst <= cv_mem->cv_small_nst) ? cv_mem->cv_eta_max_es : cv_mem->cv_eta_max_gs;
...
static void cvAdjustAdams(CVodeMem cv_mem, int deltaq)
{
  ...
  if (cv_mem->cv_q > 2)
    (void)N_VScaleAddMulti(cv_mem->cv_q - 2,
                           cv_mem->cv_cvals,
...
void cvRescale(CVodeMem cv_mem)
{
  ...
  (void)N_VScaleVectorArray(cv_mem->cv_q, cv_mem->cv_cvals, cv_mem->cv_zn + 1, cv_mem->cv_zn + 1);
...
static void cvPredict(CVodeMem cv_mem)
{
  ...
  SUNLogger_QueueMsg(CV_LOGGER, SUN_LOGLEVEL_DEBUG, "CVODE::cvPredict", "return", "predictor =", "");
...
static void cvSet(CVodeMem cv_mem)
{
  ...
  cv_mem->cv_gamrat =
      (cv_mem->cv_nst > 0) ? cv_mem->cv_gamma / cv_mem->cv_gammap : ONE; /* protect x / x != 1.0 */
}
...
static int cvNls(CVodeMem cv_mem, int nflag)
{
  ...
  if (cv_mem->cv_lsetup) {
    cv_mem->convfail = ((nflag == FIRST_CALL) || (nflag == PREV_ERR_FAIL)) ? CV_NO_FAILURES : CV_FAIL_OTHER;

    callSetup = (nflag == PREV_CONV_FAIL) || (nflag == PREV_ERR_FAIL) || (cv_mem->cv_nst == 0) ||
  ...
  flag = SUNNonlinSolSolve(cv_mem->NLS,
                           cv_mem->cv_zn[0],
                           cv_mem->cv_acor,
                           cv_mem->cv_ewt,
                           cv_mem->cv_tq[4],
...
static int cvCheckConstraints(CVodeMem cv_mem)
{
  ...
  if (cv_mem->cv_usefused) {
    cvCheckConstraints_fused(cv_mem->cv_constraints, cv_mem->cv_ewt, cv_mem->cv_y, mm, tmp);
  }
  else
#endif
  {
  ...
  if (vnorm <= cv_mem->cv_tq[4]) {
    N_VLinearSum(ONE, cv_mem->cv_acor, -ONE, tmp, cv_mem->cv_acor); /* acor <- acor - v */
    return (CV_SUCCESS);
  }
...
static int cvHandleNFlag(CVodeMem cv_mem, int *nflagPtr, realtype saved_t, int *ncfPtr)
{
  ...
  if ((SUNRabs(cv_mem->cv_h) <= cv_mem->cv_hmin * ONEPSM) || (*ncfPtr == cv_mem->cv_maxncf)) {
...
static int cvDoErrorTest(CVodeMem cv_mem, int *nflagPtr, realtype saved_t, int *nefPtr, realtype *dsmPtr)
{
  ...
  *nflagPtr = PREV_ERR_FAIL;
  ...
  if ((SUNRabs(cv_mem->cv_h) <= cv_mem->cv_hmin * ONEPSM) || (*nefPtr == cv_mem->cv_maxnef))
  ...
  if (*nefPtr <= MXNEF1) {
    ...
    cv_mem->cv_eta =
        SUNMAX(cv_mem->cv_eta_min_ef, SUNMAX(cv_mem->cv_eta, cv_mem->cv_hmin / SUNRabs(cv_mem->cv_h)));
...
static void cvCompleteStep(CVodeMem cv_mem)
{
  ...
  (void)N_VScaleAddMulti(cv_mem->cv_q + 1, cv_mem->cv_l, cv_mem->cv_acor, cv_mem->cv_zn, cv_mem->cv_zn);
  ...
  if (cv_mem->cv_monitorfun != NULL && !(cv_mem->cv_nst % cv_mem->cv_monitor_interval)) {
...
static int cvHandleFailure(CVodeMem cv_mem, int flag)
{
  ...
  */
  ...
  switch (flag) {
  case CV_ERR_FAILURE:
    cvProcessError(cv_mem, CV_ERR_FAILURE, "CVODE", "CVode", MSGCV_ERR_FAILS, cv_mem->cv_tn, cv_mem->cv_h);
    break;
  case CV_CONV_FAILURE:
    cvProcessError(cv_mem, CV_CONV_FAILURE, "CVODE", "CVode", MSGCV_CONV_FAILS, cv_mem->cv_tn, cv_mem->cv_h);
    break;
  case CV_LSETUP_FAIL:
    cvProcessError(cv_mem, CV_LSETUP_FAIL, "CVODE", "CVode", MSGCV_SETUP_FAILED, cv_mem->cv_tn);
    break;
  case CV_LSOLVE_FAIL:
    cvProcessError(cv_mem, CV_LSOLVE_FAIL, "CVODE", "CVode", MSGCV_SOLVE_FAILED, cv_mem->cv_tn);
    break;
  case CV_RHSFUNC_FAIL:
    cvProcessError(cv_mem, CV_RHSFUNC_FAIL, "CVODE", "CVode", MSGCV_RHSFUNC_FAILED, cv_mem->cv_tn);
    break;
  case CV_UNREC_RHSFUNC_ERR:
    cvProcessError(cv_mem, CV_UNREC_RHSFUNC_ERR, "CVODE", "CVode", MSGCV_RHSFUNC_UNREC, cv_mem->cv_tn);
    break;
  case CV_REPTD_RHSFUNC_ERR:
    cvProcessError(cv_mem, CV_REPTD_RHSFUNC_ERR, "CVODE", "CVode", MSGCV_RHSFUNC_REPTD, cv_mem->cv_tn);
    break;
  case CV_RTFUNC_FAIL:
    cvProcessError(cv_mem, CV_RTFUNC_FAIL, "CVODE", "CVode", MSGCV_RTFUNC_FAILED, cv_mem->cv_tn);
    break;
  case CV_TOO_CLOSE:
  ...
  case SUN_NLS_MEM_NULL:
    cvProcessError(cv_mem, CV_MEM_NULL, "CVODE", "CVode", MSGCV_NLS_INPUT_NULL, cv_mem->cv_tn);
    break;
  case CV_NLS_SETUP_FAIL:
    cvProcessError(cv_mem, CV_NLS_SETUP_FAIL, "CVODE", "CVode", MSGCV_NLS_SETUP_FAILED, cv_mem->cv_tn);
    break;
  case CV_CONSTR_FAIL:
    cvProcessError(cv_mem, CV_CONSTR_FAIL, "CVODE", "CVode", MSGCV_FAILED_CONSTR, cv_mem->cv_tn);
    break;
  case CV_NLS_FAIL:
    cvProcessError(cv_mem, CV_NLS_FAIL, "CVODE", "CVode", MSGCV_NLS_FAIL, cv_mem->cv_tn);
    break;
  case CV_PROJ_MEM_NULL:
    cvProcessError(cv_mem, CV_PROJ_MEM_NULL, "CVODE", "CVode", MSG_CV_PROJ_MEM_NULL);
    break;
  case CV_PROJFUNC_FAIL:
    cvProcessError(cv_mem, CV_PROJFUNC_FAIL, "CVODE", "CVode", MSG_CV_PROJFUNC_FAIL, cv_mem->cv_tn);
    break;
  case CV_REPTD_PROJFUNC_ERR:
    cvProcessError(cv_mem, CV_REPTD_PROJFUNC_ERR, "CVODE", "CVode", MSG_CV_REPTD_PROJFUNC_ERR, cv_mem->cv_tn);
    break;
  default:
    ...
    cvProcessError(cv_mem,
                   ...
                   "CVODE",
                   "CVode",
                   "CVODE encountered an unrecognized error. Please report this to the SUNDIALS developers "
                   "at sundials-users@llnl.gov");
...
static void cvBDFStab(CVodeMem cv_mem)
{
  ...
  if (cv_mem->cv_q >= 3) {
    ...
    for (i = 1; i <= cv_mem->cv_q - 1; i++)
      factorial *= i;
    sq = factorial * cv_mem->cv_q * (cv_mem->cv_q + 1) * cv_mem->cv_acnrm / SUNMAX(cv_mem->cv_tq[5], TINY);
...
static int cvSLdet(CVodeMem cv_mem)
{
  ...
  for (k = 1; k <= 3; k++) {
    ...
    qc[5][k] =
        cv_mem->cv_ssdat[1][k] * cv_mem->cv_ssdat[3][k] - cv_mem->cv_ssdat[2][k] * cv_mem->cv_ssdat[2][k];
    qc[4][k] =
        cv_mem->cv_ssdat[2][k] * cv_mem->cv_ssdat[3][k] - cv_mem->cv_ssdat[1][k] * cv_mem->cv_ssdat[4][k];
    qc[3][k] = ZERO;
    qc[2][k] =
        cv_mem->cv_ssdat[2][k] * cv_mem->cv_ssdat[5][k] - cv_mem->cv_ssdat[3][k] * cv_mem->cv_ssdat[4][k];
    qc[1][k] =
        cv_mem->cv_ssdat[4][k] * cv_mem->cv_ssdat[4][k] - cv_mem->cv_ssdat[3][k] * cv_mem->cv_ssdat[5][k];
...
static int cvRootfind(CVodeMem cv_mem)
{
  ...
  if (!sgnchg) {
    ...
    for (i = 0; i < cv_mem->cv_nrtfn; i++) {
      ...
      if (!cv_mem->cv_gactive[i])
        continue;
      if ((SUNRabs(cv_mem->cv_ghi[i]) == ZERO) && (cv_mem->cv_rootdir[i] * cv_mem->cv_glo[i] <= ZERO))
  ...
  for (i = 0; i < cv_mem->cv_nrtfn; i++) {
    ...
    if ((SUNRabs(cv_mem->cv_ghi[i]) == ZERO) && (cv_mem->cv_rootdir[i] * cv_mem->cv_glo[i] <= ZERO))
      cv_mem->cv_iroots[i] = cv_mem->cv_glo[i] > 0 ? -1 : 1;
    if ((cv_mem->cv_glo[i] * cv_mem->cv_ghi[i] < ZERO) && (cv_mem->cv_rootdir[i] * cv_mem->cv_glo[i] <= ZERO))
      cv_mem->cv_iroots[i] = cv_mem->cv_glo[i] > 0 ? -1 : 1;
  }
  return (RTFOUND);
}
...
static int cvEwtSetSS(CVodeMem cv_mem, N_Vector ycur, N_Vector weight)
{
  ...
  if (cv_mem->cv_usefused) {
    ...
    cvEwtSetSS_fused(cv_mem->cv_atolmin0,
                     cv_mem->cv_reltol,
                     cv_mem->cv_Sabstol,
                     ycur,
                     cv_mem->cv_tempv,
...
static int cvEwtSetSV(CVodeMem cv_mem, N_Vector ycur, N_Vector weight)
{
  ...
  if (cv_mem->cv_usefused) {
    /* We compute weight (inverse of tempv) regardless of the component test
       since it will be thrown away in this case anyways. */
    cvEwtSetSV_fused(cv_mem->cv_atolmin0,
                     cv_mem->cv_reltol,
                     cv_mem->cv_Vabstol,
                     ycur,
                     cv_mem->cv_tempv,
