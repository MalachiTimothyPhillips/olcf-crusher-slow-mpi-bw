...
static int cvAccessProjMem(void *cvode_mem, const char *fname, CVodeMem *cv_mem, CVodeProjMem *proj_mem);
...
int CVodeSetProjFn(void *cvode_mem, CVProjFn pfun)
{
  ...
  if (pfun == NULL) {
    cvProcessError(cv_mem, CV_ILL_INPUT, "CVODE", "CVodeSetProjFn", "The projection function is NULL.");
    return (CV_ILL_INPUT);
  }
  ...
  if (retval != CV_SUCCESS) {
    cvProcessError(cv_mem, CV_MEM_FAIL, "CVODE", "CVodeSetProjFn", MSG_CV_MEM_FAIL);
    return (CV_MEM_FAIL);
  }
...
int CVodeSetProjFrequency(void *cvode_mem, long int freq)
{
  ...
  /* Access memory structures */
  retval = cvAccessProjMem(cvode_mem, "CVodeSetProjFrequency", &cv_mem, &proj_mem);
  if (retval != CV_SUCCESS)
    return (retval);
...
int CVodeSetMaxNumProjFails(void *cvode_mem, int max_fails)
{
  ...
  /* Access memory structures */
  retval = cvAccessProjMem(cvode_mem, "CVodeSetMaxNumProjFails", &cv_mem, &proj_mem);
  if (retval != CV_SUCCESS)
    return (retval);
...
int CVodeGetNumProjEvals(void *cvode_mem, long int *nproj)
{
  ...
  /* Access memory structures */
  retval = cvAccessProjMem(cvode_mem, "CVodeGetNumProjectionEvals", &cv_mem, &proj_mem);
  if (retval != CV_SUCCESS)
    return (retval);
...
int cvDoProjection(CVodeMem cv_mem, int *nflagPtr, realtype saved_t, int *npfailPtr)
{
  int retval;
  N_Vector errP;
  N_Vector acorP;
  ...
  if (cv_mem->proj_mem == NULL) {
    cvProcessError(cv_mem, CV_PROJ_MEM_NULL, "CVODE", "cvDoProjection", MSG_CV_PROJ_MEM_NULL);
  ...
  retval = proj_mem->pfun(cv_mem->cv_tn, cv_mem->cv_y, acorP, proj_mem->eps_proj, errP, cv_mem->cv_user_data);
  proj_mem->nproj++;
...
static int cvAccessProjMem(void *cvode_mem, const char *fname, CVodeMem *cv_mem, CVodeProjMem *proj_mem)
{
  ...
  *cv_mem = (CVodeMem)cvode_mem;
  ...
  /* Access projection memory */
  if ((*cv_mem)->proj_mem == NULL) {
    cvProcessError(*cv_mem, CV_PROJ_MEM_NULL, "CVODE", fname, MSG_CV_PROJ_MEM_NULL);
    return (CV_PROJ_MEM_NULL);
  }
