...
static int AndersonAccelerate(SUNNonlinearSolver NLS, N_Vector gval, N_Vector x, N_Vector xold, int iter);
...
SUNNonlinearSolver SUNNonlinSol_FixedPoint(N_Vector y, int m, SUNContext sunctx)
{
  ...
  if ((y->ops->nvclone == NULL) || (y->ops->nvdestroy == NULL) || (y->ops->nvscale == NULL) ||
      (y->ops->nvlinearsum == NULL) || (y->ops->nvdotprod == NULL))
    return (NULL);
                                 ...
                                 void *mem)
{
  ...
  for (FP_CONTENT(NLS)->curiter = 0; FP_CONTENT(NLS)->curiter < FP_CONTENT(NLS)->maxiters;
    ...
    retval = FP_CONTENT(NLS)->CTest(NLS, ycor, delta, tol, w, FP_CONTENT(NLS)->ctest_data);
