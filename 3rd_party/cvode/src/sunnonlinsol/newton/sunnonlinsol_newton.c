...
SUNNonlinearSolver SUNNonlinSol_Newton(N_Vector y, SUNContext sunctx)
{
  ...
  if ((y->ops->nvclone == NULL) || (y->ops->nvdestroy == NULL) || (y->ops->nvscale == NULL) ||
...
int SUNNonlinSolInitialize_Newton(SUNNonlinearSolver NLS)
{
  ...
  if ((NEWTON_CONTENT(NLS)->Sys == NULL) || (NEWTON_CONTENT(NLS)->LSolve == NULL) ||
                             ...
                             void *mem)
{
  ...
  /* check that all required function pointers have been set */
  if ((NEWTON_CONTENT(NLS)->Sys == NULL) || (NEWTON_CONTENT(NLS)->LSolve == NULL) ||
      (callLSetup && (NEWTON_CONTENT(NLS)->LSetup == NULL)) || (NEWTON_CONTENT(NLS)->CTest == NULL)) {
    return (SUN_NLS_MEM_NULL);
  }
  ...
  for (;;) {
    ...
    if (callLSetup) {
      retval = NEWTON_CONTENT(NLS)->LSetup(jbad, &(NEWTON_CONTENT(NLS)->jcur), mem);
      if (retval != SUN_NLS_SUCCESS)
        break;
    }
    ...
    SUNLogger_QueueMsg(NLS->sunctx->logger,
                       SUN_LOGLEVEL_INFO,
    ...
    for (;;) {
      ...
      retval = NEWTON_CONTENT(NLS)->CTest(NLS, ycor, delta, tol, w, NEWTON_CONTENT(NLS)->ctest_data);
...
int SUNNonlinSolSetConvTestFn_Newton(SUNNonlinearSolver NLS, SUNNonlinSolConvTestFn CTestFn, void *ctest_data)
{
  /* check that the nonlinear solver is non-null */
  if (NLS == NULL)
    return (SUN_NLS_MEM_NULL);
