...
SUNDIALS_STATIC_INLINE
realtype *SUNMatrix_OneMklDense_BlockColumn(SUNMatrix Amat, sunindextype k, sunindextype j)
{
  SUNMatrixContent_OneMklDense A = (SUNMatrixContent_OneMklDense)Amat->content;
  return (((realtype *)A->data->ptr) + k * A->block_rows * A->block_cols + j * A->block_rows);
}
