...
inline void elementWiseDivide(const vector_type &x, const vector_type &y, vector_type &z)
{
...
inline void compare(scalar_type c, const vector_type &x, vector_type &z)
{
  ...
  Kokkos::parallel_for(
      "compare",
      Kokkos::RangePolicy<execution_space>(0, N),
      KOKKOS_LAMBDA(const local_ordinal_type &i) { z_1d(i) = std::abs(x_1d(i)) >= c ? one : zero; });
}
...
inline mag_type normWrms(const vector_type &x, const vector_type &w)
{
  ...
  const local_ordinal_type N = static_cast<local_ordinal_type>(x.getLocalLength());
  const global_ordinal_type Nglob = static_cast<global_ordinal_type>(x.getGlobalLength());
...
inline mag_type normWrmsMask(const vector_type &x, const vector_type &w, const vector_type &id)
{
  const Teuchos::RCP<const Teuchos::Comm<int>> &comm = x.getMap()->getComm();
  const local_ordinal_type N = static_cast<local_ordinal_type>(x.getLocalLength());
  const global_ordinal_type Nglob = static_cast<global_ordinal_type>(x.getGlobalLength());
...
inline bool constraintMask(const vector_type &c, const vector_type &x, vector_type &m)
{
  const Teuchos::RCP<const Teuchos::Comm<int>> &comm = x.getMap()->getComm();
  const local_ordinal_type N = static_cast<local_ordinal_type>(x.getLocalLength());
...
inline scalar_type minQuotient(const vector_type &num, const vector_type &den)
{
  ...
  const local_ordinal_type N = static_cast<local_ordinal_type>(num.getLocalLength());
...
inline scalar_type dotProdLocal(const vector_type &x, const vector_type &y)
{
  ...
  Kokkos::parallel_reduce(
      "dotProdLocal",
      Kokkos::RangePolicy<execution_space>(0, N),
      KOKKOS_LAMBDA(const local_ordinal_type &i, scalar_type &local_sum) { local_sum += x_1d(i) * y_1d(i); },
      sum);
...
inline mag_type L1NormLocal(const vector_type &x)
{
  ...
  Kokkos::parallel_reduce(
      "L1NormLocal",
      Kokkos::RangePolicy<execution_space>(0, N),
      KOKKOS_LAMBDA(const local_ordinal_type &i, mag_type &local_sum) { local_sum += std::abs(x_1d(i)); },
      sum);
...
inline mag_type WSqrSumMaskLocal(const vector_type &x, const vector_type &w, const vector_type &id)
{
  const local_ordinal_type N = static_cast<local_ordinal_type>(x.getLocalLength());
  if (x.need_sync<memory_space>())
    const_cast<vector_type &>(x).sync<memory_space>();
...
inline bool constraintMaskLocal(const vector_type &c, const vector_type &x, vector_type &m)
{
  const local_ordinal_type N = static_cast<local_ordinal_type>(x.getLocalLength());
  if (c.need_sync<memory_space>())
    const_cast<vector_type &>(c).sync<memory_space>();
