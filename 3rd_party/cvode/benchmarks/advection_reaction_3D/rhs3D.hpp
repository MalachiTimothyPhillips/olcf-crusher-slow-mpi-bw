...
static int Advection(realtype t, N_Vector y, N_Vector ydot, void *user_data)
{
  ...
  RAJA::View<realtype, RAJA::Layout<NDIMS + 1>> Yview(GetVecData(y), nxl, nyl, nzl, dof);
  RAJA::View<realtype, RAJA::Layout<NDIMS + 1>> dYview(GetVecData(ydot), nxl, nyl, nzl, dof);

  /* iterate over domain interior, computing advection */
  if (c > 0.0) {
    /* flow moving in the positive x,y,z direction */
    auto range =
        RAJA::make_tuple(RAJA::RangeSegment(1, nxl), RAJA::RangeSegment(1, nyl), RAJA::RangeSegment(1, nzl));
  ...
  if (c > 0.0) {
    if (udata->grid->npx > 1) {
      ...
      RAJA::View<realtype, RAJA::Layout<NDIMS>> Yim1jk(udata->grid->getRecvBuffer("WEST"),
                                                       nyl,
                                                       nzl,
                                                       dof); // Wrecv should have data that was sent from East
      ...
      auto range =
          RAJA::make_tuple(RAJA::RangeSegment(0, 1), RAJA::RangeSegment(0, 1), RAJA::RangeSegment(0, 1));
    ...
    if (udata->grid->npy > 1) {
      RAJA::View<realtype, RAJA::Layout<NDIMS>> Yijm1k(
          udata->grid->getRecvBuffer("SOUTH"),
          nxl,
          nzl,
          dof); // Nrecv should have data that was sent from North
    ...
    if (udata->grid->npz > 1) {
      RAJA::View<realtype, RAJA::Layout<NDIMS>> Yijkm1(udata->grid->getRecvBuffer("FRONT"),
                                                       nxl,
                                                       nyl,
                                                       dof); // Frecv should have data that was sent from Back
    ...
    if (udata->grid->nprocs() != 1) {
      ...
      RAJA::View<realtype, RAJA::Layout<3>> Yip1jk(udata->grid->getRecvBuffer("EAST"), nyl, nzl, dof);
      RAJA::View<realtype, RAJA::Layout<3>> Yijp1k(udata->grid->getRecvBuffer("NORTH"), nxl, nzl, dof);
      RAJA::View<realtype, RAJA::Layout<3>> Yijkp1(udata->grid->getRecvBuffer("BACK"), nxl, nyl, dof);
...
static int AdvectionReaction(realtype t, N_Vector y, N_Vector ydot, void *user_data)
{
  /* access problem data */
  UserData *udata = (UserData *)user_data;
  int retval;
  ...
  if (check_retval((void *)&retval, "Reaction", 1, udata->myid))
    return (-1);
...
static int AdvectionReactionResidual(realtype t, N_Vector y, N_Vector ydot, N_Vector F, void *user_data)
{
  /* access problem data */
  UserData *udata = (UserData *)user_data;
  int retval;
...
/* Solve the linear systems Ax = b where A = I - gamma*dg/dy.
   ...
   dh/dy as dg/dy. */
static int SolveReactionLinSys(N_Vector y,
                               N_Vector x,
                               N_Vector b,
                               realtype gamma,
                               raja_xyz_tuple blocks,
                               UserData *udata)
{
  /* shortcuts */
  int dof, nxl, nyl, nzl;
  ...
  /* create views of the data */
  RAJA::View<realtype, RAJA::Layout<NDIMS + 1>> Yview(GetVecData(y), nxl, nyl, nzl, dof);
  RAJA::View<realtype, RAJA::Layout<NDIMS + 1>> Bview(GetVecData(b), nxl, nyl, nzl, dof);
  RAJA::View<realtype, RAJA::Layout<NDIMS + 1>> Xview(GetVecData(x), nxl, nyl, nzl, dof);

  RAJA::kernel<XYZ_KERNEL_POL>(blocks, [=] DEVICE_FUNC(int i, int j, int k) {
    ...
    realtype scratch_6 = 1.0 / (A0 * scratch_0 - A0 * scratch_3 + A3 * scratch_2 - A3 * scratch_4 +
    ...
    x[0] = scratch_6 * (b[0] * scratch_0 - b[0] * scratch_3 + b[1] * scratch_2 - b[1] * scratch_4 +
                        b[2] * scratch_1 - b[2] * scratch_5);
    x[1] = scratch_6 * (-A0 * A5 * b[2] + A0 * A8 * b[1] + A5 * scratch_8 - A8 * scratch_10 -
                        b[1] * scratch_9 + b[2] * scratch_7);
    x[2] = (-b[2] + scratch_11 * scratch_8 + scratch_13 * (b[1] - scratch_10 * scratch_11)) /
...
static int SolveReactionLinSysRes(N_Vector y,
                                  N_Vector x,
                                  N_Vector b,
                                  realtype gamma,
                                  raja_xyz_tuple blocks,
...
static int PSolveRes(realtype t,
                     N_Vector y,
                     N_Vector ydot,
                     N_Vector F,
                     N_Vector r,
