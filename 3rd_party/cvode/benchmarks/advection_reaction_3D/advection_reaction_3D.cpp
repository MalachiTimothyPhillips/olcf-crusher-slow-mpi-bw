...
int main(int argc, char *argv[])
{
...

  {
    ...
    if (check_retval(&retval, "SetIC", 1, udata.myid))
      MPI_Abort(comm, 1);
    ...
    if (check_retval(&retval, "Evolve", 1, udata.myid))
      MPI_Abort(comm, 1);
...
int ExchangeBCOnly(N_Vector y, UserData *udata)
{
  ...
  if (myid == last) {
    ierr = MPI_Irecv(udata->grid->getRecvBuffer("EAST"),
                     nvar,
                     MPI_SUNREALTYPE,
                     first,
                     MPI_ANY_TAG,
  ...
  if (myid == first) {
    RAJA::forall<EXEC_POLICY>(RAJA::RangeSegment(0, nvar), [=] DEVICE_FUNC(int var) {
  ...
  if (c > 0.0) {
    ...
    udata->grid->ExchangeStart(
        [=](realtype *Wsend, realtype *, realtype *Ssend, realtype *, realtype *, realtype *Fsend) {
          ...
          RAJA::View<realtype, RAJA::Layout<3>> Wview(Wsend,
                                                      udata->grid->nyl,
                                                      udata->grid->nzl,
                                                      udata->grid->dof);
          RAJA::View<realtype, RAJA::Layout<3>> Sview(Ssend,
                                                      udata->grid->nxl,
                                                      udata->grid->nzl,
                                                      udata->grid->dof);
          RAJA::View<realtype, RAJA::Layout<3>> Fview(Fsend,
                                                      udata->grid->nxl,
                                                      udata->grid->nyl,
                                                      udata->grid->dof);
  ...
  if (argc > 1) {
    ...
    for (int i = 1; i < argc; i++) {
      ...
      if (argvi.compare("--help") == 0) {
        ...
        if (uopt->method != "ERK" && uopt->method != "ARK-DIRK" && uopt->method != "ARK-IMEX" &&
            uopt->method != "CV-BDF" && uopt->method != "CV-ADAMS" && uopt->method != "IDA") {
        ...
        if (uopt->nls != "newton" && uopt->nls != "tl-newton" && uopt->nls != "fixedpoint" &&
            uopt->nls != "none") {
                 ...
                 SUNContext ctx)
{
  ...
  if (check_retval((void *)&retval, "ParseArgs", 1, udata->myid))
    return -1;
...
void Gaussian3D(realtype &x, realtype &y, realtype &z, realtype xmax)
{
  ...
  const realtype sigma[3] = {xmax / RCONST(4.0),
                             xmax / RCONST(4.0),
...
int SetIC(N_Vector y, UserData *udata)
{
  ...
  RAJA::View<realtype, RAJA::Layout<NDIMS + 1>> yview(GetVecData(y), nxl, nyl, nzl, udata->grid->dof);
  auto range =
      RAJA::make_tuple(RAJA::RangeSegment(0, nxl), RAJA::RangeSegment(0, nyl), RAJA::RangeSegment(0, nzl));
  RAJA::kernel<XYZ_KERNEL_POL>(range, [=] DEVICE_FUNC(int xi, int yi, int zi) {
...
int WriteOutput(realtype t, N_Vector y, UserData *udata, UserOptions *uopt)
{
  ...
  if (uopt->save) {
    ...
    RAJA::kernel<XYZ_KERNEL_SERIAL_POLICY>(range, [=](int i, int j, int k) {
...
void InputError(char *name)
{
  ...
  if (myid == 0) {
    ...
    fprintf(stderr,
            "  --output-dir              the directory where all output files will be written (default is "
            "the CWD)\n");
    fprintf(stderr, "  --nout <int>              number of output times to print (default is 10)\n");
    fprintf(stderr,
            "  --dont-save               do not save the solution to the filesystem at the nout interval "
            "(default is to save)\n");
    fprintf(stderr, "  --method                  ERK, ARK-DIRK, ARK-IMEX (default), CV-BDF, CV-ADAMS, IDA\n");
    fprintf(stderr,
            "  --fpaccel                 the number of fixed-point acceleration vectors to use (only valid "
            "when using fixedpoint nonlinear solver)\n");
    fprintf(stderr,
            "  --nls                     nonlinear solver to use (newton, tl-newton (task-local newton), "
            "fixedpoint)\n");
    ...
    fprintf(stderr,
            "  --npxyz <int> <int> <int> number of processors in each direction (0 forces MPI to decide)\n");
