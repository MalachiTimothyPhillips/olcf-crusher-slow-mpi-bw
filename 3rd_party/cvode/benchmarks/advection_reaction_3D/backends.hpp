...
using XYZ_KERNEL_SERIAL_POLICY = RAJA::KernelPolicy<RAJA::statement::For<
    ...
    RAJA::seq_exec,
    RAJA::statement::
        For<1, RAJA::seq_exec, RAJA::statement::For<0, RAJA::seq_exec, RAJA::statement::Lambda<0>>>>>;
...
using XYZ_KERNEL_POL = RAJA::KernelPolicy<RAJA::CudaKernel<RAJA::statement::For<
    ...
    RAJA::statement::For<1,
                         ...
                         RAJA::statement::For<0, RAJA::cuda_thread_z_loop, RAJA::statement::Lambda<0>>>>>>;
...
using EXEC_POLICY = RAJA::cuda_exec<256, false>;
using XYZ_KERNEL_POL = RAJA::KernelPolicy<RAJA::statement::CudaKernel<RAJA::statement::For<
    2,
    RAJA::cuda_thread_x_loop,
    RAJA::statement::For<1,
                         RAJA::cuda_thread_y_loop,
...
using XYZ_KERNEL_POL = RAJA::KernelPolicy<RAJA::statement::HipKernel<RAJA::statement::For<
    2,
    RAJA::hip_thread_x_loop,
    RAJA::statement::For<1,
                         RAJA::hip_thread_y_loop,
                         RAJA::statement::For<0, RAJA::hip_thread_z_loop, RAJA::statement::Lambda<0>>>>>>;
...
using XYZ_KERNEL_POL = RAJA::KernelPolicy<RAJA::statement::For<
    ...
    RAJA::loop_exec,
    RAJA::statement::
        For<1, RAJA::loop_exec, RAJA::statement::For<0, RAJA::loop_exec, RAJA::statement::Lambda<0>>>>>;
...
static int EnableFusedVectorOps(N_Vector y)
{
  ...
  if (check_retval(&retval, "N_VEnableFusedOps_Cuda", 1, myid))
    return (-1);
#elif defined(USE_HIP_NVEC)
  retval = N_VEnableFusedOps_Hip(N_VGetLocalVector_MPIPlusX(y), 1);
  if (check_retval(&retval, "N_VEnableFusedOps_Hip", 1, myid))
    return (-1);
#elif defined(USE_RAJA_NVEC)
  retval = N_VEnableFusedOps_Raja(N_VGetLocalVector_MPIPlusX(y), 1);
  if (check_retval(&retval, "N_VEnableFusedOps_Raja", 1, myid))
    return (-1);
#elif defined(USE_OMPDEV_NVEC)
