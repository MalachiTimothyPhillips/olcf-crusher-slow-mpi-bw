...
int EvolveProblemDIRK(N_Vector y, UserData *udata, UserOptions *uopt)
{
  ...
  if (check_retval(&retval, "ARKStepSetOrder", 1, udata->myid))
    return 1;
  ...
  do {
    ...
    if (check_retval(&retval, "ARKStepEvolve", 1, udata->myid))
      break;
...
int EvolveProblemExplicit(N_Vector y, UserData *udata, UserOptions *uopt)
{
  ...
  if (check_retval(&retval, "ERKStepSetOrder", 1, udata->myid))
    return 1;

  /* Attach user data */
  retval = ERKStepSetUserData(arkode_mem, (void *)udata);
  if (check_retval(&retval, "ERKStepSetUserData", 1, udata->myid))
    return 1;
  ...
  do {
    ...
    if (check_retval(&retval, "ERKStepEvolve", 1, udata->myid))
      break;
...
int TaskLocalNlsResidual(N_Vector ycor, N_Vector F, void *arkode_mem)
{
  ...
  ARKStepGetNonlinearSystemData(arkode_mem, &tcur, &zpred, &z, &Fi, &gamma, &sdata, &user_data);
  udata = (UserData *)user_data;

  /* update 'z' value as stored predictor + current corrector */
  N_VLinearSum(1.0, N_VGetLocalVector_MPIPlusX(zpred), 1.0, (ycor), N_VGetLocalVector_MPIPlusX(z));

  /* compute implicit RHS and save for later */
  retval = Reaction(tcur, N_VGetLocalVector_MPIPlusX(z), N_VGetLocalVector_MPIPlusX(Fi), user_data);
                          ...
                          void *mem)
{
  ...
  if ((NLS == NULL) || (y0 == NULL) || (ycor == NULL) || (w == NULL) || (mem == NULL))
    return SUN_NLS_MEM_NULL;
  ...
  if (recover == SUN_NLS_CONV_RECVR)
    GET_NLS_CONTENT(NLS)->ncnf++;
...
int TaskLocalNewton_SetConvTestFn(SUNNonlinearSolver NLS, SUNNonlinSolConvTestFn CTestFn, void *ctest_data)
{
  /* check that the nonlinear solver is non-null */
  if (NLS == NULL)
    return SUN_NLS_MEM_NULL;
