...
int EvolveProblemBDF(N_Vector y, UserData *udata, UserOptions *uopt)
{
  ...
  if (check_retval((void *)cvode_mem, "CVodeInit", 0, udata->myid))
    return 1;

  /* Attach user data */
  retval = CVodeSetUserData(cvode_mem, (void *)udata);
  if (check_retval(&retval, "CVodeSetUserData*", 1, udata->myid))
    return 1;

  /* Specify tolerances */
  retval = CVodeSStolerances(cvode_mem, uopt->rtol, uopt->atol);
  if (check_retval(&retval, "CVodeSStolerances", 1, udata->myid))
    return 1;
  ...
  do {
    ...
    if (check_retval(&retval, "CVode", 1, udata->myid))
      break;
