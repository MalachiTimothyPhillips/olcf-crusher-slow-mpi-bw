...
int SetICDot(N_Vector y, N_Vector yp, UserData *udata)
{
  ...
  if (check_retval(&retval, "AdvectionReaction", 1, udata->myid))
    return 1;
...
int EvolveDAEProblem(N_Vector y, UserData *udata, UserOptions *uopt)
{
  ...
  /* Create ydot' vector */
  yp = N_VClone(y);
  if (check_retval((void *)yp, "N_VClone", 0, udata->myid))
    return 1;

  /* Create IDA */
  ida_mem = IDACreate(udata->ctx);
  if (check_retval((void *)ida_mem, "IDACreate", 0, udata->myid))
    return 1;

  /* Initialize IDA */
  retval = IDAInit(ida_mem, AdvectionReactionResidual, uopt->t0, y, yp);
  if (check_retval(&retval, "IDAInit", 1, udata->myid))
    return 1;

  /* Attach user data */
  retval = IDASetUserData(ida_mem, (void *)udata);
  if (check_retval(&retval, "IDASetUserData*", 1, udata->myid))
    return 1;

  /* Specify tolerances */
  retval = IDASStolerances(ida_mem, uopt->rtol, uopt->atol);
  if (check_retval(&retval, "IDASStolerances", 1, udata->myid))
    return 1;

  /* Increase the max number of steps allowed between outputs */
  retval = IDASetMaxNumSteps(ida_mem, 100000);
  if (check_retval(&retval, "IDASetMaxNumSteps", 1, udata->myid))
    return 1;
  ...
  if (check_retval(&retval, "SetICDot", 1, udata->myid))
    return 1;
  ...
  do {
    ...
    if (check_retval(&retval, "IDA", 1, udata->myid))
      break;
