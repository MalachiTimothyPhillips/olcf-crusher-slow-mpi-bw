...
enum class StencilType { UPWIND };

template <typename REAL, typename GLOBALINT, int NDIMS> class ParallelGrid {
public:
  ...
  // [in] reorder - should MPI_Cart_create do process reordering to optimize or not; defaults to false (some
  // MPI implementations ignore this)
  ParallelGrid(SUNMemoryHelper memhelp,
               MPI_Comm *comm,
               const REAL a[],
               const REAL b[],
               const GLOBALINT npts[],
               int dof,
               BoundaryType bc,
               StencilType st,
               int width = 1,
               const int npxyz[] = nullptr,
               bool reorder = false)
      : nx(1), ny(1), nz(1), nxl(1), nyl(1), nzl(1), npx(1), npy(1), npz(1), dx(0.0), dy(0.0), dz(0.0),
        ax(0.0), ay(0.0), az(0.0), bx(0.0), by(0.0), bz(0.0), dof(dof), dims{0, 0, 0}, coords{0, 0, 0},
        bc(bc), st(st), width(width), memhelp(memhelp)
  {
  ...
  void AllocateBuffersUpwind()
  {
    ...
    SUNMemoryHelper_Alloc(memhelp, &Wrecv_, sizeof(REAL) * dof * width * nyl * nzl, memoryType(), nullptr);
    SUNMemoryHelper_Alloc(memhelp, &Wsend_, sizeof(REAL) * dof * width * nyl * nzl, memoryType(), nullptr);
    ipW = MPI_PROC_NULL;
    ...
    SUNMemoryHelper_Alloc(memhelp, &Erecv_, sizeof(REAL) * dof * width * nyl * nzl, memoryType(), nullptr);
    SUNMemoryHelper_Alloc(memhelp, &Esend_, sizeof(REAL) * dof * width * nyl * nzl, memoryType(), nullptr);
    ipE = MPI_PROC_NULL;
    ...
    if (NDIMS >= 2) {
      SUNMemoryHelper_Alloc(memhelp, &Srecv_, sizeof(REAL) * dof * width * nxl * nzl, memoryType(), nullptr);
      SUNMemoryHelper_Alloc(memhelp, &Ssend_, sizeof(REAL) * dof * width * nxl * nzl, memoryType(), nullptr);
      ipS = MPI_PROC_NULL;
      ...
      SUNMemoryHelper_Alloc(memhelp, &Nrecv_, sizeof(REAL) * dof * width * nxl * nzl, memoryType(), nullptr);
      SUNMemoryHelper_Alloc(memhelp, &Nsend_, sizeof(REAL) * dof * width * nxl * nzl, memoryType(), nullptr);
      ipN = MPI_PROC_NULL;
    ...
    if (NDIMS == 3) {
      SUNMemoryHelper_Alloc(memhelp, &Brecv_, sizeof(REAL) * dof * width * nxl * nyl, memoryType(), nullptr);
      SUNMemoryHelper_Alloc(memhelp, &Bsend_, sizeof(REAL) * dof * width * nxl * nyl, memoryType(), nullptr);
      ipB = MPI_PROC_NULL;
      ...
      SUNMemoryHelper_Alloc(memhelp, &Frecv_, sizeof(REAL) * dof * width * nxl * nyl, memoryType(), nullptr);
      SUNMemoryHelper_Alloc(memhelp, &Fsend_, sizeof(REAL) * dof * width * nxl * nyl, memoryType(), nullptr);
      ipF = MPI_PROC_NULL;
  ...
  int ExchangeStart(std::function<void(REAL *, REAL *, REAL *, REAL *, REAL *, REAL *)> fill)
  {
    ...
    if (ipW != MPI_PROC_NULL) {
      retval = MPI_Irecv(getRecvBuffer("EAST"), dof * nyl * nzl, MPI_SUNREALTYPE, ipW, 1, cart_comm, req);
      assert(retval == MPI_SUCCESS);
    }

    if (ipE != MPI_PROC_NULL) {
      retval = MPI_Irecv(getRecvBuffer("WEST"), dof * nyl * nzl, MPI_SUNREALTYPE, ipE, 0, cart_comm, req + 1);
      assert(retval == MPI_SUCCESS);
    }
    ...
    if (NDIMS >= 2) {
      if (ipS != MPI_PROC_NULL) {
        retval =
            MPI_Irecv(getRecvBuffer("NORTH"), dof * nxl * nzl, MPI_SUNREALTYPE, ipS, 3, cart_comm, req + 2);
        assert(retval == MPI_SUCCESS);
      }

      if (ipN != MPI_PROC_NULL) {
        retval =
            MPI_Irecv(getRecvBuffer("SOUTH"), dof * nxl * nzl, MPI_SUNREALTYPE, ipN, 2, cart_comm, req + 3);
        assert(retval == MPI_SUCCESS);
      }
    }
    ...
    if (NDIMS >= 3) {
      if (ipB != MPI_PROC_NULL) {
        retval =
            MPI_Irecv(getRecvBuffer("FRONT"), dof * nxl * nyl, MPI_SUNREALTYPE, ipB, 5, cart_comm, req + 4);
        assert(retval == MPI_SUCCESS);
      }

      if (ipF != MPI_PROC_NULL) {
        retval =
            MPI_Irecv(getRecvBuffer("BACK"), dof * nxl * nyl, MPI_SUNREALTYPE, ipF, 4, cart_comm, req + 5);
        assert(retval == MPI_SUCCESS);
      }
    }
    ...
    if (ipW != MPI_PROC_NULL) {
      retval = MPI_Isend(getSendBuffer("EAST"), dof * nyl * nzl, MPI_SUNREALTYPE, ipW, 0, cart_comm, req + 6);
      assert(retval == MPI_SUCCESS);
    }

    if (ipE != MPI_PROC_NULL) {
      retval = MPI_Isend(getSendBuffer("WEST"), dof * nyl * nzl, MPI_SUNREALTYPE, ipE, 1, cart_comm, req + 7);
      assert(retval == MPI_SUCCESS);
    }
    ...
    if (NDIMS >= 2) {
      if (ipS != MPI_PROC_NULL) {
        retval =
            MPI_Isend(getSendBuffer("NORTH"), dof * nxl * nzl, MPI_SUNREALTYPE, ipS, 2, cart_comm, req + 8);
        assert(retval == MPI_SUCCESS);
      }

      if (ipN != MPI_PROC_NULL) {
        retval =
            MPI_Isend(getSendBuffer("SOUTH"), dof * nxl * nzl, MPI_SUNREALTYPE, ipN, 3, cart_comm, req + 9);
        assert(retval == MPI_SUCCESS);
      }
    }
    ...
    if (NDIMS == 3) {
      if (ipB != MPI_PROC_NULL) {
        retval =
            MPI_Isend(getSendBuffer("FRONT"), dof * nxl * nyl, MPI_SUNREALTYPE, ipB, 4, cart_comm, req + 10);
        assert(retval == MPI_SUCCESS);
      }

      if (ipF != MPI_PROC_NULL) {
        retval =
            MPI_Isend(getSendBuffer("BACK"), dof * nxl * nyl, MPI_SUNREALTYPE, ipF, 5, cart_comm, req + 11);
        assert(retval == MPI_SUCCESS);
      }
    }
...
private:
  ...
  {
    SUNMemory test;
    if (!SUNMemoryHelper_Alloc(memhelp, &test, sizeof(REAL), SUNMEMTYPE_PINNED, nullptr)) {
      SUNMemoryHelper_Dealloc(memhelp, test, nullptr);
      return (SUNMEMTYPE_PINNED);
    }
    if (!SUNMemoryHelper_Alloc(memhelp, &test, sizeof(REAL), SUNMEMTYPE_DEVICE, nullptr)) {
      SUNMemoryHelper_Dealloc(memhelp, test, nullptr);
      return (SUNMEMTYPE_DEVICE);
    }
    if (!SUNMemoryHelper_Alloc(memhelp, &test, sizeof(REAL), SUNMEMTYPE_UVM, nullptr)) {
      SUNMemoryHelper_Dealloc(memhelp, test, nullptr);
