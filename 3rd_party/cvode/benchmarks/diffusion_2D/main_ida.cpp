...
int main(int argc, char *argv[])
{
  ...
  if (check_flag(&flag, "SUNContextCreate", 1))
    return 1;

  flag = SUNContext_GetProfiler(ctx, &prof);
  if (check_flag(&flag, "SUNContext_GetProfiler", 1))
    return 1;
  ...
  // Add scope so objects are destroyed before MPI_Finalize
  {
    ...
    if (check_flag(&flag, "UserData::parse_args", 1))
      return 1;

    flag = uopts.parse_args(args, outproc);
    if (check_flag(&flag, "UserOptions::parse_args", 1))
      return 1;

    flag = uout.parse_args(args, outproc);
    if (check_flag(&flag, "UserOutput::parse_args", 1))
      return 1;
    ...
    flag = udata.setup();
    if (check_flag(&flag, "UserData::setup", 1))
      return 1;
...
#if defined(USE_HIP)
    N_Vector u = N_VMake_MPIPlusX(udata.comm_c, N_VNew_Hip(udata.nodes_loc, ctx), ctx);
    if (check_flag((void *)u, "N_VMake_MPIPlusX", 0))
      return 1;
#elif defined(USE_CUDA)
    N_Vector u = N_VMake_MPIPlusX(udata.comm_c, N_VNew_Cuda(udata.nodes_loc, ctx), ctx);
    if (check_flag((void *)u, "N_VMake_MPIPlusX", 0))
      return 1;
#else
    ...
    if (check_flag((void *)up, "N_VClone", 0))
      return 1;
    ...
    if (check_flag(&flag, "SolutionDerivative", 1))
      return 1;

    // Create vector for error
    if (udata.forcing) {
      uout.error = N_VClone(u);
      if (check_flag((void *)(uout.error), "N_VClone", 0))
        return 1;
    }
    ...
    if (uopts.prec) {
      ...
      if (check_flag((void *)(udata.diag), "N_VClone", 0))
        return 1;
    }
    ...
    if (check_flag(&flag, "IDASetLinearSolver", 1))
      return 1;

    if (uopts.prec) {
      // Attach preconditioner
      flag = IDASetPreconditioner(ida_mem, PSetup, PSolve);
      if (check_flag(&flag, "IDASetPreconditioner", 1))
        return 1;
    }

    // Set linear solver tolerance factor
    flag = IDASetEpsLin(ida_mem, uopts.epslin);
    if (check_flag(&flag, "IDASetEpsLin", 1))
      return 1;
    ...
    if (check_flag(&flag, "UserOutput::open", 1))
      return 1;

    flag = uout.write(t, u, &udata);
    if (check_flag(&flag, "UserOutput::write", 1))
      return 1;
    ...
    if (check_flag(&flag, "UserOutput::close", 1))
      return 1;
...
static int OutputStats(void *ida_mem, UserData *udata)
{
  ...
  if (check_flag(&flag, "IDAGetNumSteps", 1))
    return -1;
  flag = IDAGetNumErrTestFails(ida_mem, &netf);
  if (check_flag(&flag, "IDAGetNumErrTestFails", 1))
    return -1;
  flag = IDAGetNumResEvals(ida_mem, &nf);
  if (check_flag(&flag, "IDAGetNumResEvals", 1))
    return -1;
  flag = IDAGetNumNonlinSolvIters(ida_mem, &nni);
  if (check_flag(&flag, "IDAGetNumNonlinSolvIters", 1))
    return -1;
  flag = IDAGetNumNonlinSolvConvFails(ida_mem, &ncfn);
  if (check_flag(&flag, "IDAGetNumNonlinSolvConvFails", 1))
    return -1;
  flag = IDAGetNumLinIters(ida_mem, &nli);
  if (check_flag(&flag, "IDAGetNumLinIters", 1))
    return -1;
  flag = IDAGetNumLinConvFails(ida_mem, &nlcf);
  if (check_flag(&flag, "IDAGetNumLinConvFails", 1))
    return -1;
  flag = IDAGetNumLinSolvSetups(ida_mem, &nsetups);
  if (check_flag(&flag, "IDAGetNumLinSolvSetups", 1))
    return -1;
  flag = IDAGetNumLinResEvals(ida_mem, &nf_ls);
  if (check_flag(&flag, "IDAGetNumLinResEvals", 1))
    return -1;
  flag = IDAGetNumJtimesEvals(ida_mem, &nJv);
  if (check_flag(&flag, "IDAGetNumJtimesEvals", 1))
    return -1;
  ...
  if (check_flag(&flag, "IDAGetNumPrecEvals", 1))
    return -1;
  flag = IDAGetNumPrecSolves(ida_mem, &nps);
  if (check_flag(&flag, "IDAGetNumPrecSolves", 1))
    return -1;
