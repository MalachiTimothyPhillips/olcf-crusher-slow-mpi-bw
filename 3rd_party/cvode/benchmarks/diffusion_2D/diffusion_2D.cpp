...
int UserData::start_exchange(const N_Vector u)
{
  ...
  if (HaveNbrW) {
    flag = MPI_Irecv(Wrecv, (int)ny_loc, MPI_SUNREALTYPE, ipW, MPI_ANY_TAG, comm_c, &reqRW);
    if (flag != MPI_SUCCESS) {
  ...
  if (HaveNbrE) {
    flag = MPI_Irecv(Erecv, (int)ny_loc, MPI_SUNREALTYPE, ipE, MPI_ANY_TAG, comm_c, &reqRE);
    if (flag != MPI_SUCCESS) {
      cerr << "Error in MPI_Irecv = " << flag << endl;
      return -1;
    }
  ...
  if (HaveNbrS) {
    flag = MPI_Irecv(Srecv, (int)nx_loc, MPI_SUNREALTYPE, ipS, MPI_ANY_TAG, comm_c, &reqRS);
    if (flag != MPI_SUCCESS) {
      cerr << "Error in MPI_Irecv = " << flag << endl;
      return -1;
    }
  ...
  if (HaveNbrN) {
    flag = MPI_Irecv(Nrecv, (int)nx_loc, MPI_SUNREALTYPE, ipN, MPI_ANY_TAG, comm_c, &reqRN);
    if (flag != MPI_SUCCESS) {
      cerr << "Error in MPI_Irecv = " << flag << endl;
      return -1;
    }
...
int UserOutput::open(UserData *udata)
{
  ...
  if (output == 2) {
    ...
    stringstream fname;
    fname << "diffusion_2d_solution." << setfill('0') << setw(5) << udata->myid_c << ".txt";
    ...
    if (error) {
      fname.str("");
      fname.clear();
      fname << "diffusion_2d_error." << setfill('0') << setw(5) << udata->myid_c << ".txt";
...
int UserOutput::write(realtype t, N_Vector u, UserData *udata)
{
  ...
  if (output > 0) {
    ...
    if (output == 2) {
      ...
      if (check_flag(&flag, "CopyDataFromDevice", 1))
        return -1;
