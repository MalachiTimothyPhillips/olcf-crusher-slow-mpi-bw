...
int main(int argc, char *argv[])
{
  ...
  // Add scope so objects are destroyed before MPI_Finalize
  {
    ...
    if (uopts.prec) {
      ...
      if (check_flag(&flag, "ARKStepSetLSetupFrequency", 1))
        return 1;
    }
    ...
    if (check_flag(&flag, "ARKStepSetOrder", 1))
      return 1;

    // Set fixed step size or adaptivity method
    if (uopts.hfixed > ZERO) {
      flag = ARKStepSetFixedStep(arkode_mem, uopts.hfixed);
      if (check_flag(&flag, "ARKStepSetFixedStep", 1))
        return 1;
    }
    else {
      flag = ARKStepSetAdaptivityMethod(arkode_mem, uopts.controller, SUNTRUE, SUNFALSE, NULL);
    ...
    if (diagfp) {
      ...
      if (check_flag(&flag, "ARKStepSetDiagnostics", 1))
        return 1;
    }
...
static int OutputStats(void *arkode_mem, UserData *udata)
{
  ...
  if (check_flag(&flag, "ARKStepGetNumSteps", 1))
    return -1;
  flag = ARKStepGetNumStepAttempts(arkode_mem, &nst_a);
  if (check_flag(&flag, "ARKStepGetNumStepAttempts", 1))
    return -1;
  flag = ARKStepGetNumErrTestFails(arkode_mem, &netf);
  if (check_flag(&flag, "ARKStepGetNumErrTestFails", 1))
    return -1;
  ...
  if (check_flag(&flag, "ARKStepGetNumNonlinSolvConvFails", 1))
    return -1;
  ...
  if (check_flag(&flag, "ARKStepGetNumLinConvFails", 1))
    return -1;
  flag = ARKStepGetNumLinSolvSetups(arkode_mem, &nsetups);
  if (check_flag(&flag, "ARKStepGetNumLinSolvSetups", 1))
    return -1;
  flag = ARKStepGetNumLinRhsEvals(arkode_mem, &nfi_ls);
  if (check_flag(&flag, "ARKStepGetNumLinRhsEvals", 1))
    return -1;
  flag = ARKStepGetNumJtimesEvals(arkode_mem, &nJv);
  if (check_flag(&flag, "ARKStepGetNumJtimesEvals", 1))
    return -1;
  ...
  if (check_flag(&flag, "ARKStepGetNumPrecEvals", 1))
    return -1;
  flag = ARKStepGetNumPrecSolves(arkode_mem, &nps);
  if (check_flag(&flag, "ARKStepGetNumPrecSolves", 1))
    return -1;
