...
__global__ void solution_kernel(const realtype t,
                                realtype *u,
                                const sunindextype is,
                                const sunindextype ie,
                                const sunindextype js,
                                const sunindextype je,
                                const sunindextype nx,
                                const sunindextype ny,
                                ...
                                const realtype dy)
{
  ...
  if (i < nx_loc && j < ny_loc) {
    if ((is == 0 && i == 0) || (ie == nx - 1 && i == nx_loc - 1) || (js == 0 && j == 0) ||
        (je == ny - 1 && j == ny_loc - 1)) {
      u[i + j * nx_loc] = ZERO;
    }
    else {
...
__global__ void solution_p_kernel(const realtype t,
                                  realtype *up,
                                  const sunindextype is,
                                  const sunindextype ie,
                                  const sunindextype js,
...
int Solution(realtype t, N_Vector u, UserData *udata)
{
  ...
  if (check_flag((void *)uarray, "N_VGetDeviceArrayPointer", 0))
    return -1;

  dim3 block(BLOCK_SIZE_X, BLOCK_SIZE_Y);
  dim3 grid(ICEIL(nx_loc, BLOCK_SIZE_X), ICEIL(ny_loc, BLOCK_SIZE_Y));

  solution_kernel<<<grid, block>>>(t, uarray, is, ie, js, je, nx, ny, nx_loc, ny_loc, dx, dy);
...
int SolutionDerivative(realtype t, N_Vector up, UserData *udata)
{
  ...
  if (check_flag((void *)uparray, "N_VGetDeviceArrayPointer", 0))
    return -1;

  dim3 block(BLOCK_SIZE_X, BLOCK_SIZE_Y);
  dim3 grid(ICEIL(nx_loc, BLOCK_SIZE_X), ICEIL(ny_loc, BLOCK_SIZE_Y));

  solution_p_kernel<<<grid, block>>>(t, uparray, is, ie, js, je, nx, ny, nx_loc, ny_loc, dx, dy);
