...
__global__ static void f_kernel(realtype t, realtype *y, realtype *ydot, int neq, int ngroups);
...
__global__ static void j_kernel(int ngroups, realtype *ydata, realtype *Jdata);
...
int main(int argc, char *argv[])
{
  ...
  memhelper = HIP_OR_CUDA(SUNMemoryHelper_Hip(sunctx);, SUNMemoryHelper_Cuda(sunctx);)
  ...
  y = HIP_OR_CUDA(N_VNew_Hip(neq, sunctx);
                  , N_VNew_Cuda(neq, sunctx);) if (check_retval((void *)y, "N_VNew", 0)) return (1);
  abstol = N_VClone(y);
  ...
  A = SUNMatrix_MagmaDenseBlock(ngroups, GROUPSIZE, GROUPSIZE, SUNMEMTYPE_DEVICE, memhelper, NULL, sunctx);
  if (check_retval((void *)A, "SUNMatrix_MagmaDenseBlock", 0))
    return (1);

  /* Create the SUNLinearSolver object for use by CVode */
  LS = SUNLinSol_MagmaDense(y, A, sunctx);
  if (check_retval((void *)LS, "SUNLinSol_MagmaDense", 0))
    return (1);
  ...
  while (1) {
    ...
    for (groupj = 0; groupj < ngroups; groupj += 10) {
      ...
      PrintOutput(t, ydata[GROUPSIZE * groupj], ydata[1 + GROUPSIZE * groupj], ydata[2 + GROUPSIZE * groupj]);
    }
...
static int f(realtype t, N_Vector y, N_Vector ydot, void *user_data)
{
  ...
  HIP_OR_CUDA(hipError_t cuerr = hipGetLastError();, cudaError_t cuerr = cudaGetLastError();)
...
__global__ static void f_kernel(realtype t, realtype *ydata, realtype *ydotdata, int neq, int ngroups)
{
...
__global__ static void j_kernel(int ngroups, realtype *ydata, realtype *Jdata)
{
  ...
  for (groupj = blockIdx.x * blockDim.x + threadIdx.x; groupj < ngroups; groupj += blockDim.x * gridDim.x) {
    /* get y values */
    y2 = ydata[N * groupj + 1];
    y3 = ydata[N * groupj + 2];
