...
int HypreLS_Solve(SUNLinearSolver S, SUNMatrix A, N_Vector x, N_Vector b, realtype tol);
...
int main(int argc, char *argv[])
{
  ...
  // prior to the MPI_Finalize() call.
  {
    ...
    if (check_flag((void *)A, "Hypre5ptMatrix", 0))
      return 1;

    // Create linear solver
    LS = HypreLS(A, udata);
    if (check_flag((void *)LS, "HypreLS", 0))
      return 1;
...
static int OutputStats(void *cvode_mem, UserData *udata)
{
  ...
  if (check_flag(&flag, "CVodeGetNumJacEvals", 1))
    return -1;
...
static int OutputTiming(UserData *udata)
{
  ...
  MPI_Reduce(&(udata->setuptime), &maxtime, 1, MPI_DOUBLE, MPI_MAX, 0, udata->comm_c);
  if (outproc) {
    cout << "  LS setup time = " << maxtime << " sec" << endl;
  }

  MPI_Reduce(&(udata->solvetime), &maxtime, 1, MPI_DOUBLE, MPI_MAX, 0, udata->comm_c);
  if (outproc) {
...
SUNMatrix Hypre5ptMatrix(UserData *udata)
{
  ...
  flag = HYPRE_StructGridSetExtents(content->grid, content->ilower, content->iupper);
  if (flag != 0) {
    SUNMatDestroy(A);
    return NULL;
  }
  ...
  flag = HYPRE_StructMatrixCreate(udata->comm_c, content->grid, content->stencil, &(content->matrix));
  if (flag != 0) {
    SUNMatDestroy(A);
    return NULL;
  }
...
SUNMatrix Hypre5ptMatrix_Clone(SUNMatrix A) { return (Hypre5ptMatrix(H5PM_UDATA(A))); }
...
void Hypre5ptMatrix_Destroy(SUNMatrix A)
{
  ...
  if (A->content != NULL) {
    if (H5PM_WORK(A))
      delete[] H5PM_WORK(A);
    if (H5PM_MATRIX(A))
      HYPRE_StructMatrixDestroy(H5PM_MATRIX(A));
    if (H5PM_GRID(A))
      HYPRE_StructGridDestroy(H5PM_GRID(A));
    if (H5PM_STENCIL(A))
      HYPRE_StructStencilDestroy(H5PM_STENCIL(A));
...
SUNLinearSolver_Type HypreLS_GetType(SUNLinearSolver LS) { return (SUNLINEARSOLVER_MATRIX_ITERATIVE); }
...
int HypreLS_Setup(SUNLinearSolver LS, SUNMatrix A)
{
  ...
  if (udata->prec) {
    ...
    if (HLS_PRECOND(LS))
      HYPRE_StructPFMGDestroy(HLS_PRECOND(LS));
  ...
  // Set up the solver
  if (HLS_PCG(LS)) {
    flag = HYPRE_StructPCGSetup(HLS_SOLVER(LS), H5PM_MATRIX(A), HLS_B(LS), HLS_X(LS));
  }
  else {
    flag = HYPRE_StructGMRESSetup(HLS_SOLVER(LS), H5PM_MATRIX(A), HLS_B(LS), HLS_X(LS));
  }
  if (flag != 0)
    return (flag);
...
int HypreLS_Solve(SUNLinearSolver LS, SUNMatrix A, N_Vector x, N_Vector b, realtype tol)
{
  int flag;
  ...
  flag = HYPRE_StructVectorSetBoxValues(HLS_B(LS), H5PM_ILOWER(A), H5PM_IUPPER(A), N_VGetArrayPointer(b));
  if (flag != 0)
    return -1;
  ...
  flag = HYPRE_StructVectorSetBoxValues(HLS_X(LS), H5PM_ILOWER(A), H5PM_IUPPER(A), N_VGetArrayPointer(x));
  if (flag != 0)
    return -1;
  ...
  if (HLS_PCG(LS)) {
    ...
    flag = HYPRE_StructPCGSolve(HLS_SOLVER(LS), H5PM_MATRIX(A), HLS_B(LS), HLS_X(LS));
  }
  else {
    // Relative tolerance
    ...
    // Solve the linear system
    flag = HYPRE_StructGMRESSolve(HLS_SOLVER(LS), H5PM_MATRIX(A), HLS_B(LS), HLS_X(LS));
  }
  ...
  flag = HYPRE_StructVectorGetBoxValues(HLS_X(LS), H5PM_ILOWER(A), H5PM_IUPPER(A), N_VGetArrayPointer(x));
  if (flag != 0)
    return -1;
...
int HypreLS_Free(SUNLinearSolver LS)
{
  ...
  if (LS->content != NULL) {
    ...
    if (HLS_PRECOND(LS))
      HYPRE_StructPFMGDestroy(HLS_PRECOND(LS));
    if (HLS_B(LS))
      HYPRE_StructVectorDestroy(HLS_B(LS));
    if (HLS_X(LS))
      HYPRE_StructVectorDestroy(HLS_X(LS));
