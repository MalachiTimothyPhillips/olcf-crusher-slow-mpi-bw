...
static int
JTimes(N_Vector v, N_Vector Jv, realtype t, N_Vector y, N_Vector fy, void *user_data, N_Vector tmp);

// Preconditioner setup and solve functions
static int PSetup(realtype t,
                  N_Vector u,
                  N_Vector f,
                  booleantype jok,
                  booleantype *jcurPtr,
...
int main(int argc, char *argv[])
{
  ...
  if (check_flag(&flag, "MPI_Init", 1))
    return 1;

  flag = MPI_Comm_rank(comm_w, &myid);
  if (check_flag(&flag, "MPI_Comm_rank", 1))
    return 1;
  ...
  // prior to the MPI_Finalize() call.
  {
    ...
    if (check_flag(&flag, "SetupDecomp", 1))
      return 1;
    ...
    if (outproc) {
      ...
      if (udata->lsinfo) {
        ...
        if (check_flag((void *)diagfp, "fopen", 0))
          return 1;
      }
    }
    ...
    if (check_flag((void *)u, "N_VNew_Parallel", 0))
      return 1;
    ...
    if (udata->pcg) {
      ...
      if (udata->lsinfo && outproc) {
        ...
        if (check_flag(&flag, "SUNLinSolSetPrintLevel_PCG", 1))
          return (1);

        flag = SUNLinSolSetInfoFile_PCG(LS, diagfp);
        if (check_flag(&flag, "SUNLinSolSetInfoFile_PCG", 1))
          return (1);
      }
    }
    else {
      ...
      if (udata->lsinfo && outproc) {
        ...
        if (check_flag(&flag, "SUNLinSolSetPrintLevel_SPGMR", 1))
          return (1);

        flag = SUNLinSolSetInfoFile_SPGMR(LS, diagfp);
        if (check_flag(&flag, "SUNLinSolSetInfoFile_SPGMR", 1))
          return (1);
      }
    }
    ...
    if (udata->prec || udata->matvec) {
      ...
      if (check_flag(&flag, "SetupHypre", 1))
        return 1;
    }
    ...
    if (udata->prec) {
      ...
      if (check_flag(&flag, "CVodeSetLSetupFrequency", 1))
        return 1;
    }

    // Set linear solver tolerance factor
    flag = CVodeSetEpsLin(cvode_mem, udata->epslin);
    if (check_flag(&flag, "CVodeSetEpsLin", 1))
      return 1;
...
static int SetupDecomp(MPI_Comm comm_w, UserData *udata)
{
  ...
  if (udata->Erecv != NULL || udata->Wrecv != NULL || udata->Srecv != NULL || udata->Nrecv != NULL) {
...
static int f(realtype t, N_Vector u, N_Vector f, void *user_data)
{
  ...
  flag = PostRecv(udata);
  if (check_flag(&flag, "PostRecv", 1))
    return -1;

  // Send exchange data
  flag = SendData(u, udata);
  if (check_flag(&flag, "SendData", 1))
    return -1;
  ...
  if (udata->forcing) {
    ...
    for (j = jstart; j < jend; j++) {
      for (i = istart; i < iend; i++) {
        ...
        farray[IDX(i, j, nx_loc)] = -TWO * PI * sin_sqr_x * sin_sqr_y * sin_t_cos_t -
...
// Jacobian-vector product function
static int JTimes(N_Vector v, N_Vector Jv, realtype t, N_Vector y, N_Vector fy, void *user_data, N_Vector tmp)
{
  int flag;
  ...
  flag = HYPRE_StructVectorSetBoxValues(udata->vvec, udata->ilower, udata->iupper, N_VGetArrayPointer(v));
  if (flag != 0)
    return -1;
  ...
  flag = HYPRE_StructMatrixMatvec(ONE, udata->Jmatrix, udata->vvec, ZERO, udata->Jvvec);
  if (flag != 0)
    return -1;

  // Extract matrix-vector product values
  flag = HYPRE_StructVectorGetBoxValues(udata->Jvvec, udata->ilower, udata->iupper, N_VGetArrayPointer(Jv));
  if (flag != 0)
    return -1;
                  ...
                  void *user_data)
{
  ...
  *jcurPtr = SUNTRUE;
  ...
  if (udata->precond)
    HYPRE_StructPFMGDestroy(udata->precond);
  ...
  // Set up the solver
  flag = HYPRE_StructPFMGSetup(udata->precond, udata->Amatrix, udata->bvec, udata->xvec);
  if (flag != 0)
    return -1;
                  ...
                  void *user_data)
{
  ...
  flag = HYPRE_StructVectorSetBoxValues(udata->bvec, udata->ilower, udata->iupper, N_VGetArrayPointer(r));
  if (flag != 0)
    return -1;
  ...
  flag = HYPRE_StructPFMGSolve(udata->precond, udata->Amatrix, udata->bvec, udata->xvec);
  ...
  flag = HYPRE_StructVectorGetBoxValues(udata->xvec, udata->ilower, udata->iupper, N_VGetArrayPointer(z));
  if (flag != 0)
    return -1;
...
static int SetupHypre(UserData *udata)
{
  ...
  flag = HYPRE_StructMatrixCreate(udata->comm_c, udata->grid, udata->stencil, &(udata->Jmatrix));
  if (flag != 0) {
    delete udata;
    return -1;
  }
  ...
  flag = HYPRE_StructMatrixCreate(udata->comm_c, udata->grid, udata->stencil, &(udata->Amatrix));
  if (flag != 0) {
    delete udata;
    return -1;
  }
...
static int Jac(UserData *udata)
{
  ...
  if ((ilower[0] <= iupper[0]) && (ilower[1] <= iupper[1])) {
    ...
    if (ilower[1] == 0 || iupper[1] == (udata->ny - 1) || ilower[0] == 0 || iupper[0] == (udata->nx - 1)) {
      idx = 0;
      for (iy = 0; iy < ny_loc; iy++) {
        for (ix = 0; ix < nx_loc; ix++) {
          work[idx] = ONE;
    ...
    if (ilower[0] == 0) {
      ...
      if ((bc_ilower[0] <= bc_iupper[0]) && (bc_ilower[1] <= bc_iupper[1])) {
        // Modify the matrix
        flag = HYPRE_StructMatrixSetBoxValues(Jmatrix, bc_ilower, bc_iupper, 5, entries, work);
        if (flag != 0)
          return -1;
      }
    }
    ...
    if ((ilower[0] <= 1) && (iupper[0] >= 1)) {
      ...
      if ((bc_ilower[0] <= bc_iupper[0]) && (bc_ilower[1] <= bc_iupper[1])) {
        // Modify the matrix
        flag = HYPRE_StructMatrixSetBoxValues(Jmatrix, bc_ilower, bc_iupper, 1, entry, work);
        if (flag != 0)
          return -1;
      }
    }
...
static int ScaleAddI(UserData *udata, realtype gamma)
{
  ...
  flag = HYPRE_StructMatrixGetBoxValues(udata->Jmatrix, ilower, iupper, 5, entries, work);
  if (flag != 0)
    return (flag);
  ...
  flag = HYPRE_StructMatrixSetBoxValues(udata->Amatrix, ilower, iupper, 5, entries, work);
  if (flag != 0)
    return (flag);
  ...
  HYPRE_Int entry[1] = {0};
  flag = HYPRE_StructMatrixAddToBoxValues(udata->Amatrix, ilower, iupper, 1, entry, work);
  if (flag != 0)
    return (flag);
...
static int SendData(N_Vector y, UserData *udata)
{
  ...
  if (udata->HaveNbrW) {
    for (i = 0; i < ny_loc; i++)
      udata->Wsend[i] = Y[IDX(0, i, nx_loc)];
    flag = MPI_Isend(udata->Wsend,
                     (int)udata->ny_loc,
                     MPI_SUNREALTYPE,
                     udata->ipW,
                     0,
  ...
  if (udata->HaveNbrS) {
    for (i = 0; i < nx_loc; i++)
      udata->Ssend[i] = Y[IDX(i, 0, nx_loc)];
    flag = MPI_Isend(udata->Ssend,
                     (int)udata->nx_loc,
                     MPI_SUNREALTYPE,
                     udata->ipS,
                     2,
...
UserData::~UserData()
{
  ...
  if (Jvvec != NULL)
    HYPRE_StructVectorDestroy(Jvvec);
  if (precond != NULL)
...
static int OpenOutput(UserData *udata)
{
  ...
  if (udata->output == 2) {
    ...
    fname.str("");
    fname.clear();
    fname << "heat2d_solution." << setfill('0') << setw(5) << udata->myid_c << ".txt";
    ...
    if (udata->forcing) {
      fname.str("");
      fname.clear();
      fname << "heat2d_error." << setfill('0') << setw(5) << udata->myid_c << ".txt";
...
static int OutputStats(void *cvode_mem, UserData *udata)
{
  ...
  if (check_flag(&flag, "CVodeGetNumErrTestFails", 1))
    return -1;
  flag = CVodeGetNumRhsEvals(cvode_mem, &nf);
  if (check_flag(&flag, "CVodeGetNumRhsEvals", 1))
    return -1;
  flag = CVodeGetNumNonlinSolvIters(cvode_mem, &nni);
  if (check_flag(&flag, "CVodeGetNumNonlinSolvIters", 1))
    return -1;
  flag = CVodeGetNumNonlinSolvConvFails(cvode_mem, &ncfn);
  if (check_flag(&flag, "CVodeGetNumNonlinSolvConvFails", 1))
    return -1;
  flag = CVodeGetNumLinIters(cvode_mem, &nli);
  if (check_flag(&flag, "CVodeGetNumLinIters", 1))
    return -1;
  flag = CVodeGetNumLinConvFails(cvode_mem, &nlcf);
  if (check_flag(&flag, "CVodeGetNumLinConvFails", 1))
    return -1;
  flag = CVodeGetNumLinSolvSetups(cvode_mem, &nsetups);
  if (check_flag(&flag, "CVodeGetNumLinSolvSetups", 1))
    return -1;
  flag = CVodeGetNumLinRhsEvals(cvode_mem, &nf_ls);
  if (check_flag(&flag, "CVodeGetNumLinRhsEvals", 1))
    return -1;
  flag = CVodeGetNumJtimesEvals(cvode_mem, &nJv);
  if (check_flag(&flag, "CVodeGetNumJtimesEvals", 1))
    return -1;
  ...
  if (udata->prec) {
    ...
    if (check_flag(&flag, "CVodeGetNumPrecEvals", 1))
      return -1;
    flag = CVodeGetNumPrecSolves(cvode_mem, &nps);
    if (check_flag(&flag, "CVodeGetNumPrecSolves", 1))
      return -1;
...
static int OutputTiming(UserData *udata)
{
  ...
  MPI_Reduce(&(udata->evolvetime), &maxtime, 1, MPI_DOUBLE, MPI_MAX, 0, udata->comm_c);
  if (outproc) {
    cout << "  Evolve time   = " << maxtime << " sec" << endl;
  }

  MPI_Reduce(&(udata->rhstime), &maxtime, 1, MPI_DOUBLE, MPI_MAX, 0, udata->comm_c);
  if (outproc) {
    cout << "  RHS time      = " << maxtime << " sec" << endl;
  }

  MPI_Reduce(&(udata->exchangetime), &maxtime, 1, MPI_DOUBLE, MPI_MAX, 0, udata->comm_c);
  if (outproc) {
  ...
  if (udata->matvec) {
    MPI_Reduce(&(udata->jvtime), &maxtime, 1, MPI_DOUBLE, MPI_MAX, 0, udata->comm_c);
    if (outproc) {
  ...
  if (udata->prec) {
    MPI_Reduce(&(udata->matfilltime), &maxtime, 1, MPI_DOUBLE, MPI_MAX, 0, udata->comm_c);
    if (outproc) {
      cout << "  MatFill time  = " << maxtime << " sec" << endl;
    }

    MPI_Reduce(&(udata->psetuptime), &maxtime, 1, MPI_DOUBLE, MPI_MAX, 0, udata->comm_c);
    if (outproc) {
      cout << "  PSetup time   = " << maxtime << " sec" << endl;
    }

    MPI_Reduce(&(udata->psolvetime), &maxtime, 1, MPI_DOUBLE, MPI_MAX, 0, udata->comm_c);
    if (outproc) {
...
static int check_flag(void *flagvalue, const string funcname, int opt)
{
  ...
  // Check the function return flag value
    ...
    if ((opt == 1 && errflag < 0) || (opt == 2 && errflag != 0)) {
      cerr << endl << "ERROR: " << funcname << " returned with flag = " << errflag << endl << endl;
      return 1;
    }
  }
  else {
