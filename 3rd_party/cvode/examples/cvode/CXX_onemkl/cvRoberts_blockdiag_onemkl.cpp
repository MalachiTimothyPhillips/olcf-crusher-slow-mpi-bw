...
int main(int argc, char *argv[])
{
  ...
  bool direct = true;
  if (argc > 2)
    direct = (atoi(argv[2])) ? true : false;

  // Write the solution to the screen
  bool output = true;
  if (argc > 3)
    output = (atoi(argv[3])) ? true : false;
  ...
  if (direct) {
    ...
    A = SUNMatrix_OneMklDenseBlock(ngroups,
                                   GROUPSIZE,
                                   GROUPSIZE,
                                   SUNMEMTYPE_DEVICE,
                                   memhelper,
                                   &myQueue,
                                   sunctx);
    if (check_retval((void *)A, "SUNMatrix_OneMklDenseBlock", 0))
      return 1;

    // Create the SUNLinearSolver object for use by CVode
    LS = SUNLinSol_OneMklDense(y, A, sunctx);
    if (check_retval((void *)LS, "SUNLinSol_OneMklDense", 0))
      return 1;
  ...
  cout << "Evolution time: " << chrono::duration<double>(tstop - tstart).count() << endl;
