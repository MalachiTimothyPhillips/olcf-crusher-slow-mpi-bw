...
static void InitUserData(int my_pe, sunindextype local_N, MPI_Comm comm, UserData data);
static void SetInitialProfiles(N_Vector u, UserData data);
static void
PrintIntro(int npes, sunindextype mudq, sunindextype mldq, sunindextype mukeep, sunindextype mlkeep);
static void PrintOutput(void *cvode_mem, int my_pe, MPI_Comm comm, N_Vector u, realtype t);
...
static void BSend(MPI_Comm comm,
                  ...
                  sunindextype dsizey,
                  realtype uarray[]);
static void BRecvPost(MPI_Comm comm,
                      MPI_Request request[],
                      int my_pe,
                      int isubx,
                      int isuby,
...
static int flocal(sunindextype Nlocal, realtype t, N_Vector u, N_Vector udot, void *user_data);
...
int main(int argc, char *argv[])
{
  ...
  if (check_retval(&retval, "CVodeInit", 1, my_pe))
    return (1);
  ...
  mukeep = mlkeep = NVARS;
  retval = CVBBDPrecInit(cvode_mem, local_N, mudq, mldq, mukeep, mlkeep, ZERO, flocal, NULL);
  if (check_retval(&retval, "CVBBDPrecInit", 1, my_pe))
    MPI_Abort(comm, 1);

  /* Print heading */
  if (my_pe == 0)
    PrintIntro(npes, mudq, mldq, mukeep, mlkeep);
  ...
  for (jpre = SUN_PREC_LEFT; jpre <= SUN_PREC_RIGHT; jpre++) {
    ...
    for (iout = 1, tout = TWOHR; iout <= NOUT; iout++, tout += TWOHR) {
      ...
      if (check_retval(&retval, "CVode", 1, my_pe))
        break;
                      ...
                      realtype buffer[])
{
  ...
  if (isuby != 0)
    MPI_Irecv(&uext[NVARS], (int)dsizex, MPI_SUNREALTYPE, my_pe - NPEX, 0, comm, &request[0]);

  /* If isuby < NPEY-1, receive data for top x-line of uext */
  if (isuby != NPEY - 1) {
    offsetue = NVARS * (1 + (MYSUB + 1) * (MXSUB + 2));
    MPI_Irecv(&uext[offsetue], (int)dsizex, MPI_SUNREALTYPE, my_pe + NPEX, 0, comm, &request[1]);
  }

  /* If isubx > 0, receive data for left y-line of uext (via bufleft) */
  if (isubx != 0) {
    MPI_Irecv(&bufleft[0], (int)dsizey, MPI_SUNREALTYPE, my_pe - 1, 0, comm, &request[2]);
  }

  /* If isubx < NPEX-1, receive data for right y-line of uext (via bufright) */
  if (isubx != NPEX - 1) {
    MPI_Irecv(&bufright[0], (int)dsizey, MPI_SUNREALTYPE, my_pe + 1, 0, comm, &request[3]);
  }
}
