...
static int Precond(realtype tn,
                   N_Vector u,
                   N_Vector fu,
                   booleantype jok,
                   booleantype *jcurPtr,
...
static void PrintOutput(void *cvode_mem, int my_pe, MPI_Comm comm, N_Vector u, realtype t)
{
  ...
  if (my_pe == npelast) {
    ...
    if (npelast != 0) {
      ...
      if (check_retval(&retval, "MPI_Send", 3, my_pe))
        MPI_Abort(comm, 1);
    }
  }
  ...
  if (my_pe == 0) {
    if (npelast != 0) {
      ...
      if (check_retval(&retval, "MPI_Recv", 3, my_pe))
        MPI_Abort(comm, 1);
    }
...
static void BRecvPost(UserData data)
{
  ...
  if (data->isuby > 0) {
    retval = MPI_Irecv(data->RecvBufferS,
                       NVARS * MXSUB,
                       MPI_SUNREALTYPE,
                       data->my_pe - NPEX,
                       0,
  ...
  if (data->isuby < NPEY - 1) {
    retval = MPI_Irecv(data->RecvBufferN,
                       NVARS * MXSUB,
                       MPI_SUNREALTYPE,
                       data->my_pe + NPEX,
                       1,
  ...
  if (data->isubx > 0) {
    retval = MPI_Irecv(data->RecvBufferW,
                       NVARS * MYSUB,
                       MPI_SUNREALTYPE,
                       data->my_pe - 1,
                       2,
  ...
  if (data->isubx < NPEX - 1) {
    retval = MPI_Irecv(data->RecvBufferE,
                       NVARS * MYSUB,
                       MPI_SUNREALTYPE,
                       data->my_pe + 1,
                       3,
...
static void BSend(realtype c1data[], realtype c2data[], UserData data)
{
  ...
  if (data->isuby > 0) {
    for (lx = 0; lx < MXSUB; lx++)
      data->SendBufferS[lx] = c1data[lx];
    for (lx = 0; lx < MXSUB; lx++)
      data->SendBufferS[MXSUB + lx] = c2data[lx];

    retval = MPI_Isend(data->SendBufferS,
                       NVARS * MXSUB,
                       MPI_SUNREALTYPE,
                       data->my_pe - NPEX,
                       1,
  ...
  if (data->isuby < NPEY - 1) {
    ...
    retval = MPI_Isend(data->SendBufferN,
                       NVARS * MXSUB,
                       MPI_SUNREALTYPE,
                       data->my_pe + NPEX,
                       0,
  ...
  if (data->isubx > 0) {
    ...
    retval = MPI_Isend(data->SendBufferW,
                       NVARS * MYSUB,
                       MPI_SUNREALTYPE,
                       data->my_pe - 1,
                       3,
  ...
  if (data->isubx < NPEX - 1) {
    ...
    retval = MPI_Isend(data->SendBufferE,
                       NVARS * MYSUB,
                       MPI_SUNREALTYPE,
                       data->my_pe + 1,
                       2,
...
static void BRecvWait(UserData data)
{
  ...
  if (data->isuby < NPEY - 1) {
    ...
    for (lx = 0; lx < MXSUB; lx++)
      data->c2ext[(MYSUB + 1) * (MXSUB + 2) + 1 + lx] = data->RecvBufferN[MXSUB + lx];
  }
