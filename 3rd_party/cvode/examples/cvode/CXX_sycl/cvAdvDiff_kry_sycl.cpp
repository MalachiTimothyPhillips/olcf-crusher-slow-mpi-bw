...
int main(int argc, char **argv)
{
  ...
  for (sunindextype tid = 0; tid < data.NEQ; tid++) {
    ...
    udata[tid] = x * (data.xmax - x) * y * (data.ymax - y) * std::exp(FIVE * x * y);
  }
...
static void PrintHeader(realtype reltol, realtype abstol, realtype umax, UserData *data)
{
  ...
  std::cout << "Tolerance parameters: reltol = " << reltol << "   abstol = " << abstol << std::endl
            << std::endl;
...
static void PrintOutput(realtype t, realtype umax, long int nst)
{
  std::cout << "At t = " << t << "   max.norm(u) = " << umax << "   nst = " << nst << std::endl;
  return;
}
