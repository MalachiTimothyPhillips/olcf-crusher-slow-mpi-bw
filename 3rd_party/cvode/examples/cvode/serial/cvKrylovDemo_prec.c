...
static void WebRates(realtype x, realtype y, realtype t, realtype c[], realtype rate[], WebData wdata);
static void fblock(realtype t, realtype cdata[], int jx, int jy, realtype cdotdata[], WebData wdata);
...
static void v_sum_prods(realtype u[], realtype p[], realtype q[], realtype v[], realtype w[], int n);
  ...
  for (jy = 0; jy < MY; jy++) {
    ...
    for (jx = 0; jx < MX; jx++) {
      ...
      for (i = 1; i <= ns; i++) {
        ...
        cdotdata[ici] = coy[i - 1] * (dcyui - dcyli) + cox[i - 1] * (dcxui - dcxli) + fsave[ici];
      }
    }
  }
