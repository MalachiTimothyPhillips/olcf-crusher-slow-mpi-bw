...
static int proj(realtype t, N_Vector yy, N_Vector corr, realtype epsProj, N_Vector err, void *pdata);

/* Functions to integrate the Cartesian and reference solutions */
int GetSol(void *cvode_mem,
           N_Vector yy0,
           realtype rtol,
           realtype atol,
           realtype tf,
...
/* Utility functions */
static int ReadInputs(int *argc,
                      char ***argv,
                      realtype *rtol,
                      realtype *atol,
                      realtype *tf,
           ...
           N_Vector yref)
{
  ...
  /* Open output file */
  if (proj) {
    sprintf(outname, "cvPendulum_dns_rtol_%03.2" ESYM "_atol_%03.2" ESYM "_proj.txt", rtol, atol);
  }
  else {
    sprintf(outname, "cvPendulum_dns_rtol_%03.2" ESYM "_atol_%03.2" ESYM ".txt", rtol, atol);
  }
  FID = fopen(outname, "w");
  ...
  /* Output errors */
  printf("%8.2" ESYM "  %8.2" ESYM "  %8.2" ESYM "  %8.2" ESYM "  |  %8.2" ESYM "  |", x, y, xd, yd, g);
