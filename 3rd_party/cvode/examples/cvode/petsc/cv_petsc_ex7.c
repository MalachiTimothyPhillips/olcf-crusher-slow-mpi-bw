...
int main(int argc, char **argv)
{
  ...
  if (check_retval((void *)nvecx, "N_VMake_Petsc", 0))
    return 1;
  ...
  if (check_retval((void *)NLS, "SUNNonlinSol_PetscSNES", 0))
    return 1;
  ...
  if (check_retval(&ierr, "CVodeSetNonlinearSolver", 1))
    return 1;
  ...
  while (t < tf) {
    ...
    if (check_retval(&ierr, "CVode", 1))
      break;
    ierr = CVodeGetNumSteps(cvode_mem, &nsteps);
    if (check_retval(&ierr, "CVodeGetNumSteps", 1))
      break;
  }
  MyCVodeMonitor(nsteps, t, x, NULL);
  ...
  if (check_retval(&ierr, "SUNonlinSolFree", 0))
    return 1;
  ...
  */
  for (j = ys; j < ys + ym; j++) {
    for (i = xs; i < xs + xm; i++) {
      ...
      /*      f[j][i] = -(uxx + uyy); */
      f[j][i] = -u * (uxx + uyy) -
                (4.0 - 1.0) * ((x[j][i + 1] - x[j][i - 1]) * (x[j][i + 1] - x[j][i - 1]) * .25 * sx +
