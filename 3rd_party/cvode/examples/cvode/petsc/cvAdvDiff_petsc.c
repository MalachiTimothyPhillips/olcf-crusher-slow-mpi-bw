...
int main(int argc, char *argv[])
{
  ...
  if (check_retval((void *)u, "N_VMake_Petsc", 0, my_pe))
    MPI_Abort(comm, 1);
  ...
  for (iout = 1, tout = T1; iout <= NOUT; iout++, tout += DTOUT) {
    ...
    if (check_retval(&retval, "CVodeGetNumSteps", 1, my_pe))
      break;
    umax = N_VMaxNorm(u);
    if (my_pe == 0)
...
static int f(realtype t, N_Vector u, N_Vector udot, void *user_data)
{
  ...
  if (my_pe != 0)
    ...
    z[0] = ZERO;
  if (my_pe != last_pe)
    (void)MPI_Recv(&z[my_length + 1], 1, MPI_SUNREALTYPE, my_pe_p1, 0, comm, &status);
  else
    z[my_length + 1] = ZERO;
