...
__global__ static void j_kernel(int ngroups, int nnzper, realtype *ydata, realtype *Jdata);
...
int main(int argc, char *argv[])
{
  ...
  if (check_retval((void *)y, "N_VNew_Cuda", 0))
    return (1);
  abstol = N_VNew_Cuda(neq, sunctx);
  if (check_retval((void *)abstol, "N_VNew_Cuda", 0))
    return (1);
  ...
  A = SUNMatrix_cuSparse_NewBlockCSR(ngroups,
                                     GROUPSIZE,
                                     GROUPSIZE,
                                     GROUPSIZE * GROUPSIZE,
                                     cusp_handle,
