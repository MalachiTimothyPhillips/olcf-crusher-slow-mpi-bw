...
int main(int argc, char *argv[])
{
  ...
  if (argc < 5) {
    printf(
        "ERROR: FOUR (4) Input required: matrix rows, matrix cols, number of matrix blocks, print timing \n");
    return -1;
  }
  ...
  SUNMatrix A =
      SUNMatrix_OneMklDenseBlock(nblocks, matrows, matcols, SUNMEMTYPE_DEVICE, memhelper, &myQueue, sunctx);
  if (!A) {
...
void sync_device(SUNMatrix A) { ((SUNMatrixContent_OneMklDense)(A->content))->queue->wait_and_throw(); }
