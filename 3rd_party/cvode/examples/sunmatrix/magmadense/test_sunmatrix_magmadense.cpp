...
int main(int argc, char *argv[])
{
  ...
  SUNMemoryHelper memhelper = HIP_OR_CUDA(SUNMemoryHelper_Hip(sunctx);, SUNMemoryHelper_Cuda(sunctx);)
  ...
  x = HIP_OR_CUDA(N_VNew_Hip(matcols * nblocks, sunctx);, N_VNew_Cuda(matcols * nblocks, sunctx);) y =
      HIP_OR_CUDA(N_VNew_Hip(matrows * nblocks, sunctx);, N_VNew_Cuda(matrows * nblocks, sunctx);) A =
          SUNMatrix_MagmaDenseBlock(nblocks, matrows, matcols, SUNMEMTYPE_DEVICE, memhelper, NULL, sunctx);
  if (square)
    I = SUNMatClone(A);
...
void sync_device(SUNMatrix A) { HIP_OR_CUDA(hipDeviceSynchronize();, cudaDeviceSynchronize();) }
