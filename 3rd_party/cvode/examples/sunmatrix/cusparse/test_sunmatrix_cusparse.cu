...
public:
  ...
  virtual size_t gridSize(size_t numWorkElements = 0, size_t blockDim = 0) const { return 1; }

  virtual size_t blockSize(size_t numWorkElements = 0, size_t gridDim = 0) const { return 1; }

  virtual const cudaStream_t *stream() const { return &stream_; }

  virtual SUNCudaExecPolicy *clone() const { return static_cast<SUNCudaExecPolicy *>(new ATestExecPolicy()); }
...
int main(int argc, char *argv[])
{
  ...
  /* check input */
  if (argc < 7) {
    printf("ERROR: SIX (6) inputs required: matrix (filename|random|identity), matrix rows, matrix cols, "
           "number of blocks, matrix type (CSR/BCSR), print timing (0/1)\n");
    return (-1);
  }
  ...
  if (square)
    fails += Test_SetKernelExecPolicy(dI, 0);
...
int check_matrix(SUNMatrix dA, SUNMatrix dB, realtype tol)
{
  ...
  A = SUNSparseMatrix(SUNMatrix_cuSparse_Rows(dA),
                      SUNMatrix_cuSparse_Columns(dA),
                      SUNMatrix_cuSparse_NNZ(dA),
                      CSR_MAT,
                      sunctx);
  B = SUNSparseMatrix(SUNMatrix_cuSparse_Rows(dB),
                      SUNMatrix_cuSparse_Columns(dB),
                      SUNMatrix_cuSparse_NNZ(dB),
                      CSR_MAT,
                      sunctx);

  failure = SUNMatrix_cuSparse_CopyFromDevice(dA, SM_DATA_S(A), SM_INDEXPTRS_S(A), SM_INDEXVALS_S(A));
  failure = SUNMatrix_cuSparse_CopyFromDevice(dB, SM_DATA_S(B), SM_INDEXPTRS_S(B), SM_INDEXVALS_S(B));
  ...
  if (SUNMatrix_cuSparse_SparseType(A) != SUNMatrix_cuSparse_SparseType(B)) {
    printf(">>> ERROR: check_matrix: Different storage types (%d vs %d)\n",
           SUNMatrix_cuSparse_SparseType(A),
           SUNMatrix_cuSparse_SparseType(B));
    SUNMatDestroy(A);
    SUNMatDestroy(B);
    return (1);
  }
