...
/* helper functions */
int csr_from_dense(SUNMatrix Ad,
                   realtype droptol,
                   realtype **matdata,
                   sunindextype **colind,
...
int main(int argc, char *argv[])
{
  ...
  if (argc < 5) {
    if (rank == 0)
      printf(
          "ERROR: FIVE (5) arguments required: matrix size, nprow, npcol, print_all_ranks, print_timing\n");
    return (1);
  }
  ...
  if (grid.iam == 0) {
    ...
    dCreate_CompRowLoc_Matrix_dist(Asuper,
                                   M,
                                   N,
                                   NNZ_local,
                                   M_local,
...
int check_matrix(SUNMatrix A, SUNMatrix B, realtype tol)
{
  ...
  if (Asuper->nrow != Bsuper->nrow) {
    TEST_STATUS3(">>> ERROR: check_matrix: Different numbers of rows (%ld vs %ld)\n",
                 (long int)Asuper->nrow,
                 (long int)Bsuper->nrow,
                 grid->iam);
    return (1);
  }
  ...
  for (i = 0; i < Annz; i++) {
    ...
    if (SUNRCompareTol(Adata[i], Bdata[i], tol)) {
      TEST_STATUS3("Adata[%ld] != Bdata[%ld] \n", (long int)i, (long int)i, grid->iam);
    }
  }
  if (failure > ZERO) {
...
int check_matrix_entry(SUNMatrix A, realtype val, realtype tol)
{
  ...
  for (i = 0; i < nnz_loc; i++) {
    if (SUNRCompareTol(Adata[i], val, tol) != 0) {
      printf("rhs=%g\n", std::abs(val) * tol);
      printf("  Adata[%ld] = %g != %g (err = %g)\n", (long int)i, Adata[i], val, std::abs(Adata[i] - val));
      failure++;
    }
  }
