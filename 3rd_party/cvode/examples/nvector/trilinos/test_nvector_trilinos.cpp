...
int main(int argc, char *argv[])
{
  ...
  RCP<const map_type> testMap =
      rcp(new map_type(global_length, index_base, comm, Tpetra::GloballyDistributed));
  ...
  Teuchos::reduceAll<int, int>(*comm, Teuchos::REDUCE_SUM, fails, Teuchos::outArg(globfails));
 ...
 */
void set_element(N_Vector X, sunindextype i, realtype val) { set_element_range(X, i, i, val); }
...
double max_time(N_Vector X, double time)
{
  ...
  Teuchos::reduceAll<int, double>(*comm, Teuchos::REDUCE_SUM, time, Teuchos::outArg(maxtime));
  return maxtime;
}
