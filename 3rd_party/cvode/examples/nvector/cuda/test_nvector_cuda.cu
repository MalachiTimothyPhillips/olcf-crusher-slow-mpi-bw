...
int main(int argc, char *argv[])
{
  ...
  if (argc < 4) {
    printf("ERROR: THREE (3) Inputs required: vector length, CUDA threads per block (0 for default), print "
           "timing \n");
    Test_Abort(1);
  }
...
booleantype has_data(N_Vector X)
{
  ...
  if ((N_VGetHostArrayPointer_Cuda(X) == NULL) && (N_VGetDeviceArrayPointer_Cuda(X) == NULL))
    return SUNFALSE;
  return SUNTRUE;
}
