...
int main(int argc, char *argv[])
{
  ...
  if (argc < 4) {
    printf("ERROR: THREE (3) Inputs required: vector length, SYCL threads per block (0 for default), print "
           "timing \n");
    Test_Abort(-1);
  }
...
booleantype has_data(N_Vector X)
{
  ...
  if ((N_VGetHostArrayPointer_Sycl(X) == NULL) && (N_VGetDeviceArrayPointer_Sycl(X) == NULL))
    return SUNFALSE;
  return SUNTRUE;
}
