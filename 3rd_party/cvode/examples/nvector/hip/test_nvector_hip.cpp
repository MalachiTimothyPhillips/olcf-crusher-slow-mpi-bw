...
int main(int argc, char *argv[])
{
  ...
  if (argc < 4) {
    printf("ERROR: THREE (3) Inputs required: vector length, hip threads per block (0 for default), print "
           "timing \n");
    Test_Abort(1);
  }
...
booleantype has_data(N_Vector X)
{
  ...
  if ((N_VGetHostArrayPointer_Hip(X) == NULL) && (N_VGetDeviceArrayPointer_Hip(X) == NULL))
    return SUNFALSE;
  return SUNTRUE;
}
