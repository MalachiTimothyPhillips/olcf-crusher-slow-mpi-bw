 ...
 * --------------------------------------------------------------------*/
int Test_N_VWrmsNormMask(N_Vector X, N_Vector W, N_Vector ID, sunindextype local_length, int myid)
{
  int fails = 0, failure = 0;
  double start_time, stop_time, maxt;
  sunindextype global_length;
...
int Test_N_VL1NormLocal(N_Vector X, sunindextype local_length, int myid)
{
  ...
  failure = (ans < ZERO) ? 1 : SUNRCompareTol(ans, (realtype)myid, SUNRsqrt(UNIT_ROUNDOFF));
