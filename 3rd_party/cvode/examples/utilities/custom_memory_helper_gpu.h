...
int MyMemoryHelper_Alloc(SUNMemoryHelper helper,
                         SUNMemory *memptr,
                         size_t memsize,
...
int MyMemoryHelper_Copy(SUNMemoryHelper helper, SUNMemory dst, SUNMemory src, size_t memory_size, void *queue)
{
  switch (src->type) {
  case SUNMEMTYPE_HOST:
    if (dst->type == SUNMEMTYPE_HOST) {
      ...
      MY_GPUCHK(MY_GPU(Memcpy)(dst->ptr, src->ptr, memory_size, MY_GPU(MemcpyHostToDevice)));
    }
    break;
  case SUNMEMTYPE_DEVICE:
    if (dst->type == SUNMEMTYPE_HOST) {
      MY_GPUCHK(MY_GPU(Memcpy)(dst->ptr, src->ptr, memory_size, MY_GPU(MemcpyDeviceToHost)));
    }
    else if (dst->type == SUNMEMTYPE_DEVICE) {
      MY_GPUCHK(MY_GPU(Memcpy)(dst->ptr, src->ptr, memory_size, MY_GPU(MemcpyDeviceToDevice)));
    }
    break;
  default:
    return (-1);
