...
int main(int argc, char *argv[])
{
  ...
  sycl::queue myQueue(selector, sycl::property_list{sycl::property::queue::in_order{}});
  ...
  std::cout << " is accelerator? " << (dev.is_accelerator() ? "Yes" : "No") << std::endl;
  std::cout << " is the queue in order? " << (myQueue.is_in_order() ? "Yes" : "No") << std::endl;
  std::cout << " supports usm host allocations? "
            << (dev.get_info<sycl::info::device::usm_host_allocations>() ? "Yes" : "No") << std::endl;
  std::cout << " supports usm device allocations? "
            << (dev.get_info<sycl::info::device::usm_device_allocations>() ? "Yes" : "No") << std::endl;
  std::cout << " suports usm shared allocations? "
            << (dev.get_info<sycl::info::device::usm_shared_allocations>() ? "Yes" : "No") << std::endl;
  std::cout << " max work group size: " << dev.get_info<sycl::info::device::max_work_group_size>()
            << std::endl;
  std::cout << " max global memory size (bytes): " << dev.get_info<sycl::info::device::global_mem_size>()
            << std::endl;
  std::cout << " max local memory size (bytes): " << dev.get_info<sycl::info::device::local_mem_size>()
            << std::endl;
  std::cout << std::endl;
  ...
  SUNMatrix A;
  if (nblocks > 1) {
    A = SUNMatrix_OneMklDenseBlock(nblocks, rows, cols, SUNMEMTYPE_DEVICE, memhelper, &myQueue, sunctx);
  }
  else {
    A = SUNMatrix_OneMklDense(rows, cols, SUNMEMTYPE_DEVICE, memhelper, &myQueue, sunctx);
  }
  ...
  realtype *Idata = (realtype *)malloc(sizeof(realtype) * SUNMatrix_OneMklDense_LData(I));
  if (!Idata) {
    printf("Data allocation failed\n");
    N_VDestroy(x);
    N_VDestroy(y);
  ...
  for (k = 0; k < nblocks; k++)
    for (j = 0; j < cols; j++)
      for (i = 0; i < rows; i++)
        Adata[k * cols * rows + j * rows + i] = (realtype)rand() / (realtype)RAND_MAX / cols;

  // Create anti-identity matrix
  for (k = 0; k < nblocks; k++)
    for (j = 0; j < cols; j++)
      for (i = 0; i < rows; i++)
        Idata[k * cols * rows + j * rows + i] = ((rows - 1 - i) == j) ? RCONST(1.0) : RCONST(0.0);

  // Add anti-identity to ensure the solver needs to do row-swapping
  for (k = 0; k < nblocks; k++)
    for (i = 0; i < rows; i++)
      for (j = 0; j < cols; j++)
        Adata[k * cols * rows + j * rows + i] += Idata[k * cols * rows + j * rows + i];
