...
int main(int argc, char *argv[])
{
  ...
  B = SUNMatClone(A);
  I = SUNMatClone(A);
  x = HIP_OR_CUDA(N_VNew_Hip(cols * nblocks, sunctx);, N_VNew_Cuda(cols * nblocks, sunctx);) y = N_VClone(x);
  b = N_VClone(x);
...
int check_vector(N_Vector X, N_Vector Y, realtype tol)
{
  ...
  HIP_OR_CUDA(N_VCopyFromDevice_Hip(X);, N_VCopyFromDevice_Cuda(X);)
...
void sync_device() { HIP_OR_CUDA(hipDeviceSynchronize();, cudaDeviceSynchronize();) }
