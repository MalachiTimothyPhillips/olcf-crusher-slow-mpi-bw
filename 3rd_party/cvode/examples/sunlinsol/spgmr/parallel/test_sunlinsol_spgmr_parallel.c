...
int main(int argc, char *argv[])
{
  ...
  /* Create vectors */
  x = N_VNew_Parallel(ProbData.comm, ProbData.Nloc, ProbData.nprocs * ProbData.Nloc, sunctx);
  if (check_flag(x, "N_VNew_Parallel", 0))
    return 1;
  xhat = N_VNew_Parallel(ProbData.comm, ProbData.Nloc, ProbData.nprocs * ProbData.Nloc, sunctx);
  if (check_flag(xhat, "N_VNew_Parallel", 0))
    return 1;
  b = N_VNew_Parallel(ProbData.comm, ProbData.Nloc, ProbData.nprocs * ProbData.Nloc, sunctx);
  if (check_flag(b, "N_VNew_Parallel", 0))
    return 1;
  ProbData.d = N_VNew_Parallel(ProbData.comm, ProbData.Nloc, ProbData.nprocs * ProbData.Nloc, sunctx);
  if (check_flag(ProbData.d, "N_VNew_Parallel", 0))
    return 1;
  ProbData.s1 = N_VNew_Parallel(ProbData.comm, ProbData.Nloc, ProbData.nprocs * ProbData.Nloc, sunctx);
  if (check_flag(ProbData.s1, "N_VNew_Parallel", 0))
    return 1;
  ProbData.s2 = N_VNew_Parallel(ProbData.comm, ProbData.Nloc, ProbData.nprocs * ProbData.Nloc, sunctx);
  if (check_flag(ProbData.s2, "N_VNew_Parallel", 0))
    return 1;
  ...
  fails += Test_SUNLinSolSetPreconditioner(LS, &ProbData, PSetup, PSolve, ProbData.myid);
  fails += Test_SUNLinSolSetScalingVectors(LS, ProbData.s1, ProbData.s2, ProbData.myid);
...
int ATimes(void *Data, N_Vector v_vec, N_Vector z_vec)
{
  ...
  if (ProbData->myid > 0) { /* left neighbor exists */
    ierr = MPI_Irecv(&vL, 1, MPI_SUNREALTYPE, ProbData->myid - 1, MPI_ANY_TAG, ProbData->comm, &RecvReqL);
    if (ierr != MPI_SUCCESS)
      return 1;
    ierr = MPI_Isend(&vsL, 1, MPI_SUNREALTYPE, ProbData->myid - 1, 0, ProbData->comm, &SendReqL);
    if (ierr != MPI_SUCCESS)
      return 1;
  }
  if (ProbData->myid < ProbData->nprocs - 1) { /* right neighbor exists */
    ierr = MPI_Irecv(&vR, 1, MPI_SUNREALTYPE, ProbData->myid + 1, MPI_ANY_TAG, ProbData->comm, &RecvReqR);
    if (ierr != MPI_SUCCESS)
      return 1;
    ierr = MPI_Isend(&vsR, 1, MPI_SUNREALTYPE, ProbData->myid + 1, 1, ProbData->comm, &SendReqR);
    if (ierr != MPI_SUCCESS)
      return 1;
  }
